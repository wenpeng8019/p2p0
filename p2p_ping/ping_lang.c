...
static const char* messages_en[MSG_PING_COUNT] = {
    ...
    [MSG_PING_OPT_LAN_PUNCH]    = "  --lan-punch       Test PUNCH/PUNCH_ACK state machine over LAN (skips STUN/TURN, uses nat_start_punch)",
    ...
    [MSG_PING_LAN_PUNCH]        = "[TEST] LAN punch mode: PUNCH/PUNCH_ACK over Host candidates (nat_start_punch)",
...
static const char* messages_zh[MSG_PING_COUNT] = {
    ...
    [MSG_PING_OPT_LAN_PUNCH]    = "  --lan-punch       在 LAN 内测试打洞状态机（跳过 STUN/TURN，使用 nat_start_punch）",
    ...
    [MSG_PING_LAN_PUNCH]        = "[测试] LAN 打洞模式：通过 Host 候选走完整打洞状态机（nat_start_punch）",
