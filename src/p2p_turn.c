...
int p2p_turn_allocate(p2p_session_t *s) {
    ...
    if (!he) {
        print("E:", LA_F("%s %s", LA_F6, 263), LA_W("Failed to resolve TURN server:", LA_W29, 35), s->cfg.turn_server);
        return -1;
    }
                ...
                if (attr_len >= 8 && buf[offset + 1] == 0x01) {  /* 0x01 = IPv4 */
                    ...
                    /* 将中继地址加入本地 ICE 候选列表 */
                    {
                        ...
                        if (idx >= 0) {
                            ...
                            print("I:", LA_F("%s %s:%u (%s=%u)", LA_F23, 280),
                                         LA_W("Gathered Relay Candidate", LA_W36, 43),
                                         inet_ntoa(c->addr.sin_addr), ntohs(c->addr.sin_port),
                                         LA_W("priority", LA_W72, 81), c->priority);
