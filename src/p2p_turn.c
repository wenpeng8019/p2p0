...
int p2p_turn_allocate(p2p_session_t *s) {
    ...
    P2P_LOG_INFO("TURN", "%s %s:%d",
                 LA_W("Sending Allocate Request to", LA_W111), s->cfg.turn_server, s->cfg.turn_port ? s->cfg.turn_port : 3478);
    ...
    if (!he) {
        P2P_LOG_ERROR("TURN", "%s %s", LA_W("Failed to resolve TURN server:", LA_W34), s->cfg.turn_server);
        return -1;
    }
...
void p2p_turn_handle_packet(p2p_session_t *s, const uint8_t *buf, int len, 
    ...
    if (type == TURN_ALLOCATE_SUCCESS) {
        P2P_LOG_INFO("TURN", "%s", LA_S("Allocation successful!", LA_S6));
                    ...
                    /* 将中继地址加入本地 ICE 候选列表 */
                    {
                        ...
                        if (c) {
                            ...
                            P2P_LOG_INFO("ICE", "%s %s:%u (%s=%u)",
                                         LA_W("Gathered Relay Candidate", LA_W42),
                                         inet_ntoa(c->addr.sin_addr), ntohs(c->addr.sin_port),
                                         LA_W("priority", LA_W80), c->priority);
