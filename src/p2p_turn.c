...
static int allocate_auth(struct p2p_instance *inst) {
    ...
    if (!t->has_key) {
        if (!inst->cfg.turn_user || !inst->cfg.turn_pass) {
            print("E:", LA_F("TURN auth required but no credentials configured", LA_F439, 439));
            t->state = TURN_FAILED;
            return -1;
        }
...
int p2p_turn_allocate(struct p2p_instance *inst) {
    ...
    if (!resolve_server(t, inst->cfg.turn_server, inst->cfg.turn_port)) {
        print("E:", LA_F("Failed to resolve TURN server: %s", LA_F314, 314), inst->cfg.turn_server);
        t->state = TURN_FAILED;
        return -1;
    }

    print("I:", LA_F("Sending Allocate Request to %s:%d", LA_F408, 408),
    ...
    if (ret <= 0) {
        print("E:", LA_F("Failed to send Allocate Request: %d", LA_F315, 315), ret);
        return -1;
    }
...
int p2p_turn_handle_packet(struct p2p_instance *inst, const struct sockaddr_in *from,
    ...
    if (type == TURN_ALLOCATE_SUCCESS) {
        ...
        print("I:", LA_F("TURN Allocated relay %s:%u (lifetime=%us)", LA_F433, 433),
        ...
        for (struct p2p_session *s = inst->sessions_head; s; s = s->next) {
            ...
            print("I:", LA_F("Gathered Relay Candidate %s:%u (priority=%u)", LA_F323, 323),
    ...
    if (type == TURN_ALLOCATE_ERROR) {
        ...
        if (error_code == 401 && t->state == TURN_ALLOCATING && realm[0] && nonce[0]) {
            print("I:", LA_F("TURN 401 Unauthorized (realm=%s), authenticating...", LA_F431, 431), realm);
        ...
        print("E:", LA_F("TURN Allocate failed with error %d", LA_F432, 432), error_code);
        t->state = TURN_FAILED;
    ...
    if (type == TURN_CREATE_PERM_ERROR) {
        ...
        print("E:", LA_F("TURN CreatePermission failed (error=%d)", LA_F434, 434), error_code);
        return 0;
    }
    ...
    if (type == TURN_REFRESH_SUCCESS) {
        ...
        print("V:", LA_F("TURN Refresh ok (lifetime=%us)", LA_F438, 438), lifetime);
        return 0;
    }
    ...
    if (type == TURN_REFRESH_ERROR) {
        ...
        print("E:", LA_F("TURN Refresh failed (error=%d)", LA_F437, 437), error_code);
        return 0;
    }
    ...
    if (type == TURN_DATA_INDICATION) {
        ...
        if (data && data_len > 0 && peer.sin_family == AF_INET) {
            print("V:", LA_F("TURN Data Indication from %s:%u (%d bytes)", LA_F436, 436),
...
static int turn_create_permission(struct p2p_instance *inst, const struct sockaddr_in *peer_addr) {
    ...
    print("V:", LA_F("TURN CreatePermission for %s", LA_F435, 435),
