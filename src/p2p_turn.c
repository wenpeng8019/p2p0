...
int p2p_turn_allocate(p2p_session_t *s) {
    ...
    print("I:", LA_F("Sending Allocate Request to %s:%d", LA_F172, 264),
    ...
    if (!he) {
        print("E:", LA_F("Failed to resolve TURN server: %s", LA_F134, 226), s->cfg.turn_server);
        return -1;
    }
...
void p2p_turn_handle_packet(p2p_session_t *s, const uint8_t *buf, int len, 
    ...
    if (type == TURN_ALLOCATE_SUCCESS) {
        print("I: %s", LA_S("Allocation successful!", LA_S12, 37));
                ...
                if (attr_len >= 8 && buf[offset + 1] == 0x01) {  /* 0x01 = IPv4 */
                    ...
                    /* 将中继地址加入本地 ICE 候选列表 */
                    {
                        ...
                        if (idx >= 0) {
                            ...
                            print("I:", LA_F("Gathered Relay Candidate %s:%u (priority=%u)", LA_F137, 229),
