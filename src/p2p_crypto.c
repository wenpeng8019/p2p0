...
/*
 * MD5 Implementation (RFC 1321) — TURN long-term credential key derivation
 */
typedef struct {
    uint32_t state[4];
    uint64_t count;
...
void p2p_md5(const uint8_t *data, int len, uint8_t digest[16]) {
    ...
    for (int j = 0; j < 8; j++)
        ctx.buffer[56 + j] = (uint8_t)(bits >> (j * 8));
    MD5Transform(ctx.state, ctx.buffer);

    for (int j = 0; j < 4; j++) {
        digest[j*4+0] = (uint8_t)(ctx.state[j]);
        digest[j*4+1] = (uint8_t)(ctx.state[j] >> 8);
        digest[j*4+2] = (uint8_t)(ctx.state[j] >> 16);
        digest[j*4+3] = (uint8_t)(ctx.state[j] >> 24);
    }
}


/*
 * SHA1 Implementation (Simplified for internal use)
 */
typedef struct {
    uint32_t state[5];
    uint32_t count[2];
...
static void SHA1Final(unsigned char digest[20], SHA1_CTX* context) {
    ...
    while ((context->count[0] & 504) != 448) SHA1Update(context, (const unsigned char *)"\0", 1);
