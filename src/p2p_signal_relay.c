...
ret_t p2p_signal_relay_login(p2p_signal_relay_ctx_t *ctx, const char *server_ip, int port, const char *my_name) {
    ...
    print("I:", LA_F("Connected to server %s:%d as '%s'", LA_F126, 222), server_ip, port, my_name);
    return E_NONE;
}
...
ret_t p2p_signal_relay_send_connect(p2p_signal_relay_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    if (send(ctx->fd, (const char *)&hdr, sizeof(hdr), 0) != sizeof(hdr)) {
        print("E: %s", LA_S("Failed to send header", LA_S38, 59));
        return -1;
    }
    if (send(ctx->fd, target, P2P_PEER_ID_MAX, 0) != P2P_PEER_ID_MAX) {
        print("E: %s", LA_S("Failed to send target name", LA_S41, 62));
        return -1;
    }
    if (send(ctx->fd, (const char *)data, len, 0) != len) {
        print("E: %s", LA_S("Failed to send payload", LA_S39, 60));
        return -1;
    }
    
    print("I:", LA_F("Sent connect request to '%s' (%d bytes)", LA_F203, 299), target_name, len);
...
int p2p_signal_relay_reply_connect(p2p_signal_relay_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    print("I:", LA_F("Sent answer to '%s' (%d bytes)", LA_F202, 298), target_name, len);
    return 0;
}
...
void p2p_signal_relay_tick_recv(p2p_signal_relay_ctx_t *ctx, struct p2p_session *s) {
    ...
    if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
        print("I: %s", LA_S("P2P connected, closing signaling TCP connection", LA_S52, 73));
        p2p_signal_relay_close(ctx);
        return;
    }
    ...
    if (ctx->waiting_for_peer) {
        ...
        if (elapsed > P2P_RELAY_PEER_WAIT_TIMEOUT_MS) {
            print("W:", LA_F("Waiting for peer '%s' timed out (%dms), giving up", LA_F219, 314),
    ...
    for(;;) {
        ...
        case RELAY_READ_HEADER: {
            ...
            if (n == 0) {
                /* 连接关闭 */
                print("W: %s", LA_S("Connection closed by server", LA_S23, 46));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                /* 真正的错误 */
                print("E:", LA_F("recv error %d", LA_F234, 326), P_sock_errno());
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (ctx->read_hdr.magic != P2P_RLY_MAGIC) {
                    print("W:", LA_F("Invalid magic 0x%x (expected 0x%x), resetting", LA_F149, 245),
                           ctx->read_hdr.magic, P2P_RLY_MAGIC);
                    ctx->read_state = RELAY_READ_IDLE;
                    return;
                }
                
                printf(LA_F("relay_tick: recv header complete, magic=0x%x, type=%d, length=%u", LA_F238, 330),
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER || ctx->read_hdr.type == P2P_RLY_FORWARD) {
                    ...
                    if (ctx->read_hdr.length > 0) {
                        ...
                        if (!ctx->read_payload) {
                            print("E: %s", LA_S("Failed to allocate ACK payload buffer", LA_S29, 52));
                            ctx->read_state = RELAY_READ_IDLE;
                            return;
                        }
                        ctx->read_offset = 0;
        ...
        case RELAY_READ_SENDER: {
            ...
            if (n == 0) {
                print("W: %s", LA_S("Connection closed while reading sender", LA_S26, 49));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("recv error %d while reading sender", LA_F237, 329), P_sock_errno());
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (payload_len > 0) {
                    ...
                    if (!ctx->read_payload) {
                        print("E:", LA_F("Failed to allocate %u bytes", LA_F133, 229), payload_len);
                        ctx->read_state = RELAY_READ_IDLE;
                        return;
                    }
        ...
        case RELAY_READ_PAYLOAD: {
            ...
            if (n == 0) {
                print("W: %s", LA_S("Connection closed while reading payload", LA_S25, 48));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("recv error %d while reading payload", LA_F236, 328), P_sock_errno());
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (ctx->read_hdr.type == P2P_RLY_CONNECT_ACK) {
                    if (payload_len >= sizeof(p2p_relay_connect_ack_t)) {
                        ...
                        print("I:", LA_F("Received ACK (status=%d, candidates_acked=%d)", LA_F169, 265),
                        ...
                        switch (ack->status) {
                            case 0:  // 对端在线
                                print("I:", LA_F("Peer online, forwarded %d candidates", LA_F165, 261),
                                       ack->candidates_acked);
                                ctx->waiting_for_peer = false;
                                break;
                            case 1:  // 对端离线，已缓存
                                print("I:", LA_F("Peer offline, cached %d candidates", LA_F164, 260), ack->candidates_acked);
                                ctx->waiting_for_peer = false;
                                break;
                            case 2:  // 缓存已满
                                print("I: %s", LA_S("Storage full, waiting for peer to come online", LA_S74, 93));
                                ctx->waiting_for_peer = true;
                                ctx->waiting_start_time = P_tick_ms();
                                break;
                            default:
                                print("W:", LA_F("Unknown ACK status %d", LA_F216, 311), ack->status);
                                break;
                        }
                    }
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER) {
                    ...
                    if (s->remote_peer_id[0] == '\0') {
                        ...
                        print("I:", LA_F("Passive peer learned remote ID '%s' from OFFER", LA_F159, 255), ctx->read_sender);
                    }
                }
                ...
                if (ctx->read_hdr.type == P2P_RLY_FORWARD && ctx->waiting_for_peer &&
                    ...
                    print("I:", LA_F("Peer '%s' is now online (FORWARD received), resuming", LA_F163, 259), ctx->read_sender);
                    ctx->waiting_for_peer = false;
                    ctx->waiting_target[0] = '\0';
                }
                
                print("I:", LA_F("%s '%s' (%u %s)", LA_F4, 100), LA_W("Received signal from", LA_W13, 14), ctx->read_sender, payload_len, LA_W("bytes", LA_W1, 2));
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER || ctx->read_hdr.type == P2P_RLY_FORWARD) {
                    ...
                    if (should_reset && (s->remote_cand_cnt > 0 || s->ice_state != P2P_ICE_STATE_INIT)) {
                        printf(LA_F("%s received (ice_state=%d), resetting ICE and clearing %d stale candidates", LA_F9, 105),
                ...
                if (payload_len >= 76 && unpack_signaling_payload_hdr(&p, ctx->read_payload) == 0 &&
                    ...
                    for (int i = 0; i < p.candidate_count; i++) {
                        ...
                        if (idx >= 0) {
                            ...
                            print("I:", LA_F("Added Remote Candidate: %d -> %s:%d", LA_F118, 214),
                            ...
                            if (s->ice_state == P2P_ICE_STATE_CHECKING) {
                                ...
                                print("I:", LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F227, 320),
        ...
        case RELAY_READ_DISCARD: {
            ...
            if (!ctx->read_payload && ctx->read_expected > 0) {
                ...
                if (!ctx->read_payload) {
                    print("E: %s", LA_S("Failed to allocate discard buffer, closing connection", LA_S30, 53));
                    p2p_signal_relay_close(ctx);
                    return;
                }
            }
            ...
            if (n == 0) {
                print("W: %s", LA_S("Connection closed while discarding", LA_S24, 47));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("recv error %d while discarding", LA_F235, 327), P_sock_errno());
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                printf(LA_F("Discarded %d bytes payload of message type %d", LA_F132, 228),
        ...
        default:
            /* 不应该到达这里 */
            print("W:", LA_F("Invalid read state %d, resetting", LA_F150, 246), ctx->read_state);
            ctx->read_state = RELAY_READ_IDLE;
            break;
        }
    } // for(;;)
            ...
            if (start_idx < s->local_cand_cnt) {
                ...
                if (n > 0) {
                    ...
                    if (ret >= 0) {
                        ...
                        if (ret > 0) {
                            print("I:", LA_F("[SIGNALING] Sent candidates, forwarded [%d-%d] to %s (forwarded=%d)", LA_F226, 319),
                                         start_idx, start_idx + batch_size - 1,
                                         s->remote_peer_id,
                                         ret);
                        } else {
                            print("I:", LA_F("[SIGNALING] Sent candidates (cached, peer offline) %d to %s", LA_F225, 318),
                                         batch_size, s->remote_peer_id);
                        }
                    } else if (ret == -2) {
                        /* 服务器缓存满（status=2）：停止发送，等待对端上线后收到 FORWARD */
                        /* waiting_for_peer 已在 send_connect() 中设置为 true */
                        print("W: %s", LA_S("[SIGNALING] Server storage full, waiting for peer to come online", LA_S11, 35));
                    } else {
                        s->cands_pending_send = true;  /* TCP 发送失败（-1/-3），标记待重发 */
                        print("W:", LA_F("[SIGNALING] Failed to send candidates, will retry (ret=%d)", LA_F224, 317), ret);
                    }
                }
