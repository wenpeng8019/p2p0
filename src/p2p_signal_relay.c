...
ret_t p2p_signal_relay_login(p2p_signal_relay_ctx_t *ctx, const char *server_ip, int port, const char *my_name) {
    ...
    print("I:", LA_F("%s %s:%d %s '%s'", LA_F19, 225), LA_W("Connected to server", LA_W14, 16), server_ip, port, LA_W("as", LA_W7, 9), my_name);
    return E_NONE;
}
...
ret_t p2p_signal_relay_send_connect(p2p_signal_relay_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    if (send(ctx->fd, (const char *)&hdr, sizeof(hdr), 0) != sizeof(hdr)) {
        print("E:", LA_S("Failed to send header", LA_S37, 134));
        return -1;
    }
    if (send(ctx->fd, target, P2P_PEER_ID_MAX, 0) != P2P_PEER_ID_MAX) {
        print("E:", LA_S("Failed to send target name", LA_S40, 137));
        return -1;
    }
    if (send(ctx->fd, (const char *)data, len, 0) != len) {
        print("E:", LA_S("Failed to send payload", LA_S38, 135));
        return -1;
    }
    
    print("I:", LA_F("%s %s '%s' (%d %s)", LA_F16, 222), LA_W("Sent connect", LA_W78, 81), LA_S("request to", LA_S72, 189), target_name, len, LA_W("bytes", LA_W10, 12));
...
int p2p_signal_relay_reply_connect(p2p_signal_relay_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    print("I:", LA_F("%s '%s' (%d %s)", LA_F26, 232), LA_W("Sent answer to", LA_W75, 78), target_name, len, LA_W("bytes", LA_W10, 12));
    return 0;
}
...
void p2p_signal_relay_tick_recv(p2p_signal_relay_ctx_t *ctx, struct p2p_session *s) {
    ...
    if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
        print("I: %s", LA_S("P2P connected, closing signaling TCP connection", LA_S57, 154));
        p2p_signal_relay_close(ctx);
        return;
    }
    ...
    if (ctx->waiting_for_peer) {
        ...
        if (elapsed > P2P_RELAY_PEER_WAIT_TIMEOUT_MS) {
            print("W:", LA_F("%s '%s' %s (%dms), %s", LA_F23, 229),
                   LA_W("Waiting for peer", LA_W95, 98), ctx->waiting_target, LA_W("timed out", LA_W85, 88),
                   P2P_RELAY_PEER_WAIT_TIMEOUT_MS, LA_W("giving up", LA_W31, 33));
    ...
    for(;;) {
        ...
        case RELAY_READ_HEADER: {
            ...
            if (n == 0) {
                /* 连接关闭 */
                print("W:", LA_S("Connection closed by server", LA_S22, 119));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                /* 真正的错误 */
                print("E:", LA_F("%s %d", LA_F4, 210), LA_W("recv error", LA_W64, 67), P_sock_errno());
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (ctx->read_hdr.magic != P2P_RLY_MAGIC) {
                    print("W:", LA_F("%s 0x%x (%s 0x%x), %s", LA_F31, 237),
                           LA_W("Invalid magic", LA_W33, 35), ctx->read_hdr.magic,
                           LA_W("expected", LA_W19, 21), P2P_RLY_MAGIC, LA_W("resetting", LA_W68, 71));
                    ctx->read_state = RELAY_READ_IDLE;
                    return;
                }
                
                printf(LA_F("[DEBUG] relay_tick: recv header complete, magic=0x%x, type=%d, length=%u", LA_F212, 404),
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER || ctx->read_hdr.type == P2P_RLY_FORWARD) {
                    ...
                    if (ctx->read_hdr.length > 0) {
                        ...
                        if (!ctx->read_payload) {
                            print("E:", LA_S("Failed to allocate ACK payload buffer", LA_S30, 127));
                            ctx->read_state = RELAY_READ_IDLE;
                            return;
                        }
                        ctx->read_offset = 0;
        ...
        case RELAY_READ_SENDER: {
            ...
            if (n == 0) {
                print("W:", LA_S("Connection closed while reading sender", LA_S25, 122));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("%s %d %s", LA_F5, 211), LA_W("recv error", LA_W64, 67), P_sock_errno(), LA_W("while reading sender", LA_W98, 101));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (payload_len > 0) {
                    ...
                    if (!ctx->read_payload) {
                        print("E:", LA_F("%s %u %s", LA_F21, 227), LA_W("Failed to allocate", LA_W20, 22), payload_len, LA_W("bytes", LA_W10, 12));
                        ctx->read_state = RELAY_READ_IDLE;
                        return;
                    }
        ...
        case RELAY_READ_PAYLOAD: {
            ...
            if (n == 0) {
                print("W:", LA_S("Connection closed while reading payload", LA_S24, 121));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("%s %d %s", LA_F5, 211), LA_W("recv error", LA_W64, 67), P_sock_errno(), LA_W("while reading payload", LA_W97, 100));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                ...
                if (ctx->read_hdr.type == P2P_RLY_CONNECT_ACK) {
                    if (payload_len >= sizeof(p2p_relay_connect_ack_t)) {
                        ...
                        print("I:", LA_F("%s (status=%d, candidates_acked=%d)", LA_F30, 236),
                               LA_W("Received ACK", LA_W60, 63), ack->status, ack->candidates_acked);
                        ...
                        switch (ack->status) {
                            case 0:  // 对端在线
                                print("I:", LA_F("%s, %s %d %s", LA_F59, 264),
                                       LA_W("Peer online", LA_W53, 56), LA_S("forwarded", LA_S41, 138), ack->candidates_acked, LA_W("candidates", LA_W12, 14));
                                ctx->waiting_for_peer = false;
                                break;
                            case 1:  // 对端离线，已缓存
                                print("I:", LA_F("%s, %s %d %s", LA_F59, 264),
                                       LA_W("Peer offline", LA_W52, 55), LA_S("cached", LA_S17, 114), ack->candidates_acked, LA_W("candidates", LA_W12, 14));
                                ctx->waiting_for_peer = false;
                                break;
                            case 2:  // 缓存已满
                                print("I:", LA_F("%s, %s", LA_F58, 263),
                                       LA_W("Storage full", LA_W80, 83), LA_S("waiting for peer to come online", LA_S88, 205));
                                ctx->waiting_for_peer = true;
                                ctx->waiting_start_time = P_tick_ms();
                                break;
                            default:
                                print("W:", LA_F("%s %d", LA_F4, 210), LA_W("Unknown ACK status", LA_W92, 95), ack->status);
                                break;
                        }
                    }
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER) {
                    ...
                    if (s->remote_peer_id[0] == '\0') {
                        ...
                        print("I:", LA_F("%s '%s' %s OFFER", LA_F25, 231),
                               LA_W("Passive peer learned remote ID", LA_W49, 52), ctx->read_sender, LA_W("from", LA_W26, 28));
                    }
                }
                ...
                if (ctx->read_hdr.type == P2P_RLY_FORWARD && ctx->waiting_for_peer &&
                    ...
                    print("I:", LA_F("%s '%s' %s (%s FORWARD), %s", LA_F24, 230),
                           LA_W("Peer", LA_W51, 54), ctx->read_sender, LA_W("is now online", LA_W35, 37),
                           LA_S("received", LA_S68, 185), LA_S("resuming", LA_S73, 190));
                    ctx->waiting_for_peer = false;
                    ctx->waiting_target[0] = '\0';
                }
                
                print("I:", LA_F("%s '%s' (%u %s)", LA_F27, 233), LA_W("Received signal from", LA_W63, 66), ctx->read_sender, payload_len, LA_W("bytes", LA_W10, 12));
                ...
                if (ctx->read_hdr.type == P2P_RLY_OFFER || ctx->read_hdr.type == P2P_RLY_FORWARD) {
                    ...
                    if (should_reset && (s->remote_cand_cnt > 0 || s->ice_state != P2P_ICE_STATE_INIT)) {
                        printf(LA_F("[DEBUG] %s received (ice_state=%d), resetting ICE and clearing %d stale candidates", LA_F210, 402),
                ...
                if (payload_len >= 76 && unpack_signaling_payload_hdr(&p, ctx->read_payload) == 0 &&
                    ...
                    for (int i = 0; i < p.candidate_count; i++) {
                        ...
                        if (idx >= 0) {
                            ...
                            print("I:", LA_F("%s: %d -> %s:%d", LA_F62, 268),
                                   LA_W("Added Remote Candidate", LA_W3, 4), rc->cand.type, inet_ntoa(rc->cand.addr.sin_addr), ntohs(rc->cand.addr.sin_port));

                            /* Trickle ICE：如果 ICE 已在 CHECKING 状态，立即向新候选发送探测包 */
                            if (s->ice_state == P2P_ICE_STATE_CHECKING) {

                                print("I:", LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F215, 407),
        ...
        case RELAY_READ_DISCARD: {
            ...
            if (!ctx->read_payload && ctx->read_expected > 0) {
                ...
                if (!ctx->read_payload) {
                    print("E:", LA_S("Failed to allocate discard buffer, closing connection", LA_S31, 128));
                    p2p_signal_relay_close(ctx);
                    return;
                }
            }
            ...
            if (n == 0) {
                print("W:", LA_S("Connection closed while discarding", LA_S23, 120));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (n < 0) {
                ...
                print("E:", LA_F("%s %d %s", LA_F5, 211), LA_W("recv error", LA_W64, 67), P_sock_errno(), LA_W("while discarding", LA_W96, 99));
                p2p_signal_relay_close(ctx);
                return;
            }
            ...
            if (ctx->read_offset >= ctx->read_expected) {
                printf(LA_F("[DEBUG] %s %d %s %s %d", LA_F209, 401),
                       LA_W("Discarded", LA_W18, 20), ctx->read_expected, LA_W("bytes", LA_W10, 12),
                       LA_S("payload of message type", LA_S62, 158), ctx->read_hdr.type);
        ...
        default:
            /* 不应该到达这里 */
            print("W:", LA_F("%s %d, %s", LA_F6, 212), LA_W("Invalid read state", LA_W34, 36), ctx->read_state, LA_W("resetting", LA_W68, 71));
            ctx->read_state = RELAY_READ_IDLE;
            break;
        }
    } // for(;;)
                    ...
                    if (ret >= 0) {
                        ...
                        if (ret > 0) {
                            print("I: [SIGNALING] %s [%d-%d] %s %s (%s=%d)",
                                         LA_W("Sent candidates, forwarded", LA_W77, 80),
                                         start_idx, start_idx + batch_size - 1,
                                         LA_W("to", LA_W88, 91), s->remote_peer_id,
                                         LA_W("forwarded", LA_W25, 27), ret);
                        } else {
                            print("I: [SIGNALING] %s %d %s %s",
                                         LA_W("Sent candidates (cached, peer offline)", LA_W76, 79),
                                         batch_size, LA_W("to", LA_W88, 91), s->remote_peer_id);
                        }
                    } else if (ret == -2) {
                        /* 服务器缓存满（status=2）：停止发送，等待对端上线后收到 FORWARD */
                        /* waiting_for_peer 已在 send_connect() 中设置为 true */
                        print("W: [SIGNALING] %s", LA_W("Server storage full, waiting for peer to come online", LA_W79, 82));
                    } else {
                        s->cands_pending_send = true;  /* TCP 发送失败（-1/-3），标记待重发 */
                        print("W: [SIGNALING] %s (ret=%d)", LA_W("Failed to send candidates, will retry", LA_W23, 25), ret);
                    }
                }
