...
int p2p_tcp_punch_connect(p2p_session_t *s, const struct sockaddr_in *remote) {
    ...
    if (bind(sock, (struct sockaddr *)&loc, sizeof(loc)) < 0) {
        ...
        printf(LA_F("%s %s %d, %s", LA_F8, 264), LA_W("Bind failed", LA_W9, 16),
                      LA_S("to", LA_S91, 209), s->cfg.tcp_port, LA_W("port busy, trying random port", LA_W64, 79));
        loc.sin_port = 0;
        if (bind(sock, (struct sockaddr *)&loc, sizeof(loc)) < 0) {
    ...
    }
    {
        ...
        printf(LA_F("%s :%d", LA_F34, 295), LA_W("Bound to", LA_W10, 17), ntohs(bound.sin_port));
    }
    ...
    /* 进行三次握手的"同时发起"尝试 */
    print("I:", LA_F("Attempting Simultaneous Open to %s:%d", LA_F59, 315),
