...
int p2p_tcp_punch_connect(struct p2p_session *s, const struct sockaddr_in *remote) {
    ...
    if (bind(sock, (struct sockaddr *)&loc, sizeof(loc)) < 0) {
        /* 如果端口被占用，尝试随机端口并更新配置 */
        printf(LA_F("Bind failed to %d, port busy, trying random port", LA_F284, 284), 
                      s->inst->cfg.tcp_port);
        loc.sin_port = 0;
        if (bind(sock, (struct sockaddr *)&loc, sizeof(loc)) < 0) {
            print("E:", LA_F("Bind failed", LA_F283, 283));
             P_sock_close(sock);
             return -1;
        }
    }
    {
        ...
        printf(LA_F("Bound to :%d", LA_F285, 285), ntohs(bound.sin_port));
    }
    ...
    /* 进行三次握手的"同时发起"尝试 */
    print("I:", LA_F("Attempting Simultaneous Open to %s:%d", LA_F280, 280),
