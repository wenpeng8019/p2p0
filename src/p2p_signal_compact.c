...
static int build_peer_info_packet(p2p_session_t *s, uint16_t seq,
                                  uint8_t *flags_out,
                                  uint8_t *payload, int *payload_len_out) {
    p2p_signal_compact_ctx_t *ctx = &s->sig_compact_ctx;

    int cand_unit = (P2P_MAX_PAYLOAD - 8 - 1 - 1) / (int)sizeof(p2p_compact_candidate_t);
    if (cand_unit > MAX_CANDS_PER_PACKET) cand_unit = MAX_CANDS_PER_PACKET;
    if (cand_unit <= 0) return -1;

    int start = ctx->candidates_cached + (int)(seq - 1) * cand_unit;
    if (start > s->local_cand_cnt) start = s->local_cand_cnt;

    int remaining = s->local_cand_cnt - start;
    int cnt = remaining > cand_unit ? cand_unit : remaining;
    if (cnt < 0) cnt = 0;

    uint8_t flags = 0;
    if (start + cnt >= s->local_cand_cnt) {
    ...
    for (int i = 0; i < cnt; i++) {
        int idx = start + i;
        payload[off] = (uint8_t)s->local_cands[idx].type;
        memcpy(payload + off + 1, &s->local_cands[idx].addr.sin_addr.s_addr, 4);
        memcpy(payload + off + 5, &s->local_cands[idx].addr.sin_port, 2);
        off += (int)sizeof(p2p_compact_candidate_t);
    }
...
static void send_rest_candidates_and_fin(p2p_session_t *s) {
    ...
    int cand_unit = (P2P_MAX_PAYLOAD - 8 - 1 - 1) / (int)sizeof(p2p_compact_candidate_t);
    if (cand_unit > MAX_CANDS_PER_PACKET) cand_unit = MAX_CANDS_PER_PACKET;
    if (cand_unit <= 0) return;

    int cand_remain = s->local_cand_cnt - ctx->candidates_cached;
    if (cand_remain < 0) cand_remain = 0;

    int pkt_cnt = cand_remain == 0 ? 1 : (cand_remain + cand_unit - 1) / cand_unit;
    if (pkt_cnt > 16) pkt_cnt = 16;

    ctx->candidates_mask = (pkt_cnt >= 16) ? 0xFFFFu : (uint16_t)((1u << pkt_cnt) - 1u);
    ctx->candidates_acked = 0;

    for (uint16_t seq = 1; seq <= (uint16_t)pkt_cnt; seq++) {
        uint8_t flags = 0;
        int payload_len = 0;
        if (build_peer_info_packet(s, seq, &flags, payload, &payload_len) != 0) {
            continue;
        }
...
static void resend_rest_candidates_and_fin(p2p_session_t *s) {
    ...
    for (uint16_t seq = 1; seq <= 16; seq++) {
        ...
        if ((ctx->candidates_mask & bit) == 0) continue;
...
static void parse_peer_info(p2p_session_t *s, const uint8_t *payload, int cand_cnt) {
    ...
    for (int offset = 10, i = 0; i < cand_cnt; i++) {
...
//                             ctx->probe_port);
        ...
        if (ctx->candidates_cached > payload[1/*max_candidates*/])
        ...
        // 标记进入 REGISTERED 状态，停止发送 REGISTER
        ...
        // 如果已获得和对方建立的 session id
        // + 向对方发送后续候选队列、以及 FIN 通知
        if (ctx->session_id) {
        ...
        if (ctx->probe_port > 0) {
            ...
            // lan_punch 模式：本地直接打洞，无需探测，NAT 类型为 OPEN
            if (s->cfg.lan_punch) {
...
int p2p_signal_compact_tick(struct p2p_session *s) {
    ...
    // 进入 REGISTERED 之前，定时重发 REGISTER
    if (ctx->state == SIGNAL_COMPACT_REGISTERING) {
    ...
    // 完成注册且在（对方上线）向对法同步后续候选队列之前、或完成 FIN 确认后，定期向服务器发送保活包
