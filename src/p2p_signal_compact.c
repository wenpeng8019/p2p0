/*
 * COMPACT 模式信令实现（UDP 无状态）
 */
#pragma clang diagnostic push
#pragma ide diagnostic ignored "OCUnusedGlobalDeclarationInspection"

#define MOD_TAG "COMPACT"

#include "p2p_internal.h"
#include "p2p_probe.h"

#define REG_INTERVAL_MS              1000    /* REG/SYN0 重发间隔 */
#define SYNC_INTERVAL_MS                500     /* SYNC 重发间隔 */
#define MAX_SIG_ATTEMPTS                10      /* 最大 REG/SYN0 重发次数 */
#define REG_KEEPALIVE_INTERVAL_MS  20000   /* REG 状态保活重注册间隔（防服务器超时清除槽位） */
#define TRICKLE_BATCH_MS                1000    /* TURN trickle 攒批窗口（多个 TURN 响应在此窗口内合并为一个包） */
#define MAX_CANDS_PER_PACKET            10      /* 每个 SYNC 包最大候选数 */
#define NAT_PROBE_MAX_RETRIES           3       /* NAT_PROBE 最大发送次数 */
#define NAT_PROBE_INTERVAL_MS           1000    /* NAT_PROBE 重发间隔 */

#define REQ_INTERVAL_MS             500     /* REQ 重发间隔 */
#define REQ_MAX_RETRIES             5       /* REQ 最大重发次数，超出后报超时失败 */

///////////////////////////////////////////////////////////////////////////////

#define TASK_REG                     "REG"
#define TASK_SYNC                       "SYNC"
#define TASK_SYNC_REMOTE                "SYNC REMOTE"
#define TASK_RPC                        "RPC"
#define TASK_NAT_PROBE                  "NAT PROBE"

static inline err_t udp_send(struct p2p_instance *inst, const char* PROTO,
                             uint8_t type, uint16_t seq, uint8_t flags,
                             uint8_t* payload, int payload_len,
                             uint64_t now) {

    ret_t ret = p2p_udp_send_packet(inst, &inst->sig_ctx.compact.server_addr,
                                    type, flags, seq, payload, payload_len);
    if (ret < 0) {
        print("E:", LA_F("[C] %s send failed(%d)\n", LA_F457, 457), PROTO, E_EXT_CODE(ret));
        return ret;
    }

    printf(LA_F("[C] %s send, seq=0, flags=0x%02x, len=%d\n", LA_F460, 460),
           PROTO, seq, flags, payload_len);

    inst->sig_ctx.compact.last_send_time = now;

    return E_NONE;
}

/*
 * 解析 SYNC 负载，追加到 session 的 remote_cands[]
 * 注意：这里对方的候选列表顺序并未按对方原始顺序排序，而是 FIFO 追加到 remote_cands[] 中
 *
 * 格式: [session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)][candidates(N*23)]
 */
static void unpack_remote_candidates(struct p2p_session *s, const uint8_t *payload, int cand_cnt) {
    char _ab[INET6_ADDRSTRLEN];

    assert(cand_cnt && s->remote_cand_cnt + cand_cnt <= s->remote_cand_cap);

    int offset = P2P_SESS_ID_SZ + 2;  // 第一个 candidates 列表的起始位置
    p2p_remote_candidate_entry_t*c;

    // 对于 sync0, 即由 server 维护的双方握手包
    if (payload[P2P_SESS_ID_SZ]/* base_index */ == 0) {

        unpack_candidate(c = &s->remote_cands[0], payload + offset);
        if (s->remote_cand_cnt == 0) s->remote_cand_cnt = 1;

        // 其首个地址候选（idx=0）肯定有效，即肯定是（服务器观察到的）对方公网地址
        if ((p2p_cand_type_t)c->type != P2P_CAND_SRFLX) {

            print("W:", LA_F("%s: unexpected non-srflx cand in sync0, treating as srflx\n", LA_F266, 266),
                  TASK_SYNC_REMOTE);

            c->type = P2P_CAND_SRFLX;
        }

        print("I:", LA_F("%s: sync0 srflx cand[%d]<%s:%d>%s\n", LA_F255, 255),
                        TASK_SYNC_REMOTE, 0, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr),
                        s->inst->cfg.test_ice_srflx_off ? " (disabled)" : "");

        if (s->inst->cfg.test_ice_srflx_off) --s->remote_cand_cnt;
        else { s->remote_srflx_cnt++;

            // 这里启动打洞需要依赖于信令服务器的 REG_ACK 包中携带的 feature_relay 标志
            // + 该标志用于决定所使用的冷打洞机制，如果冷打洞不依赖服务器中转，则打洞可以不依赖 REG_ACK 包
            if (s->sig_sess.compact.state >= SIG_COMPACT_SESS_WAIT_SYN0_ACK && nat_punch(s, 0) != E_NONE) {
                print("W:", LA_F("%s: punch remote cand[%d]<%s:%d> failed\n", LA_F199, 199),
                    TASK_SYNC_REMOTE, 0, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));
            }
        }

        --cand_cnt; offset += (int)sizeof(p2p_candidate_t);
    }
    // 如果 sync0 以外的其他 info 包先到达，则需要保留 cand[0] 给对方（在 sync0 中的）的公网地址
    else if (s->remote_cand_cnt == 0 && !s->inst->cfg.test_ice_srflx_off) {
        memset(&s->remote_cands[0], 0, sizeof(p2p_remote_candidate_entry_t));
        s->remote_cand_cnt = 1;
    }

    for (int i = 0, idx; i < cand_cnt; i++, offset += (int)sizeof(p2p_candidate_t)) {

        unpack_candidate(c = &s->remote_cands[idx = s->remote_cand_cnt], payload + offset);
        int dup_idx = p2p_find_remote_candidate_by_addr(s, &c->addr);
        if (dup_idx >= 0) {
            if (s->remote_cands[dup_idx].type == P2P_CAND_PRFLX && c->type != P2P_CAND_PRFLX) {
                s->remote_cands[dup_idx].type = c->type;
                s->remote_cands[dup_idx].priority = c->priority;
                print("I:", LA_F("%s: promoted prflx cand[%d]<%s:%d> → %s\n", LA_F194, 194),
                      TASK_SYNC_REMOTE, dup_idx, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr),
                      p2p_candidate_type_str(c->type));
            } else {
                print("V:", LA_F("%s: duplicate remote cand<%s:%d> from signaling, skipped\n", LA_F133, 133),
                      TASK_SYNC_REMOTE, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));
            }
            continue;
        }

        const char* type_str; uint16_t* cand_cnt_ptr; bool opt_off = false;
        if (c->type == P2P_CAND_HOST) { type_str = "host"; cand_cnt_ptr = &s->remote_host_cnt; opt_off = s->inst->cfg.test_ice_host_off; }
        else if (c->type == P2P_CAND_SRFLX) { type_str = "srflx"; cand_cnt_ptr = &s->remote_srflx_cnt; opt_off = s->inst->cfg.test_ice_srflx_off; }
        else if (c->type == P2P_CAND_RELAY) { type_str = "relay"; cand_cnt_ptr = &s->remote_relay_cnt; opt_off = s->inst->cfg.test_ice_relay_off; }
        else { --s->remote_cand_cnt;
            print("E:", LA_F("%s: unexpected remote cand type %d, skipped\n", LA_F267, 267),
                  TASK_SYNC_REMOTE, c->type);
            continue;
        }

        if (opt_off
            || (c->addr.family == AF_INET  && s->inst->cfg.test_ice_ipv4_off)
            || (c->addr.family == AF_INET6 && s->inst->cfg.test_ice_ipv6_off)) {
            print("I:", LA_F("%s: remote %s cand[%d]<%s:%d> (disabled)\n", LA_F221, 221),
                  TASK_SYNC_REMOTE, type_str, idx, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));
            continue;
        }

        ++s->remote_cand_cnt; ++*cand_cnt_ptr;

        print("I:", LA_F("%s: remote %s cand[%d]<%s:%d> accepted\n", LA_F222, 222),
              TASK_SYNC_REMOTE, type_str, idx, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));

        if (s->sig_sess.compact.state < SIG_COMPACT_SESS_WAIT_SYN0_ACK) continue;

        if (nat_punch(s, idx) != E_NONE)
            print("E:", LA_F("%s: punch remote cand[%d]<%s:%d> failed\n", LA_F199, 199),
                  TASK_SYNC_REMOTE, idx, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));
    }
}

/* 一个 SYNC 包所承载的候选数量（单位）
 * + 这里 10（字节）表示 SYNC 负载头：[session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)] = 10 字节
 *   负载头后面的剩余空间就是候选列表，通过预定义、和 MTU 上限共同限制计算得出该单位大小
 */
#define SYNC_CAND_UNIT \
    (((P2P_MAX_PAYLOAD - P2P_SESS_ID_SZ - 2) / (int)sizeof(p2p_candidate_t)) < MAX_CANDS_PER_PACKET \
     ? ((P2P_MAX_PAYLOAD - P2P_SESS_ID_SZ - 2) / (int)sizeof(p2p_candidate_t)) \
     : MAX_CANDS_PER_PACKET)

/*
 * 构建 SYNC 的候选队列，返回 payload 总长度
 *
 * 格式: [session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)][candidates(N*23)]
 */
static int pack_local_candidates(struct p2p_session *s, uint16_t seq, uint8_t *payload, uint8_t *r_flags) {

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    int base_index, cnt;

    // 对于 trickle 候选队列的前面部分，也就是首批一次性发送的候选队列
    // + 此时 SYNC 包的候选队列数量都是以 SYNC_CAND_UNIT 为单位的
    if (seq < sess_ctx->trickle_seq_base) {

        const int cand_unit = SYNC_CAND_UNIT;
        base_index = sess_ctx->candidates_cached + (int)(seq - 1) * cand_unit;
        if (base_index > s->local_cand_cnt) base_index = s->local_cand_cnt;

        int remaining = (int)sess_ctx->trickle_idx_base - base_index;
        if (remaining > cand_unit) cnt = cand_unit;
        else cnt = remaining;
    }
    // 对于 trickle 候选队列部分
    else {
        base_index = sess_ctx->trickle_idx_base;
        for(int i=sess_ctx->trickle_seq_base; i<seq; i++)
            base_index += sess_ctx->trickle_queue[i];
        cnt = sess_ctx->trickle_queue[seq];
    }

    // 对于最后一个 SYNC 包，设置 FIN 标志
    if (seq == 16 || (s->inst->srflx_active >= s->inst->srflx_count && !s->inst->turn_pending && base_index + cnt >= s->local_cand_cnt)) {
        *r_flags |= SIG_SYNC_FLAG_FIN;
    }

    payload[P2P_SESS_ID_SZ] = (uint8_t)base_index;
    payload[P2P_SESS_ID_SZ + 1] = (uint8_t)cnt;

    int offset = P2P_SESS_ID_SZ + 2; // 负载头：[session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)]
    for (int i = 0, idx; i < cnt; i++) { idx = base_index + i;
        offset += pack_candidate(&s->local_cands[idx], payload + offset);
    }

    return offset;
}

/*
 * 向信令服务器发送 REG（上线登录）请求
 *
 * 包头: [type=SIG_PKT_REG | flags=0 | seq=0]
 * 负载: [local_peer_id(32)][instance_id(4)]
 *   - instance_id: 本次 connect() 的实例 ID（网络字节序，32位，必须非 0）
 * 注：候选地址通过后续 SYN0 包单独提交
 */
static void send_online(struct p2p_instance *inst, p2p_compact_ctx_t *sig_ctx, uint64_t now) {
    const char* PROTO = "REG";

    assert(sig_ctx->state == SIG_COMPACT_WAIT_REG_ACK);

    uint8_t payload[SIG_PKT_REG_PSZ];

    // local_peer_id（32 字节，超过部分截断，不足部分补零）
    int n = (int)strlen(sig_ctx->local_peer_id);
    memcpy(payload, sig_ctx->local_peer_id, ++n);
    if (n < P2P_PEER_ID_MAX) {
        memset(payload + n, 0, P2P_PEER_ID_MAX - n);
        n = P2P_PEER_ID_MAX;
    }

    // instance_id（4 字节大端序）
    nwrite_l(payload + n, sig_ctx->instance_id); n += 4;

    err_t err = udp_send(inst, PROTO, SIG_PKT_REG, 0, 0, payload, n, now);
    if (err != E_NONE) return;

    print("V:", LA_F("%s sent, inst_id=%u\n", LA_F59, 59), PROTO, sig_ctx->instance_id);
}

/*
 * 向信令服务器提交 SYN0（首批候选 + 指定对端）
 *
 * 包头: [type=SIG_PKT_SYN0 | flags=0 | seq=0]
 * 负载: [auth_key(SIG_AUTH_KEY_PSZ)][remote_peer_id(32)][candidate_count(1)][candidates(N*23)]
 *   - auth_key: 坥自 REG_ACK 的客户端-服务器认证令牌
 *   - remote_peer_id: 目标对端 ID
 *   - candidate_count: 首批候选数量（最多 candidates_cached 个）
 */
static void send_sync0(struct p2p_instance *inst, struct p2p_session *s, uint64_t now) {
    const char* PROTO = "SYN0";

    assert(s->inst == inst && !s->id);

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    assert(sig_ctx->auth_key != 0);

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    assert(sess_ctx->state == SIG_COMPACT_SESS_WAIT_SYN0_ACK);

    uint8_t payload[P2P_MAX_PAYLOAD]; int n = 0;

    // auth_key
    nwrite_ll(payload, sig_ctx->auth_key); n += SIG_AUTH_KEY_PSZ;

    // remote_peer_id（P2P_PEER_ID_MAX 字节，不足补零）
    int len = (int)strnlen(sess_ctx->remote_peer_id, P2P_PEER_ID_MAX);
    memcpy(payload + n, sess_ctx->remote_peer_id, len);
    if (n < P2P_PEER_ID_MAX) memset(payload + n + len, 0, P2P_PEER_ID_MAX - len);
    n += P2P_PEER_ID_MAX;

    // candidate_count + candidates
    int cand_cnt = (int)SYNC_CAND_UNIT;
    if (s->local_cand_cnt < cand_cnt) cand_cnt = s->local_cand_cnt;
    if (cand_cnt > sig_ctx->max_candidates)
        cand_cnt = sig_ctx->max_candidates;
    sess_ctx->candidates_cached = cand_cnt;
    payload[n++] = (uint8_t)cand_cnt;
    for (int i = 0; i < cand_cnt; i++) {
        n += pack_candidate(&s->local_cands[i], payload + n);
    }

    err_t err = udp_send(inst, PROTO, SIG_PKT_SYN0, 0, 0, payload, n, now);
    if (err != E_NONE) return;

    print("V:", LA_F("%s sent, auth_key=%" PRIu64 ", remote='%.32s', cands=%d\n", LA_F58, 58),
          PROTO, sig_ctx->auth_key, sess_ctx->remote_peer_id, cand_cnt);

    sess_ctx->sync_send_time = now;
}

/*
 * 在首次收到 SYNC 包，且已经收到 REG_ACK 的情况下，发送剩余候选队列和 FIN 包给对方
 * 注意：首次收到的 SYNC 包，可能是服务器下发的 seq=0 的 SYNC 包；
 *       也可能是对方发送的 seq!=1 的 SYNC 包（在并发网络状况下，对方的 SYNC 包可能先到达）
 *
 * 包头: [type=SIG_PKT_SYNC | flags=见下 | seq=1-16]
 * 负载: [session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)][candidates(N*7)]
 *   - session_id: 会话 ID（网络字节序）
 *   - base_index: 本批候选的起始索引
 *   - candidate_count: 本批候选数量
 *   - candidates: 每个候选 7 字节 [type(1)+ip(4)+port(2)]
 *   - flags: SIG_PKT_FLAG_MORE_CAND (0x02) 表示后续还有包
 *           SIG_PKT_FLAG_FIN (0x01) 表示发送完毕
 */
static void send_rest_candidates_and_fin(struct p2p_session *s, uint64_t now) {
    const char* PROTO = "SYNC";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    assert(s->id && sess_ctx->state == SIG_COMPACT_SESS_SYNCING);

    // 计算剩余候选数量
    // + 注意，此时的剩余数量，是此刻的剩余数量；因为候选队列可能是动态增加的（如 TURN Allocate 后追加的 Relay 候选）
    int cand_remain = s->local_cand_cnt - sess_ctx->candidates_cached;
    if (cand_remain < 0) cand_remain = 0;

    // 至少发送一个包（即使没有剩余候选），以确保对方收到 FIN 信号
    const int cand_unit = SYNC_CAND_UNIT;
    int pkt_cnt = cand_remain == 0 ? 1 : (cand_remain + cand_unit - 1) / cand_unit; // 单位 ceil 取整
    if (pkt_cnt >= 16) { pkt_cnt = 16;                                              // 目前协议设计最多支持 16 个包（seq=1-16）
        sess_ctx->candidates_mask = 0xFFFFu;
        sess_ctx->trickle_seq_base = 17;                                                 // 无 trickle 空间（seq 溢出区）
    } else {
        sess_ctx->trickle_seq_base = (uint8_t)(pkt_cnt + 1);                             // trickle_seq_base 从下一个 seq 开始
        sess_ctx->candidates_mask = (uint16_t)((1u << pkt_cnt) - 1u);                    // 计算候选确认窗口的 mask
    }
    sess_ctx->trickle_seq_next = sess_ctx->trickle_seq_base;                                   // 初始 trickle_seq_next = trickle_seq_base

    // 记录首批候选的分界点（trickle 候选从此索引开始）
    sess_ctx->trickle_idx_base = (uint16_t)s->local_cand_cnt;
    memset(sess_ctx->trickle_queue, 0, sizeof(sess_ctx->trickle_queue));

    // 初始重置确认状态
    sess_ctx->candidates_acked = 0;

    // session_id 所有包相同，只写一次
    uint8_t payload[P2P_MAX_PAYLOAD];
    nwrite_l(payload, s->id);

    int e=0;
    for (uint16_t seq = 1; seq <= (uint16_t)pkt_cnt; seq++) {

        uint8_t flags = 0; int payload_len = pack_local_candidates(s, seq, payload, &flags);

        err_t err = udp_send(s->inst, PROTO, SIG_PKT_SYNC, seq, flags, payload, payload_len, now);
        if (err != E_NONE) ++e;
    }

    if (e == pkt_cnt) return;

    print("V:", LA_F("%s sent (ses_id=%u), total=%d, err=%d\n", LA_F51, 51),
          PROTO, s->id, pkt_cnt, e);

    // 首批发完后，如果还有异步候选收集未完成，启动攒批计时器
    if (s->inst->srflx_active < s->inst->srflx_count || s->inst->turn_pending)
        sess_ctx->trickle_last_pack_time = P_tick_ms();

    sess_ctx->sync_send_time = now;
}

/*
 * 向对方发送新收集到的中继候选（Trickle candidates）
 *
 * 行为：
 *   1. 在现有候选窗口后追加一个 SYNC 包（seq = 已发包数 + 1）
 *   2. 扩展 candidates_mask，将新包纳入确认窗口
 *   3. 当所有 TURN 候选收集完毕（turn_pending==0）且候选已全部打包时，pack 自动附带 FIN
 *   4. 如果当前状态已到 READY（前序包均已确认），回退到 SYNCING 以等待新包确认
 *   5. 支持攒批：多个 TURN 响应在 TRICKLE_BATCH_MS 窗口内合并为一个包
 */

/* 将攒批累积的 trickle 候选打包发送（trickle_turn 和 tick flush 共用） */
static void send_trickle_candidates(struct p2p_session *s) {
    const char* PROTO = "SYNC";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    uint16_t seq = sess_ctx->trickle_seq_next;
    assert(seq <= 16 && sess_ctx->trickle_queue[seq] > 0);
    assert(sess_ctx->state == SIG_COMPACT_SESS_SYNCING);

    // 扩展确认窗口
    sess_ctx->candidates_mask |= (uint16_t)(1u << (seq - 1));
    sess_ctx->trickle_seq_next++;

    int new_cands = sess_ctx->trickle_queue[seq];

    uint8_t payload[P2P_MAX_PAYLOAD];
    nwrite_l(payload, s->id);

    uint8_t flags = 0;
    int payload_len = pack_local_candidates(s, seq, payload, &flags);

    uint64_t now = P_tick_ms();
    err_t err = udp_send(s->inst, PROTO, SIG_PKT_SYNC, seq, flags, payload, payload_len, now);
    if (err != E_NONE) return;

    print("I:", LA_F("%s trickle (ses_id=%u), cnt=%d, seq=%u \n", LA_F68, 68),
          PROTO, s->id, new_cands, seq);

    sess_ctx->trickle_last_pack_time = s->inst->srflx_active < s->inst->srflx_count || s->inst->turn_pending ? now : 0;
}

/*
 * 周期将未确认的 SYNC 包重发给对方
 *
 * 包头: [type=SIG_PKT_SYNC | flags=见下 | seq=1-16]
 * 负载: [session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)][candidates(N*7)]
 * 说明: 重发所有未收到 ACK 的 SYNC 包
 */
static void resend_candidates_and_fin(struct p2p_session *s, uint64_t now) {
    const char* PROTO = "SYNC";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    assert(sess_ctx->state == SIG_COMPACT_SESS_SYNCING && s->id);
    assert((sess_ctx->candidates_acked & sess_ctx->candidates_mask) != sess_ctx->candidates_mask);

    // session_id 所有包相同，只写一次
    uint8_t payload[P2P_MAX_PAYLOAD];
    nwrite_l(payload, s->id);

    uint16_t seq = 1; int pkt_cnt = 0; int e=0;
    for (; seq <= 16; seq++) {
        uint16_t bit = (uint16_t)(1u << (seq - 1));
        if ((sess_ctx->candidates_mask & bit) == 0) break;           // 遇到第一个 0 就可以停止循环（mask 是低位连续段，高位全为 0）
        if ((sess_ctx->candidates_acked & bit) != 0) continue;
        pkt_cnt++;

        uint8_t flags = 0; int payload_len = pack_local_candidates(s, seq, payload, &flags);
        err_t err = udp_send(s->inst, PROTO, SIG_PKT_SYNC, seq, flags, payload, payload_len, now);
        if (err != E_NONE) ++e;
    }

    if (e < pkt_cnt) {
        print("V:", LA_F("%s resent (ses_id=%u), (total=%d, err=%d)/%d\n", LA_F46, 46),
              PROTO, s->id, pkt_cnt, e, seq - 1);
        sess_ctx->sync_send_time = now;
    }
}

/*
 * 通过服务器向对端发送 RPC 消息请求
 *
 * 包头: [type=SIG_PKT_REQ | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][msg(1)][data(N)]
 *   - session_id: 本端会话 ID（来自 REG_ACK）
 *   - sid: 序列号（2字节，网络字节序），用于匹配响应
 *   - msg: 消息 ID（1字节，用户自定义）
 *   - data: 消息数据（可选）
 */
static void send_rpc_req(struct p2p_session *s, uint64_t now) {
    const char* PROTO = "REQ";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint8_t payload[P2P_SESS_ID_SZ + 3 + P2P_MSG_DATA_MAX]; int n = 0;
    nwrite_l(payload + n, s->id); n += P2P_SESS_ID_SZ;
    nwrite_s(payload + n, sess_ctx->req_sid); n += 2;
    payload[n++] = sess_ctx->req_msg;
    if (sess_ctx->req_data_len > 0) {
        memcpy(payload + n, sess_ctx->req_data, (size_t)sess_ctx->req_data_len);
        n += sess_ctx->req_data_len;
    }

    err_t err = udp_send(s->inst, PROTO, SIG_PKT_REQ, 0, 0, payload, n, now);
    if (err != E_NONE) return;

    print("V:", LA_F("%s sent (ses_id=%u), sid=%u msg=%u size=%d\n", LA_F49, 49),
          PROTO, s->id, sess_ctx->req_sid, sess_ctx->req_msg, sess_ctx->req_data_len);
}

/*
 * （对端）通过服务器向源端回复 RPC 消息响应
 *
 * 包头: [type=SIG_PKT_RSP | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][code(1)][data(N)]
 *   - session_id: A端的会话 ID（网络字节序）
 *   - sid: 序列号，必顾与 REQ 中的 sid 一致
 *   - code: 响应码
 *   - data: 响应数据
 */
static void send_rpc_resp(struct p2p_session *s, uint64_t now) {
    const char* PROTO = "RSP";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint8_t payload[P2P_SESS_ID_SZ + 2 + 1 + P2P_MSG_DATA_MAX]; int n = 0;
    nwrite_l(payload, sess_ctx->resp_session_id); n = P2P_SESS_ID_SZ;
    nwrite_s(payload + n, sess_ctx->resp_sid); n += 2;
    payload[n++] = sess_ctx->resp_code;
    if (sess_ctx->resp_data_len > 0) {
        memcpy(payload + n, sess_ctx->resp_data, (size_t)sess_ctx->resp_data_len);
        n += sess_ctx->resp_data_len;
    }

    err_t err = udp_send(s->inst, PROTO, SIG_PKT_RSP, 0, 0, payload, n, now);
    if (err != E_NONE) return;

    print("V:", LA_F("%s: sent (ses_id=%u), sid=%u code=%u size=%d\n", LA_F234, 234),
          PROTO, sess_ctx->resp_sid, sess_ctx->resp_code, sess_ctx->resp_data_len);
}

/*
 * 发送 NAT_PROBE 探测包
 * 即向服务器探测端口发送空包，服务器观察源地址来判断 NAT 类型
 *
 * 包头: [type=SIG_PKT_NAT_PROBE | flags=0 | seq=探测重试次数]
 * 负载: 空
 */
static void send_nat_probe(struct p2p_instance *inst, uint64_t now) {
    const char* PROTO = "NAT_PROBE";

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;

    struct sockaddr_in probe_addr = sig_ctx->server_addr;
    probe_addr.sin_port = htons(sig_ctx->probe_port);

    ret_t ret = p2p_udp_send_packet(inst, &probe_addr, SIG_PKT_NAT, 0, sig_ctx->nat_probe_retries, NULL, 0);
    if (ret < 0) {
        print("E:", LA_F("[C] %s send to port:%d failed(%d)\n", LA_F458, 458),
              PROTO, sig_ctx->probe_port, E_EXT_CODE(ret));
        return;
    }

    printf(LA_F("[C] %s send to port:%d, seq=%u, flags=0, len=0\n", LA_F459, 459),
           PROTO, sig_ctx->probe_port, sig_ctx->nat_probe_retries);

    print("V:", LA_F("%s sent, retry=%u\n", LA_F61, 61), PROTO, sig_ctx->nat_probe_retries);

    sig_ctx->nat_probe_send_time = now;
}

///////////////////////////////////////////////////////////////////////////////

/*
 * 初始化信令上下文
 */
void p2p_signal_compact_init(p2p_compact_ctx_t *ctx) {
    memset(ctx, 0, sizeof(*ctx));
    ctx->state = SIG_COMPACT_INIT;
}

static void reset_peer(p2p_compact_session_t *sess_ctx) {

    // 清理发给对端的候选及其确认状态
    sess_ctx->candidates_mask = 0;
    sess_ctx->candidates_acked = 0;
    sess_ctx->trickle_seq_base = 1;
    sess_ctx->trickle_seq_next = 1;
    sess_ctx->trickle_idx_base = 0;
    memset(sess_ctx->trickle_queue, 0, sizeof(sess_ctx->trickle_queue));
    sess_ctx->trickle_last_pack_time = 0;

    // 清理从对端收到的候选状态
    sess_ctx->remote_candidates_0 = false;
    sess_ctx->remote_candidates_mask = 0;
    sess_ctx->remote_candidates_done = 0;
    sess_ctx->remote_addr_notify_seq = 0;

    // 清理 MSG RPC 状态
    sess_ctx->rpc_last_sid = 0;
    sess_ctx->req_sid = 0;
    sess_ctx->req_state = 0;
    sess_ctx->resp_sid = 0;
    sess_ctx->resp_state = 0;
    sess_ctx->resp_session_id = 0;
}

///////////////////////////////////////////////////////////////////////////////

/*
 * 处理 REG_ACK，服务器上线确认
 *
 * 包头: [type=SIG_PKT_REG_ACK | flags=见下 | seq=0]
 * 负载: [auth_key(SIG_AUTH_KEY_PSZ) | instance_id(4) | max_candidates(1) | public_ip(4) | public_port(2) | probe_port(2)]
 *   - auth_key: 客户端-服务器认证令牌（0=服务器拒绝登录，无可用槽位）
 *   - max_candidates: 服务器缓存的最大候选数量（0=不支持缓存）
 *   - public_ip/port: 客户端的公网地址（服务器观察到的 UDP 源地址）
 *   - probe_port: NAT 探测端口（0=不支持探测）
 *   - flags: SIG_ONACK_FLAG_RELAY (0x01) 表示服务器支持中继
 */
void compact_on_online_ack(struct p2p_instance *inst, uint16_t seq, uint8_t flags,
                              const uint8_t *payload, int len) {
    (void)seq; (void)len;
    const char* PROTO = "REG_ACK";

    // instance_id 先于 auth_key，可在验证state前快速过滤过期 ACK
    uint32_t ack_instance_id = 0;
    nread_l(&ack_instance_id, payload + 0);

    uint64_t ack_auth_key = nget_ll(payload + 4);
    if (ack_auth_key == 0) {
        print("E:", LA_F("%s: server rejected (no slot)\n", LA_F236, 236), PROTO);
        return;
    }

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    if (sig_ctx->state != SIG_COMPACT_WAIT_REG_ACK) {
        print("V:", LA_F("%s: ignored in state=%d\n", LA_F150, 150), PROTO, (int)sig_ctx->state);
        return;
    }

    // 验证 echo instance_id（确保 ACK 对应当前上线请求）
    if (ack_instance_id != sig_ctx->instance_id) {
        print("V:", LA_F("%s: stale ACK(ack_inst=%u local_inst=%u), ignored\n", LA_F246, 246),
              PROTO, ack_instance_id, sig_ctx->instance_id);
        return;
    }

    // REG_ACK 下发 auth_key（客户端-服务器认证令牌）
    sig_ctx->auth_key = ack_auth_key;

    sig_ctx->feature_relay = (flags & SIG_REG_FLAG_RELAY) != 0;      // 服务器是否支持数据中继转发
    sig_ctx->feature_msg   = (flags & SIG_REG_FLAG_MSG) != 0;        // 服务器是否支持 MSG RPC
    sig_ctx->max_candidates = payload[12];  // payload: [instance_id(4)][auth_key(SIG_AUTH_KEY_PSZ)][max(1)][ip(4)][port(2)][probe(2)]

    // 解析自己的公网地址（服务器主端口探测到的 UDP 源地址）
    memset(&sig_ctx->public_addr, 0, sizeof(sig_ctx->public_addr));
    sig_ctx->public_addr.sin_family = AF_INET;
    memcpy(&sig_ctx->public_addr.sin_addr.s_addr, payload + 13, 4);
    memcpy(&sig_ctx->public_addr.sin_port, payload + 17, 2);

    // 解析服务器提供的 NAT 探测端口，0 表示服务器不支持
    nread_s(&sig_ctx->probe_port, payload + 19);

    print("V:", LA_F("%s: accepted, public=%s:%d auth_key=%" PRIu64 " max_cands=%d probe_port=%d relay=%s msg=%s\n", LA_F115, 115),
          PROTO, inet_ntoa(sig_ctx->public_addr.sin_addr), ntohs(sig_ctx->public_addr.sin_port),
          sig_ctx->auth_key, sig_ctx->max_candidates, sig_ctx->probe_port,
          sig_ctx->feature_relay ? "yes" : "no",
          sig_ctx->feature_msg ? "yes" : "no");

    sig_ctx->state = SIG_COMPACT_REG;
    inst->state = P2P_SIG_ST_READY;

    // 如果服务器支持数据中继
    if (sig_ctx->feature_relay) {

        // 启动数据中继功能
        assert(!inst->signaling_relay_fn);
        inst->signaling_relay_fn = p2p_signal_compact_relay;

        // 将 SIGNALING 路径添加到路径管理器（作为 fallback）
        path_manager_enable_signaling(inst, &sig_ctx->server_addr);
        print("I:", LA_F("SIGNALING path enabled (server supports relay)\n", LA_F398, 398));
    }

    // 如果服务器支持 NAT 探测端口，则启动 NAT_PROBE 探测流程
    if (sig_ctx->probe_port > 0) {

        // 标记进入 NAT_PROBE 探测中状态，发送第一轮探测包
        inst->nat_type = P2P_NAT_DETECTING;
        print("I:", LA_F("%s: started, sending first probe\n", LA_F247, 247), TASK_NAT_PROBE);
        sig_ctx->nat_probe_retries = 0/* 初始化启动探测 */;
        send_nat_probe(inst, P_tick_ms());
    }
    else inst->nat_type = P2P_NAT_UNDETECTABLE;

    for (struct p2p_session *s = inst->sessions_head; s; s = s->next) {
        p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

        // 上线完成之前，session 肯定处于 WAIT SYNCABLE 阶段
        assert(sess_ctx->remote_peer_id[0] && sess_ctx->state == SIG_COMPACT_SESS_WAIT_REG);

        sess_ctx->state = SIG_COMPACT_SESS_WAIT_SYN0_ACK;
        send_sync0(inst, s, P_tick_ms());
        sess_ctx->sync_attempts = 1;
        print("I:", LA_F("REG: auth_key acquired, auto SYN0 sent\n", LA_F368, 368));

        // 根据服务器能力设置探测状态
        if (sig_ctx->feature_msg) {
            s->probe.state = P2P_PROBE_STATE_READY;
        } else {
            s->probe.state = P2P_PROBE_STATE_NO_SUPPORT;
        }
    }
}

/*
 * 处理 ALIVE_ACK，服务器保活确认
 *
 * 包头: [type=SIG_PKT_ALIVE_ACK | flags=0 | seq=0]
 * 负载: 无
 */
void compact_on_alive_ack(struct p2p_instance *inst) {
    const char* PROTO = "ALIVE_ACK";

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;

    if (sig_ctx->state < SIG_COMPACT_REG) {
        print("V:", LA_F("%s: ignored in state=%d\n", LA_F150, 150), PROTO, (int)sig_ctx->state);
        return;
    }

    print("V:", LA_F("%s: accepted\n", LA_F116, 116), PROTO);

    // 确认服务器未掉线
    uint64_t now = P_tick_ms();
    sig_ctx->last_recv_time = now;

    // 通知路径管理器：ALIVE_ACK 确认（seq=0），完成 RoundTrip 测量
    // 仅当 SIGNALING 作为 relay 路径被启用时才统计 RTT
    if (inst->signaling.active) {
        path_manager_on_sig_alive_recv(inst, now);
    }
}

/*
 * 处理 SYN0_ACK（即连接应答）, session_id 和首批候选同步提交确认
 *
 * 包头: [type=SIG_PKT_SYN0_ACK | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][online(1)]
 *   - session_id: 对端配对会话 ID（由服务器在 SYN0 时分配，标识 client↔peer 会话）
 *   - online: 1=对端已上线（已有配对），0=对端尚未上线
 */
void compact_on_sync0_ack(struct p2p_session *s, const uint8_t *payload, int len) {
    (void)len;
    const char* PROTO = "SYN0_ACK";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    // 发送 SYN0_ACK（方向 2：二次确认，告知服务器已收到 session_id）
    {
        uint8_t ack_payload[SIG_PKT_SYN0_ACK_C2S_PSZ];
        nwrite_l(ack_payload, s->id);
        udp_send(s->inst, PROTO, SIG_PKT_SYN0_ACK, 0, 0, ack_payload, sizeof(ack_payload), P_tick_ms());
    }

    if (sess_ctx->state != SIG_COMPACT_SESS_WAIT_SYN0_ACK) return;

    uint8_t online = payload[P2P_SESS_ID_SZ];
    print("V:", LA_F("%s: accepted (ses_id=%u), peer=%s\n", LA_F107, 107),
          PROTO, s->id, online ? "online" : "offline");

    assert(s->state == P2P_STATE_SIGNALING);
    s->state = P2P_STATE_WAITING;

    sess_ctx->state = SIG_COMPACT_SESS_WAIT_PEER;
    if (!online) {
        print("I:", LA_F("%s: peer offline in SYN0_ACK, waiting for peer to come online\n", LA_F187, 187), TASK_SYNC);
        return;
    }

    // 进入 SYNCING 有两个入口，谁先发生谁先执行：
    // 1. 本处（SYN0_ACK online=1）：对端已在线，直接进入 SYNCING，立即发送本端剩余候选
    // 2. compact_on_server_sync0 收到服务器下发的 SYN0：服务器下发对端候选，同时触发 SYNCING
    // 两者均在 WAIT_PEER 状态下判断，状态机本身保证幂等（已 SYNCING 则跳过）

    sess_ctx->state = SIG_COMPACT_SESS_SYNCING;
    print("I:", LA_F("%s: entered, peer online in SYN0_ACK\n", LA_F140, 140), TASK_SYNC);
    send_rest_candidates_and_fin(s, P_tick_ms());

    // 启动 NAT 打洞（即使当前没有候选也要启动，以便打洞超时后 fallback 到信令中转）
    nat_punch(s, -1/* all candidates */);
}

/*
 * 处理 SYNC_ACK，对端确认已收到自己发过去的候选地址信息
 *
 * 包头: [type=SIG_PKT_SYNC_ACK | flags=0 | seq=确认的 SYNC 序列号]
 * 负载: [session_id(P2P_SESS_ID_SZ)]
 *   - session_id: 会话 ID（网络字节序）
 *   - seq: 确认的 SYNC 序列号（0 表示确认服务器下发的 SYNC(seq=0)）
 */
void compact_on_sync_ack(struct p2p_session *s, uint16_t seq, uint8_t flags,
                         const uint8_t *payload, int len, uint64_t now) {
    (void)flags; (void)payload; (void)len; (void)now;
    const char* PROTO = "SYNC_ACK";

    if (seq == 0 || seq > 16) {
        print("E:", LA_F("%s: invalid ack_seq=%u\n", LA_F152, 152), PROTO, seq);
        return;
    }

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint16_t bit = (uint16_t)(1u << (seq - 1));
    if ((sess_ctx->candidates_mask & bit) == 0) {
        print("E:", LA_F("%s: unexpected ack_seq=%u mask=0x%04x\n", LA_F265, 265),
              PROTO, seq, (unsigned)sess_ctx->candidates_mask);
        return;
    }

    if ((sess_ctx->candidates_acked & bit)) {
        print("V:", LA_F("%s: ignored for duplicated seq=%u, already acked\n", LA_F146, 146), PROTO, seq);
        return;
    }

    print("V:", LA_F("%s: accepted for ack_seq=%u\n", LA_F111, 111), PROTO, seq);

    sess_ctx->candidates_acked |= bit;

    // 如果所有动态收集的候选都已经完成
    if (s->inst->srflx_active >= s->inst->srflx_count && !s->inst->turn_pending && (seq > 16 || !sess_ctx->trickle_queue[seq])) {

        // 如果所有的候选队列都已确认被对方接收完成
        if ((sess_ctx->candidates_acked & sess_ctx->candidates_mask) == sess_ctx->candidates_mask) {
            sess_ctx->state = SIG_COMPACT_SESS_READY;
            print("I:", LA_F("%s: sync complete (ses_id=%u)\n", LA_F250, 250), TASK_SYNC, s->id);
        }
    }
}


/*
 * 处理服务器下发的 SYN0（首次对端候选推送）
 *
 * 包头: [type=SIG_PKT_SYN0 | flags=0 | seq=0]  （server→client 方向）
 * 负载: [session_id(P2P_SESS_ID_SZ)][0x00(1)][candidate_count(1)][candidates(N*23)]
 *   - session_id: 服务器分配的对端配对会话 ID（来自对端 SYN0 触发的配对）
 *   - 0x00: 保留字节（固定为 0，供 unpack_remote_candidates 识别为初始推送）
 *   - candidate_count: 对端候选数量（首个为服务器观察到的公网地址 srflx，必须 >= 1）
 *   客户端收到后以 SIG_PKT_SYNC_ACK(seq=0) 确认
 */
void compact_on_peer_sync0(struct p2p_session *s,
                             const uint8_t *payload, int len,
                             uint64_t now) {
    const char* PROTO = "SYN0";

    int cand_cnt = payload[P2P_SESS_ID_SZ + 1];
    if (len < (int)(SIG_PKT_SYN0_S2C_PSZ(cand_cnt) - P2P_PEER_ID_MAX)) {
        print("E:", LA_F("%s: bad payload(len=%d cand_cnt=%d)\n", LA_F123, 123), PROTO, len, cand_cnt);
        return;
    }

    if (!cand_cnt) {
        print("E:", LA_F("%s: invalid cand_cnt=0\n", LA_F154, 154), PROTO);
        return;
    }

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    // 进入 SYNCING 有两个入口，谁先发生谁先执行：
    // 1. compact_on_sync0_ack（SYN0_ACK online=1）：对端已在线，直接进入 SYNCING，立即发送本端剩余候选
    // 2. 本处（收到服务器下发的 SYN0/SYNC）：服务器下发对端候选，同时触发 SYNCING
    // 两者均在 WAIT_PEER 状态下判断，状态机本身保证幂等（已 SYNCING 则跳过）
    if (sess_ctx->state == SIG_COMPACT_SESS_WAIT_PEER) {
        sess_ctx->state = SIG_COMPACT_SESS_SYNCING;
        print("I:", LA_F("%s: entered, %s arrived\n", LA_F139, 139), TASK_SYNC, PROTO);
        send_rest_candidates_and_fin(s, P_tick_ms());
    }

    bool new_seq = false;

    // 排重
    if (!sess_ctx->remote_candidates_0) {

        if (p2p_remote_cands_reserve(s, cand_cnt) != E_NONE) {
            print("E:", LA_F("Failed to reserve remote candidates (cnt=%d)\n", LA_F311, 311), cand_cnt);
            return;
        }

        unpack_remote_candidates(s, payload, cand_cnt);

        sess_ctx->remote_candidates_0 = new_seq = true;

        print("V:", LA_F("%s: accepted cand_cnt=%d\n", LA_F110, 110), PROTO, cand_cnt);
    }

    // 无论 peer_online 之前是否已设置，只要 NAT 打洞还未启动就启动
    // 注：Bob 在 SYN0_ACK(online=1) 时已设置 peer_online，但 punch 尚未启动，需在此触发
    if (s->nat.state < NAT_PUNCHING) {
        print("I:", LA_F("%s: peer online, starting NAT punch\n", LA_F189, 189), PROTO);
        nat_punch(s, -1/* all candidates */);
    }

    if (new_seq) {

        // 如果对方所有的候选队列都已经接收完成
        if (sess_ctx->remote_candidates_0 && sess_ctx->remote_candidates_mask &&
            (sess_ctx->remote_candidates_done & sess_ctx->remote_candidates_mask) == sess_ctx->remote_candidates_mask) {

            s->remote_cand_done = true;

            print("I:", LA_F("%s: sync complete (ses_id=%u, mask=0x%04x)\n", LA_F251, 251),
                  TASK_SYNC_REMOTE, s->id, (unsigned)sess_ctx->remote_candidates_mask);
        }
    }

    // 发送 SYN0_ACK（client→server 方向）确认收到服务器 SYN0
    {
        uint8_t ack_payload[SIG_PKT_SYN0_ACK_C2S_PSZ];
        nwrite_l(ack_payload, s->id);

        err_t err = udp_send(s->inst, PROTO, SIG_PKT_SYN0_ACK, 0, 0, ack_payload, sizeof(ack_payload), now);
        if (err != E_NONE) return;

        print("V:", LA_F("%s sent (ses_id=%u)\n", LA_F52, 52), "SYN0_ACK", s->id);
    }
}


/*
 * 包头: [type=SIG_PKT_SYNC | flags=见下 | seq=序列号]
 * 负载: [session_id(P2P_SESS_ID_SZ) | base_index(1) | candidate_count(1) | candidates(N*7)]
 *   - session_id: 会话 ID（网络字节序）
 *   - base_index: 本批候选的起始索引（0-based）
 *   - candidate_count: 本批候选数量，0 表示结束标识
 *   - seq=0: 服务器发送，base_index!=0，对端公网地址变更通知（首次推送已改用 SIG_PKT_SYN0）
 *   - seq>0: 客户端发送，继续同步剩余候选
 *   - flags: SIG_SYNC_FLAG_FIN (0x01) 表示候选列表发送完毕
 */
void compact_on_peer_sync(struct p2p_session *s, uint16_t seq, uint8_t flags,
                          const uint8_t *payload, int len,
                          uint64_t now) {
    char _ab[INET6_ADDRSTRLEN];
    const char* PROTO = "SYNC";

    if (seq > 16) {
        print("E:", LA_F("%s: invalid seq=%u\n", LA_F157, 157), PROTO, seq);
        return;
    }

    int cand_cnt = payload[P2P_SESS_ID_SZ + 1];
    if (len < (int)SIG_PKT_SYNC_PSZ(cand_cnt)) {
        print("E:", LA_F("%s: bad payload(len=%d cand_cnt=%d)\n", LA_F123, 123), PROTO, len, cand_cnt);
        return;
    }

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    // seq>0 时 base_index 为候选起始索引；seq=0 时 base_index 为循环通知序号（1..255）
    uint8_t base_index = payload[P2P_SESS_ID_SZ];

    // SYNCING 通常由 compact_on_sync0_ack 或 compact_on_peer_sync0 触发
    // 但在 UDP 网络中 sync0/sync 包是并发的，先后无法确定。
    if (sess_ctx->state == SIG_COMPACT_SESS_WAIT_PEER) {
        sess_ctx->state = SIG_COMPACT_SESS_SYNCING;
        print("I:", LA_F("%s: entered early, %s arrived before SYN0\n", LA_F138, 138), TASK_SYNC, PROTO);
        send_rest_candidates_and_fin(s, P_tick_ms());
    }

    bool new_seq = false;

    // seq!=0 说明是对端发来的后续 SYNC 包
    if (seq != 0) {

        print("V:", LA_F("%s: accepted seq=%u cand_cnt=%d flags=0x%02x\n", LA_F112, 112), PROTO, seq, cand_cnt, flags);

        // 排重
        if ((new_seq = (sess_ctx->remote_candidates_done & (1u << (seq - 1))) == 0)) {

            // 对于 FIN 包，计算对方候选地址集合序列掩码（即计算全集区间）
            if ((flags & SIG_SYNC_FLAG_FIN) || !cand_cnt) {

                sess_ctx->remote_candidates_mask = (1u << seq) - 1u;
            }

            // 维护分配远端候选列表的空间
            // + 这里 payload[P2P_SESS_ID_SZ](base_index) + cand_cnt，表示该包至少需要的远端候选数量; 1 为至少包含一个对方的公网地址
            if (p2p_remote_cands_reserve(s, 1 + payload[P2P_SESS_ID_SZ] + cand_cnt) != E_NONE) {
                print("E:", LA_F("Failed to reserve remote candidates (base=%u cnt=%d)\n", LA_F310, 310), payload[P2P_SESS_ID_SZ], cand_cnt);
                return;
            }

            if (cand_cnt)
                unpack_remote_candidates(s, payload, cand_cnt);

            sess_ctx->remote_candidates_done |= 1u << (seq - 1);
        }
    }
    // seq=0: 对端公网地址变更通知，必须只携带一个候选地址（变更后的公网地址），且不带 FIN 标识
    // base_index（循环通知序号 1..255，服务器侧保证跳过 0）用于接收端排重，确保只处理最新通知
    else if (cand_cnt != 1 || (flags & SIG_SYNC_FLAG_FIN)) {

        print("E:", LA_F("%s NOTIFY: invalid(base=%u cand_cnt=%d flags=0x%02x)\n", LA_F36, 36),
              PROTO, base_index, cand_cnt, flags);
        return;
    }
    // 确保地址变更通知是最新的
    else if (sess_ctx->remote_addr_notify_seq == 0 || uint8_circle_newer(base_index, sess_ctx->remote_addr_notify_seq)) {

        print("V:", LA_F("%s NOTIFY: accepted\n", LA_F34, 34), PROTO);

        if (p2p_remote_cands_reserve(s, 1) != E_NONE) {
            print("E:", LA_F("Failed to reserve remote candidates (cnt=1)\n", LA_F312, 312));
            return;
        }

        if (s->inst->cfg.test_ice_srflx_off) {
            print("I:", LA_F("%s NOTIFY: srflx addr update (disabled)\n", LA_F37, 37), PROTO);
            return;
        }

        p2p_remote_candidate_entry_t *c = &s->remote_cands[0];
        c->type = (p2p_cand_type_t)payload[P2P_SESS_ID_SZ + 2];
        c->priority = 0;
        c->addr.family = AF_INET;
        sockaddr_init_with_net(&c->addr.addr.v4, (uint32_t *) (payload + P2P_SESS_ID_SZ + 3),
                               (uint16_t *) (payload + P2P_SESS_ID_SZ + 7));
        c->last_punch_send_ms = 0;
        if (s->remote_cand_cnt == 0) s->remote_cand_cnt = 1;

        // Trickle candidates：NAT 打洞已启动时，立即探测最新地址
        if (s->nat.state == NAT_PUNCHING || s->nat.state == NAT_RELAY) {

            print("I:", LA_F("%s: Peer addr changed -> %s:%d, retrying punch\n", LA_F88, 88),
                  TASK_SYNC_REMOTE, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr));

            // 标记旧的活跃路径为失效（地址已变更）
            // fixme: 这个逻辑好像有问题
            // if (s->active_path >= 0 && s->active_path < s->remote_cand_cnt) {
            //     path_manager_set_path_state(s, s->active_path, PATH_STATE_FAILED);
            //     print("V:", LA_F("Marked old path (idx=%d) as FAILED due to addr change\n", LA_F268, 268),
            //            s->active_path);
            // }

            if (sess_ctx->state >= SIG_COMPACT_SESS_WAIT_SYN0_ACK) {

                if (nat_punch(s, 0) != E_NONE) {
                    print("E:", LA_F("Failed to send punch packet for new peer addr\n", LA_F318, 318));
                }
            }
        }
        else {
            print("I:", LA_F("%s: Peer addr changed -> %s:%d, punch deferred (NAT=%d)\n", LA_F87, 87),
                  TASK_SYNC_REMOTE, sockAddr_str(&c->addr, _ab, sizeof(_ab)), sockAddr_port(&c->addr), (int)s->nat.state);
        }

        sess_ctx->remote_addr_notify_seq = base_index;
    }
    else print("V:", LA_F("%s NOTIFY: ignored old notify base=%u (current=%u)\n", LA_F35, 35),
               PROTO, base_index, sess_ctx->remote_addr_notify_seq);

    // 无论 peer_online 之前是否已设置，只要 NAT 打洞还未启动就启动
    if (s->nat.state < NAT_PUNCHING) {
        print("I:", LA_F("%s: peer online, starting NAT punch\n", LA_F189, 189), PROTO);
        nat_punch(s, -1/* all candidates */);
    }

    if (new_seq) {

        // 如果对方所有的候选队列都已经接收完成
        // 注：此状态用于 NAT 打洞超时判断，只有候选同步完成后才会触发打洞超时
        if (sess_ctx->remote_candidates_0 && sess_ctx->remote_candidates_mask &&
            (sess_ctx->remote_candidates_done & sess_ctx->remote_candidates_mask) == sess_ctx->remote_candidates_mask) {

            // 标记远程候选交换完成（供 NAT 层判断打洞超时使用）
            s->remote_cand_done = true;

            print("I:", LA_F("%s: sync complete (ses_id=%u, mask=0x%04x)\n", LA_F251, 251),
                  TASK_SYNC_REMOTE, s->id, (unsigned)sess_ctx->remote_candidates_mask);
        }
    }

    /*
     * 发送 SYNC_ACK 确认包
     * 说明: 确认收到对方的候选地址包
     *
     * 包头: [type=SIG_PKT_SYNC_ACK | flags=0 | seq=被确认的SYNC包序号]
     * 负载: [session_id(P2P_SESS_ID_SZ)]
     *   - session_id: 会话 ID（网络字节序）
     *   - seq: 被确认的 SYNC 包的序列号
     */
    {
        uint8_t ack_payload[P2P_SESS_ID_SZ];
        nwrite_l(ack_payload, s->id);

        err_t err = udp_send(s->inst, PROTO, SIG_PKT_SYNC_ACK, seq, 0, ack_payload, sizeof(ack_payload), now);
        if (err != E_NONE) return;

        print("V:", LA_F("%s sent (ses_id=%u), seq=%u\n", LA_F48, 48), "SYNC_ACK", s->id, seq);
    }
}

/*
 * 处理 FIN，对端离线通知
 *
 * 包头: [type=SIG_PKT_FIN | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)]
 *   - session_id: 已断开的会话 ID（网络字节序）
 */
void compact_on_fin(struct p2p_session *s, uint16_t seq, uint8_t flags,
                    const uint8_t *payload, int len, uint64_t now) {
    (void)seq; (void)flags; (void)len; (void)now;
    const char* PROTO = "FIN";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint32_t session_id = nget_l(payload);

    print("V:", LA_F("%s: accepted (ses_id=%u)\n", LA_F108, 108), PROTO, session_id);

    // 重置到 WAIT_PEER 被动等待状态（对端主动断开，不自动重连）
    // ! 这里可以明确知道是对方断开了连接，所以自己和信令服务器之间的连接和数据状态都是正常的，不需要重置
    //   auth_key 不需要重置，因为它是 client↔server 的认证令牌，对端断开不影响与服务器的关系
    //   session_id 对应 client↔peer 会话，对端断开后将在下一轮 SYN0_ACK 中重新获得
    sess_ctx->state = SIG_COMPACT_SESS_WAIT_PEER;
    s->id = 0;

    // 清除双方协商信息
    reset_peer(sess_ctx);

    print("I:", LA_F("%s: peer disconnected (ses_id=%u), reset to WAIT_PEER\n", LA_F185, 185), PROTO, session_id);

    // 标记 NAT 为已关闭
    // + 这里将信令层的 peer close 转换为 NAT 层的 closed 状态，主循环会统一以 NAT 层的 NAT_CLOSED 状态机变更为准
    //   并统一调用 p2p_session_reset
    s->nat.state = NAT_CLOSED;
}

/*
 * 处理 REQ，（服务器代理转发的）源端消息请求
 * 说明: B端收到服务器转发的消息请求，A端发出的原始请求(flags=0)不会到达客户端
 *
 * 包头: [type=SIG_PKT_REQ | flags=SIG_FLAG_RELAY | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][msg(1)][data(N)]
 *   - session_id: A端的会话 ID（网络字节序，64位）
 *   - sid: 序列号（2字节，网络字节序）
 *   - msg: 消息 ID（1字节）
 *   - data: 消息数据
 *   - flags: SIG_FLAG_RELAY (0x02) 表示服务器转发
 */
void compact_on_request(struct p2p_session *s, uint16_t seq, uint8_t flags,
                        const uint8_t *payload, int len, uint64_t now) {
    (void)seq; (void)now;
    const char* PROTO = "REQ";

    // 客户端收到 req 肯定都是 Server 转发过来，而不是对方直接发来的原始请求
    if (!(flags & SIG_FLAG_RELAY)) {
        print("E:", LA_F("%s: invalid for non-relay req\n", LA_F155, 155), PROTO);
        return;
    }

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint32_t session_id = nget_l(payload);
    uint16_t sid = nget_s(payload + P2P_SESS_ID_SZ);
    uint8_t msg = payload[P2P_SESS_ID_SZ + 2];
    const uint8_t *req_data = payload + P2P_SESS_ID_SZ + 3;
    int req_len = len - (int)SIG_PKT_REQ_MIN_PSZ;

    /* 判断是否是新请求（使用循环序列号比较）：
     * 1. 如果正在处理请求且 sid 相同 → 忽略重复包
     * 2. 如果 sid <= last_sid（旧请求）→ 忽略
     * 3. 如果 sid > last_sid（新请求）→ 处理（可能覆盖正在处理的旧请求）*/
    if (sess_ctx->resp_state == 1 && sess_ctx->resp_sid == sid) {
        print("V:", LA_F("%s: duplicate request ignored (sid=%u, already processing)\n", LA_F136, 136), PROTO, sid);
        return;
    }

    // 忽略旧请求（sid <= last_sid）
    if (sess_ctx->rpc_last_sid != 0 && !uint16_circle_newer(sid, sess_ctx->rpc_last_sid)) {
        print("V:", LA_F("%s: old request ignored (sid=%u <= last_sid=%u)\n", LA_F178, 178),
              PROTO, sid, sess_ctx->rpc_last_sid);
        return;
    }

    // 如果正在处理旧请求但收到新请求（sid > last_sid），则覆盖旧请求
    if (sess_ctx->resp_state == 1) {
        print("W:", LA_F("%s: new request (sid=%u) overrides pending request (sid=%u)\n", LA_F164, 164),
              PROTO, sid, sess_ctx->resp_sid);
    }

    sess_ctx->resp_sid         = sid;
    sess_ctx->resp_session_id  = session_id;

    // msg=0: 默认自动 echo 回复（无需应用层介入）
    if (msg == 0) {
        print("V:", LA_F("%s msg=0 accepted (ses_id=%u), echo reply sid=%u len=%d\n", LA_F42, 42), PROTO, s->id, sid, req_len);
        p2p_signal_compact_response(s, 0, req_data, req_len);
        return;
    }

    print("V:", LA_F("%s accepted (ses_id=%u), sid=%u msg=%u\n", LA_F39, 39), PROTO, s->id, sid, msg);

    // 触发用户回调
    if (s->inst->cfg.on_request)
        s->inst->cfg.on_request((p2p_session_t)s, sid, msg, req_data, req_len, s->inst->cfg.userdata);
}

/*
 * 处理 REQ_ACK，服务器对自己发起的请求的确认
 * 说明: 该请求已经被服务器代理接管，并确保完成向对端的转发
 * 所以收到该消息后，自己就可以停止重发请求了，接下来只需要等待 RSP 即可知道请求的最终结果
 *
 * 包头: [type=SIG_PKT_REQ_ACK | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][status(1)]
 *   - session_id: A端的会话 ID（用于验证响应合法性）
 *   - sid: 序列号，与 REQ 中的 sid 对应
 *   - status: 0=已缓存开始中转, 1=B不在线（失败）
 */
void compact_on_request_ack(struct p2p_session *s, uint16_t seq, uint8_t flags,
                             const uint8_t *payload, int len, uint64_t now) {
    (void)seq; (void)flags; (void)len; (void)now;
    const char* PROTO = "REQ_ACK";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint16_t sid = nget_s(payload + P2P_SESS_ID_SZ);
    uint8_t status = payload[P2P_SESS_ID_SZ + 2];

    if (sess_ctx->req_sid != sid) {
        print("V:", LA_F("%s: ignored for sid=%u (current sid=%u)\n", LA_F148, 148), PROTO, sid, sess_ctx->req_sid);
        return;
    }
    if (sess_ctx->req_state != 1/* waiting REQ_ACK */) {
        print("V:", LA_F("%s: ignored in invalid state=%d\n", LA_F149, 149), PROTO, (int)sess_ctx->req_state);
        return;
    }

    // msg=0 data=0 是 probe 保留的空 RPC，短路处理，不走应用层
    if (sess_ctx->req_msg == 0 && sess_ctx->req_data_len == 0) {
        probe_compact_on_req_ack(s, sid, status);
        if (status == 0)
            sess_ctx->req_state = 2/* waiting RSP */;
        else {
            sess_ctx->req_state = 0;
            sess_ctx->req_sid   = 0;
        }
        return;
    }

    // 成功：服务器已收到请求并开始向对端中转，停止重发，等待 RSP
    if (status == 0) {
        sess_ctx->req_state = 2/* waiting RSP */;
        print("V:", LA_F("%s accepted (ses_id=%u), waiting for response (sid=%u)\n", LA_F41, 41), PROTO, s->id, sess_ctx->req_sid);
    }
    // 对端不在线：请求失败，通知上层
    else {

        uint16_t saved_id  = sess_ctx->req_sid;
        uint8_t  saved_msg = sess_ctx->req_msg;
        sess_ctx->req_state  = 0;
        sess_ctx->req_sid = 0;

        print("W:", LA_F("%s: RPC fail due to peer offline (sid=%u)\n", LA_F91, 91), PROTO, saved_id);

        if (s->inst->cfg.on_response)
            s->inst->cfg.on_response((p2p_session_t)s, saved_id, saved_msg, NULL, -1, s->inst->cfg.userdata);
    }
}

/*
 * 处理 RSP，服务器代理转发的，对端（对自己向对端请求的）消息响应
 *
 * 包头: [type=SIG_PKT_RSP | flags=见下 | seq=0]
 * 负载:
 * > [session_id(P2P_SESS_ID_SZ)][sid(2)][code(1)][data(N)] （正常响应）
 * > [session_id(P2P_SESS_ID_SZ)][sid(2)]（错误响应）
 *   - session_id: A端的会话 ID（用于验证响应合法性）
 *   - sid: 序列号，与 REQ 中的 sid 对应
 *   - code: 响应消息 ID（正常响应时）
 *   - data: 响应数据（正常响应时）
 *   - flags: 0=正常响应（B端返回的数据）
 *            SIG_MSG_FLAG_PEER_OFF (0x02)=B端在 REQ_ACK 之后离线
 *            SIG_MSG_FLAG_TIMEOUT (0x04)=服务器向B端转发请求超时
 */
void compact_on_response(struct p2p_session *s, uint16_t seq, uint8_t flags,
                        const uint8_t *payload, int len, uint64_t now) {
    (void)seq;
    const char* PROTO = "RSP";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint16_t sid = nget_s(payload + P2P_SESS_ID_SZ);

    /*
     * 发送 RSP_ACK 确认包
     * 服务器收到该确认后，将会停止重发该响应到本地，即结束整个请求-响应流程
     *
     * 包头: [type=SIG_PKT_RSP_ACK | flags=0 | seq=0]
     * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)]
     *   - session_id: A端的会话 ID（用于 O(1) 哈希查找）
     *   - sid: 序列号，与 RSP 中的 sid 一致
     * 说明: A端确认收到B端的响应，幂等操作，即使已处理过也补发
     */
    {
        uint8_t ack[SIG_PKT_RSP_ACK_PSZ]; int n = 0;
        nwrite_l(ack + n, s->id); n += P2P_SESS_ID_SZ;
        nwrite_s(ack + n, sid); n += 2;

        err_t err = udp_send(s->inst, "RSP_ACK", SIG_PKT_RSP_ACK, 0, 0, ack, n, now);
        if (err == E_NONE) {
            print("V:", LA_F("%s sent (ses_id=%u), sid=%u\n", LA_F50, 50), "RSP_ACK", s->id, sid);
        }
    }

    // 仅命中当前挂起请求时，才需要继续解析响应内容
    if (!(sess_ctx->req_state == 2/* waiting RSP */ && sess_ctx->req_sid == sid)) {
        print("V:", LA_F("%s: duplicate/irrelevant response acked (sid=%u, current sid=%u, state=%d)\n", LA_F137, 137),
              PROTO, sid, sess_ctx->req_sid, (int)sess_ctx->req_state);
        return;
    }

    // msg=0 data=0 是 probe 保留的空 RPC，短路处理，不走应用层
    if (sess_ctx->req_msg == 0 && sess_ctx->req_data_len == 0) {
        probe_compact_on_response(s, sid);
        sess_ctx->req_state = 0;
        sess_ctx->req_sid   = 0;
        return;
    }

    uint8_t res_code = 0;
    const uint8_t *res_data = NULL;
    int res_size = -1; // -1 表示错误

    if (flags & (SIG_RPC_FLAG_PEER_OFF | SIG_RPC_FLAG_TIMEOUT)) {

        res_code = (flags & SIG_RPC_FLAG_PEER_OFF) ? P2P_RPC_ERR_PEER_OFF : P2P_RPC_ERR_TIMEOUT;
    }
    else {

        // 正常响应：需要包含 code 和可选的 data
        // 最小长度：session_id(P2P_SESS_ID_SZ) + sid(2) + code(1) = 11
        if (len < (int)SIG_PKT_RSP_MIN_PSZ) {
            print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, len);
            return;
        }
        res_code = payload[P2P_SESS_ID_SZ + 2];
        res_data = payload + P2P_SESS_ID_SZ + 3;
        res_size = len - (int)SIG_PKT_RSP_MIN_PSZ;
    }

    print("V:", LA_F("%s accepted (ses_id=%u), sid=%u code=%u len=%u\n", LA_F38, 38),
          PROTO, s->id, sid, res_code, res_size);

    sess_ctx->req_state  = 0;
    sess_ctx->req_sid = 0;

    /* 根据 flags 输出不同的日志 */
    if (flags & SIG_RPC_FLAG_PEER_OFF) {
        print("W:", LA_F("%s: RPC fail due to peer offline (sid=%u)\n", LA_F91, 91), PROTO, sid);
    }
    else if (flags & SIG_RPC_FLAG_TIMEOUT) {
        print("W:", LA_F("%s: RPC fail due to relay timeout (sid=%u)\n", LA_F92, 92), PROTO, sid);
    }
    else {
        print("I:", LA_F("%s: RPC complete (sid=%u)\n", LA_F90, 90), PROTO, sid);
    }

    if (s->inst->cfg.on_response)
        s->inst->cfg.on_response((p2p_session_t)s, sid, res_code, res_data, res_size, s->inst->cfg.userdata);
}

/*
 * 处理 RSP_ACK，服务器对自己（经由服务器代理）向源方发出的请求的响应的确认
 * 收到该确认后，自己就不会再重发 response 了，且可以记录该 sid 已完成
 *
 * 包头: [type=SIG_PKT_RSP_ACK | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)]
 *   - session_id: B端的会话 ID（用于 O(1) 哈希查找）
 *   - sid: 序列号，与 RSP 中的 sid 对应
 */
void compact_on_response_ack(struct p2p_session *s, uint16_t seq, uint8_t flags,
                             const uint8_t *payload, int len, uint64_t now) {
    (void)seq; (void)flags; (void)len; (void)now;
    const char* PROTO = "RSP_ACK";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    uint16_t sid = nget_s(payload + P2P_SESS_ID_SZ);
    if (sess_ctx->resp_sid != sid) {
        print("V:", LA_F("%s: ignored for sid=%u (current sid=%u)\n", LA_F148, 148), PROTO, sid, sess_ctx->resp_sid);
        return;
    }
    if (sess_ctx->resp_state != 1/* waiting RSP_ACK */) {
        print("V:", LA_F("%s: ignored in invalid state=%d\n", LA_F149, 149), PROTO, (int)sess_ctx->resp_state);
        return;
    }

    // 记录最后完成的 sid
    print("V:", LA_F("%s accepted (ses_id=%u), sid=%u\n", LA_F40, 40), PROTO, s->id, sid);

    sess_ctx->rpc_last_sid = sess_ctx->resp_sid;

    // 成功：Server 已收到 B 的 RSP，结束 RSP 重发
    sess_ctx->resp_sid = 0;
    sess_ctx->resp_state = 0;
    sess_ctx->resp_session_id = 0;

    print("I:", LA_F("%s: RPC finished (sid=%u)\n", LA_F93, 93), PROTO, sid);
}

/*
 * 处理 NAT_PROBE_ACK，NAT 探测响应
 *
 * 包头: [type=SIG_PKT_NAT_PROBE_ACK | flags=0 | seq=对应的 NAT_PROBE 请求 seq]
 * 负载: [probe_ip(4) | probe_port(2)]
 *   - probe_ip/port: 服务器在探测端口观察到的客户端源地址（第二次映射）
 *   - seq: 复制请求包的 seq，用于客户端匹配响应
 */
void compact_on_nat_probe_ack(struct p2p_instance *inst, uint16_t seq,
                              const uint8_t *payload, int len) {
    (void)len;
    const char* PROTO = "NAT_PROBE_ACK";

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;

    if (seq != sig_ctx->nat_probe_retries) {
        print("V:", LA_F("%s: ignored for seq=%u (expect=%d)\n", LA_F147, 147),
              PROTO, seq, (int)sig_ctx->nat_probe_retries);
        return;
    }

    struct sockaddr_in probe_mapped;
    memset(&probe_mapped, 0, sizeof(probe_mapped));
    probe_mapped.sin_family = AF_INET;
    memcpy(&probe_mapped.sin_addr.s_addr, payload, 4);
    memcpy(&probe_mapped.sin_port,        payload + 4, 2);

    print("V:", LA_F("%s: accepted, probe_mapped=%s:%d\n", LA_F114, 114),
          PROTO, inet_ntoa(probe_mapped.sin_addr), ntohs(probe_mapped.sin_port));

    // 端口一致性：主端口映射端口 == 探测端口映射端口 → 锥形，否则 → 对称
    sig_ctx->nat_is_port_consistent = (probe_mapped.sin_port == sig_ctx->public_addr.sin_port) ? 1 : 0;

    // 检测 OPEN：公网地址 IP 与任意本地地址相同（无 NAT）
    int is_open = 0;
    const route_ctx_t *rt = route_shared_get();
    for (int i = 0; rt && i < rt->addr_count; i++) {
        if (sig_ctx->public_addr.sin_addr.s_addr == rt->local_addrs[i].sin_addr.s_addr) {
            is_open = 1;
            break;
        }
    }

    if (is_open) inst->nat_type = P2P_NAT_OPEN;
    else if (sig_ctx->nat_is_port_consistent)
        inst->nat_type = P2P_NAT_FULL_CONE; // 满足端口一致性 → Cone NAT（无法区分 Full/Restricted/Port-Restricted，取最乐观估计）
    else inst->nat_type = P2P_NAT_SYMMETRIC;
    sig_ctx->nat_probe_retries = -1/* 探测完成 */;

    print("I:", LA_F("%s: completed, mapped=%s:%d probe=%s:%d -> %s\n", LA_F132, 132),
          TASK_NAT_PROBE,
          inet_ntoa(sig_ctx->public_addr.sin_addr), ntohs(sig_ctx->public_addr.sin_port),
          inet_ntoa(probe_mapped.sin_addr),     ntohs(probe_mapped.sin_port),
          p2p_nat_type_str(inst->nat_type));
}


void p2p_signal_compact_proto(struct p2p_instance *inst, uint8_t type, uint8_t flags, uint16_t seq,
                              uint8_t *payload, int payload_len, uint64_t now) {

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;

    do { const char *PROTO;

        if (type == SIG_PKT_REG_ACK) { PROTO = "REG_ACK";

            printf(LA_F("[C] %s recv, seq=%u, flags=0x%02x, len=%d\n", LA_F454, 454),
                PROTO, seq, flags, payload_len);

            if (payload_len < (int)SIG_PKT_REG_ACK_PSZ) {
                print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, payload_len);
                break;
            }
            compact_on_online_ack(inst, seq, flags, payload, payload_len);
            break;
        }

        if (type == SIG_PKT_ALV_ACK) { PROTO = "ALIVE_ACK";

            printf(LA_F("[C] %s recv\n", LA_F456, 456), PROTO);

            compact_on_alive_ack(inst);
            break;
        }

        if (type == SIG_PKT_NAT_ACK) { PROTO = "NAT_PROBE_ACK";

            printf(LA_F("[C] %s recv, seq=%u, len=%d\n", LA_F455, 455),
                PROTO, seq, payload_len);

            if (payload_len < (int)SIG_PKT_NAT_ACK_PSZ) {
                print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, payload_len);
                break;
            }
            compact_on_nat_probe_ack(inst, seq, payload, payload_len);
            break;
        }

        if (type == SIG_PKT_SYN0_ACK) { PROTO = "SYN0_ACK";

            printf(LA_F("[C] %s recv, len=%d\n", LA_F453, 453), PROTO, payload_len);

            if (payload_len < (int)SIG_PKT_SYN0_ACK_PSZ) {
                print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, payload_len);
                break;
            }

            uint32_t session_id = nget_l(payload + P2P_PEER_ID_MAX);
            if (!session_id) {
                print("W:", LA_F("%s: invalid session_id=0\n", LA_F159, 159), PROTO);
                break;
            }

            struct p2p_session *s = inst->sessions_head;
            for (; s; s = s->next) {
                if (strncmp(s->sig_sess.compact.remote_peer_id, (const char *)payload, P2P_PEER_ID_MAX - 1) == 0) break;
            }
            if (!s) {
                print("W:", LA_F("%s: no session for peer_id=%.*s\n", LA_F170, 170),
                    PROTO, (int)(P2P_PEER_ID_MAX - 1), (const char *)payload);
                break;
            }

            if (s->sig_sess.compact.state < SIG_COMPACT_SESS_WAIT_SYN0_ACK) {
                print("V:", LA_F("%s: ignored in state=%d\n", LA_F150, 150), PROTO, (int)s->sig_sess.compact.state);
                break;
            }

            // 会话初始化：首次收到 session_id 时绑定
            if (!s->id) { s->id = session_id; if (s->inst->cfg.use_ice) p2p_ice_set_ufrag_from_id(s); }
            else if (s->id != session_id) {
                print("W:", LA_F("%s: session_id changed (old=%u new=%u)\n", LA_F243, 243),
                      PROTO, s->id, session_id);
                break;
            }

            compact_on_sync0_ack(s, payload + P2P_PEER_ID_MAX, payload_len - P2P_PEER_ID_MAX);
            break;
        }

        if (type == SIG_PKT_SYN0) { PROTO = "SYN0";

            printf(LA_F("[C] %s recv, len=%d\n", LA_F453, 453), PROTO, payload_len);

            if (payload_len < (int)SIG_PKT_SYN0_S2C_PSZ(0)) {
                print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, payload_len);
                break;
            }

            uint32_t session_id = nget_l(payload + P2P_PEER_ID_MAX);
            if (!session_id) {
                print("W:", LA_F("%s: invalid session_id=0\n", LA_F159, 159), PROTO);
                break;
            }

            struct p2p_session *s = inst->sessions_head;
            for (; s; s = s->next) {
                if (strncmp(s->sig_sess.compact.remote_peer_id, (const char *)payload, P2P_PEER_ID_MAX - 1) == 0) break;
            }
            if (!s) {
                print("W:", LA_F("%s: no session for peer_id=%.*s\n", LA_F170, 170),
                    PROTO, (int)(P2P_PEER_ID_MAX - 1), (const char *)payload);
                break;
            }

            if (s->sig_sess.compact.state < SIG_COMPACT_SESS_WAIT_SYN0_ACK) {
                print("V:", LA_F("%s: ignored in state=%d\n", LA_F150, 150), PROTO, (int)s->sig_sess.compact.state);
                break;
            }

            // 二次确认机制下，s->id 必已在 SYN0_ACK 中设置
            if (!s->id) {
                print("E:", LA_F("%s: unexpected s->id=0\n", LA_F268, 268), PROTO);
                break;
            }

            // session_id 变更：对端重新发起连接（崩溃重启），强制重置会话
            if (s->id != session_id) {
                print("W:", LA_F("%s: renew session (local=%u pkt=%u)\n", LA_F224, 224),
                      PROTO, s->id, session_id);

                if (s->state >= P2P_STATE_LOST) {
                    if (s->inst->cfg.on_state) s->inst->cfg.on_state((p2p_session_t)s, s->state, P2P_STATE_CLOSED, s->inst->cfg.userdata);
                }

                reset_peer(&s->sig_sess.compact);
                p2p_session_reset(s, false);

                s->id = session_id;
                if (s->inst->cfg.use_ice) p2p_ice_set_ufrag_from_id(s);
                s->sig_sess.compact.state = SIG_COMPACT_SESS_WAIT_PEER;
                s->state = P2P_STATE_WAITING;
            }

            compact_on_peer_sync0(s, payload + P2P_PEER_ID_MAX, payload_len - P2P_PEER_ID_MAX, now);
            break;
        }

        // ── session_id 查找的会话级协议 ──

        uint16_t payload_min;
        void (*handler)(struct p2p_session*, uint16_t, uint8_t, const uint8_t*, int, uint64_t);

        switch (type) {
        case SIG_PKT_SYNC:      PROTO = "SYNC";      payload_min = SIG_PKT_SYNC_PSZ(0);              handler = compact_on_peer_sync; break;
        case SIG_PKT_SYNC_ACK:  PROTO = "SYNC_ACK";  payload_min = SIG_PKT_SYNC_ACK_PSZ;             handler = compact_on_sync_ack; break;
        case SIG_PKT_FIN:       PROTO = "FIN";       payload_min = SIG_PKT_FIN_PSZ;                  handler = compact_on_fin; break;
        case SIG_PKT_REQ:       PROTO = "REQ";       payload_min = SIG_PKT_REQ_MIN_PSZ; handler = compact_on_request; break;
        case SIG_PKT_RSP:       PROTO = "RSP";      payload_min = (uint16_t)(P2P_SESS_ID_SZ + 2u); handler = compact_on_response; break;
        case SIG_PKT_RSP_ACK:   PROTO = "RSP_ACK";  payload_min = SIG_PKT_RSP_ACK_PSZ;         handler = compact_on_response_ack; break;
        default:
            print("W:", LA_F("[C] Unknown pkt type 0x%02x, len=%d\n", LA_F461, 461), type, payload_len);
            return;
        }

        printf(LA_F("[C] %s recv, seq=%u, flags=0x%02x, len=%d\n", LA_F454, 454),
            PROTO, seq, flags, payload_len);

        if (payload_len < (int)payload_min) {
            print("E:", LA_F("%s: bad payload(len=%d)\n", LA_F124, 124), PROTO, payload_len);
            break;
        }

        uint32_t session_id = nget_l(payload);
        if (!session_id) {
            print("W:", LA_F("%s: invalid session_id=0\n", LA_F159, 159), PROTO);
            break;
        }

        struct p2p_session *s = inst->sessions_head;
        for (; s; s = s->next) {
            if (s->id == session_id) break;
        }
        if (!s) {
            print("W:", LA_F("%s: no session for session_id=%u\n", LA_F172, 172), PROTO, session_id);
            break;
        }

        if (s->sig_sess.compact.state < SIG_COMPACT_SESS_WAIT_SYN0_ACK) {
            print("V:", LA_F("%s: ignored in state=%d\n", LA_F150, 150), PROTO, (int)s->sig_sess.compact.state);
            break;
        }

        handler(s, seq, flags, payload, payload_len, now);

    } while(0);

    sig_ctx->last_recv_time = P_tick_ms();
}

///////////////////////////////////////////////////////////////////////////////

/*
 * 客户端上线（发送 REG，建立 client↔server 关系，获取 auth_key）
 *
 * 包头: [type=SIG_PKT_REG | flags=0 | seq=0]
 * 负载: [local_peer_id(32)][instance_id(4)]
 *   - local_peer_id: 本端 ID（32 字节，不足补零）
 *   - instance_id:   本次上线实例 ID（非零，用于服务器区分重启会话）
 */
ret_t p2p_signal_compact_online(struct p2p_instance *inst, const char *local_peer_id,
                                const struct sockaddr_in *server) {

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    P_check(sig_ctx->state == SIG_COMPACT_INIT, return E_NONE_CONTEXT;)

    sig_ctx->server_addr = *server;

    // 每次 online() 生成新的实例 ID（加密安全随机数），用于服务器区分重新连接会话
    uint32_t rid = sig_ctx->instance_id;
    while (rid == sig_ctx->instance_id) rid = P_rand32();
    sig_ctx->instance_id = rid;

    strncpy(sig_ctx->local_peer_id, local_peer_id, P2P_PEER_ID_MAX - 1);
    sig_ctx->local_peer_id[P2P_PEER_ID_MAX - 1] = '\0';

    sig_ctx->state = SIG_COMPACT_WAIT_REG_ACK;
    send_online(inst, sig_ctx, P_tick_ms());
    sig_ctx->sig_attempts = 1;

    return E_NONE;
}

/*
 * 客户端下线（发送 OFF，清理服务器上的全部状态，回到 INIT）
 *
 * 包头: [type=SIG_PKT_OFF | flags=0 | seq=0]
 * 负载: [auth_key(SIG_AUTH_KEY_PSZ)]
 */
ret_t p2p_signal_compact_offline(struct p2p_instance *inst) {
    const char* PROTO = "OFF";

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    if (sig_ctx->state == SIG_COMPACT_INIT) return E_NONE;

    // fixme: 这需要异步等待，如果直接用 remote peer_id 则可以避免异步等待
    if (!sig_ctx->auth_key) return E_NONE_CONTEXT;          // 尚未完成上线（auth_key 未分配）

    uint8_t payload[SIG_PKT_OFF_PSZ];
    nwrite_ll(payload, sig_ctx->auth_key);

    err_t err = udp_send(inst, PROTO, SIG_PKT_OFF, 0, 0, payload, (int) sizeof(payload), 0);
    if (err != E_NONE) return err;

    for (int n = 2; n--;) { // 最多重试 2 次，确保服务器收到注销请求
        P_usleep(50 * 1000);
        p2p_udp_send_packet(inst, &sig_ctx->server_addr, SIG_PKT_OFF, 0, 0, payload, (int) sizeof(payload));
    }

    print("V:", LA_F("%s sent, inst_id=%u\n", LA_F59, 59), PROTO, sig_ctx->instance_id);

    p2p_signal_compact_init(sig_ctx);
    return E_NONE;
}

/*
 * 建立与对端的会话（发送 SYN0，建立 client↔peer 关系，获取 session_id）
 *
 * 包头: [type=SIG_PKT_SYN0 | flags=0 | seq=0]
 * 负载: [auth_key(SIG_AUTH_KEY_PSZ)][remote_peer_id(32)][candidate_count(1)][candidates(N*23)]
 *   - auth_key:        REG_ACK 中分配的客户端令牌
 *   - remote_peer_id:  目标对端 ID（32 字节，不足补零）
 *   - candidates:      首批本地候选（最多 candidates_cached 个）
 * 注：若状态为 WAIT_REG_ACK，仅存储 remote_peer_id，SYN0 在收到 REG_ACK 后自动触发
 */
ret_t p2p_signal_compact_connect(struct p2p_session *s, const char *remote_peer_id) {

    P_check(remote_peer_id && remote_peer_id[0], return E_INVALID;)

    p2p_compact_ctx_t *sig_ctx = &s->inst->sig_ctx.compact;
    if (sig_ctx->state == SIG_COMPACT_INIT) {
        return E_NONE_CONTEXT;  // 还没调用 online()
    }

    p2p_compact_session_t *ssss_ctx = &s->sig_sess.compact;

    // 幂等：相同 remote 则成功，不同则忙
    if (ssss_ctx->remote_peer_id[0]) {
        return strncmp(ssss_ctx->remote_peer_id, remote_peer_id, P2P_PEER_ID_MAX) == 0 ? E_NONE : E_BUSY;
    }

    strncpy(ssss_ctx->remote_peer_id, remote_peer_id, P2P_PEER_ID_MAX - 1);
    ssss_ctx->remote_peer_id[P2P_PEER_ID_MAX - 1] = '\0';

    // REG_ACK 已收到，且无需等待 stun 返回的异步候选，则立即发 SYN0
    if (sig_ctx->state == SIG_COMPACT_REG) {

        ssss_ctx->state = SIG_COMPACT_SESS_WAIT_SYN0_ACK;
        send_sync0(s->inst, s, P_tick_ms());
        ssss_ctx->sync_attempts = 1;
    }
    else ssss_ctx->state = SIG_COMPACT_SESS_WAIT_REG;

    return E_NONE;
}

/*
 * 断开与对端的会话（发送 OFF，清理 peer 会话状态，回到 REG）
 *
 * 包头: [type=SIG_PKT_OFF | flags=0 | seq=0]
 * 负载: [auth_key(SIG_AUTH_KEY_PSZ)]
 *   - auth_key: REG_ACK 中分配的客户端令牌，服务器据此查找并释放配对槽位
 * 注：若尚在 WAIT_REG_ACK 状态（auth_key 未分配），仅清除 remote_peer_id，不发包
 */
ret_t p2p_signal_compact_disconnect(struct p2p_session *s) {
    const char* PROTO = "OFF";

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    if (!sess_ctx->remote_peer_id[0]) return E_NONE;        // 没有建立过配对

    // 尚未完成配对（WAIT_SYN0_ACK 之前）：仅清除 remote，无需通知服务器
    if (sess_ctx->state == SIG_COMPACT_SESS_WAIT_REG)
        sess_ctx->state = SIG_COMPACT_SESS_SUSPENDED;
    if (sess_ctx->state == SIG_COMPACT_SESS_SUSPENDED) {
        *sess_ctx->remote_peer_id = 0;
        return E_NONE;
    }

    p2p_compact_ctx_t *sig_ctx = &s->inst->sig_ctx.compact;

    uint8_t payload[SIG_PKT_OFF_PSZ];
    nwrite_ll(payload, sig_ctx->auth_key);

    err_t err = udp_send(s->inst, PROTO, SIG_PKT_OFF, 0, 0, payload, (int) sizeof(payload), P_tick_ms());
    if (err != E_NONE) return err;

    print("V:", LA_F("%s sent (ses_id=%u)\n", LA_F52, 52), PROTO, s->id);

    // 清理 peer 会话状态
    reset_peer(sess_ctx);
    memset(sess_ctx->remote_peer_id, 0, sizeof(sess_ctx->remote_peer_id));

    s->id = 0;
    return E_NONE;
}

/*
 * 本地候选异步补发（Trickle candidates，发送 SYNC 包追加 STUN/TURN 候选）
 *
 * 包头: [type=SIG_PKT_SYNC | flags=SIG_SYNC_FLAG_FIN | seq=1-16]
 * 负载: [session_id(P2P_SESS_ID_SZ)][base_index(1)][candidate_count(1)][candidates(N*23)]
 *   - session_id:    会话 ID（SYN0_ACK 中分配）
 *   - base_index:   本批候选在本端候选列表中的起始索引
 *   - candidates:   本批候选（最多 SYNC_CAND_UNIT 个）
 *   - FIN flag:     末包置位，通知对端本端候选列表已全部发送完毕
 * 注：支持 TRICKLE_BATCH_MS 窗口攒批，多个 STUN/TURN 响应合并为一个包后发送
 */
void p2p_signal_compact_trickle_candidate(struct p2p_session *s) {

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    if (!s->id || sess_ctx->state < SIG_COMPACT_SESS_SYNCING) return;

    uint16_t seq = sess_ctx->trickle_seq_next;
    if (seq > 16) {
        print("W:", LA_F("SYNC(trickle): seq overflow, cannot trickle more\n", LA_F406, 406));
        return;
    }

    // O(1) 累加：每次本地异步候选（STUN/TURN）带来 1 个新候选
    sess_ctx->trickle_queue[seq]++;

    // 攒批间隔控制（固定窗口策略）
    uint64_t now = P_tick_ms();
    if (sess_ctx->trickle_last_pack_time && tick_diff(now, sess_ctx->trickle_last_pack_time) < TRICKLE_BATCH_MS) {
        print("V:", LA_F("SYNC(trickle): batching, queued %d cand(s) for seq=%u\n", LA_F405, 405),
              sess_ctx->trickle_queue[seq], seq);
        return;
    }

    send_trickle_candidates(s);
}

/*
 * 通过 COMPACT 信令中转发送数据包（通用接口）
 *
 * 包头：[type=P2P_PKT_DATA/ACK/CRYPTO/REACH/CONN/CONN_ACK | flags|=P2P_FLAG_SESSION|SIG_FLAG_RELAY | seq]
 * 负载：[session_id(P2P_SESS_ID_SZ)][原始 payload]
 */
ret_t p2p_signal_compact_relay(struct p2p_session *s,
                               uint8_t type, uint8_t flags, uint16_t seq,
                               const void *payload, uint16_t payload_len) {

    p2p_compact_ctx_t *sig_ctx = &s->inst->sig_ctx.compact;
    P_check(!payload_len || payload, return E_INVALID;)
    P_check(sig_ctx->feature_relay, return E_NO_SUPPORT;)
    P_check(s->id, return E_NONE_CONTEXT;)

    // 构造完整负载：[session_id(P2P_SESS_ID_SZ)][原始 payload]
    uint8_t relay_payload[P2P_MAX_PAYLOAD];
    if (payload_len + (int)P2P_SESS_ID_SZ > P2P_MAX_PAYLOAD) {
        print("E:", LA_F("[C] relay payload too large: %d", LA_F462, 462), payload_len);
        return E_OUT_OF_CAPACITY;
    }

    nwrite_l(relay_payload, s->id);
    if (payload_len > 0 && payload)
        memcpy(relay_payload + P2P_SESS_ID_SZ, payload, payload_len);

    err_t err = udp_send(s->inst, "RELAY", type, seq,
                        flags | P2P_FLAG_SESSION | SIG_FLAG_RELAY,
                         relay_payload, (int)P2P_SESS_ID_SZ + payload_len, P_tick_ms());
    if (err != E_NONE) return err;

    print("V:", LA_F("RELAY sent (ses_id=%u), type=0x%02x seq=%u flags=0x%02x", LA_F374, 374),
          s->id, type, seq, flags);

    return E_NONE;
}

/*
 * 通过信令服务器向对端发起 RPC 消息请求（A 端）
 *
 * 包头: [type=SIG_PKT_REQ | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][msg(1)][data(N)]
 *   - session_id: 本端会话 ID（SYN0_ACK 中分配，用于服务器路由到对端）
 *   - sid:        RPC 序列号（非零，循环自增，用于匹配响应）
 *   - msg:        应用层消息类型（1 字节，应用自定义）
 *   - data:       消息数据（可选，最多 P2P_MSG_DATA_MAX 字节）
 */
ret_t p2p_signal_compact_request(struct p2p_session *s,
                                 uint8_t msg, const void *data, int len) {

    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

    P_check(len == 0 || data, return E_INVALID;)
    P_check(len >= 0 && len <= P2P_MSG_DATA_MAX, return E_INVALID;)
    P_check(sess_ctx->state >= SIG_COMPACT_SESS_WAIT_SYN0_ACK, return E_NONE_CONTEXT;)    // 未上线
    p2p_compact_ctx_t *sig_ctx = &s->inst->sig_ctx.compact;
    if (!sig_ctx->feature_msg) {
        print("E:", LA_F("MSG RPC not supported by server\n", LA_F344, 344));
        return E_NO_SUPPORT;
    }

    if (sess_ctx->req_state != 0) return E_BUSY;                                     // 已有挂起请求

    // 生成非零（循环）序列号
    static uint16_t _sid_base = 0;
    uint16_t sid = ++_sid_base;
    if (sid == 0) sid = ++_sid_base;

    sess_ctx->req_state     = 1/* waiting REQ_ACK */;
    sess_ctx->req_sid       = sid;
    sess_ctx->req_msg       = msg;
    sess_ctx->req_data_len  = len;
    if (len > 0) memcpy(sess_ctx->req_data, data, (size_t)len);

    send_rpc_req(s, P_tick_ms());
    sess_ctx->req_retries = 0;
    return E_NONE;
}

/*
 * 通过信令服务器向请求端回复 RPC 消息响应（B 端）
 *
 * 包头: [type=SIG_PKT_RSP | flags=0 | seq=0]
 * 负载: [session_id(P2P_SESS_ID_SZ)][sid(2)][code(1)][data(N)]
 *   - session_id: A 端会话 ID（来自 REQ 的 relay 包，用于服务器路由回 A 端）
 *   - sid:        RPC 序列号，必须与对应 REQ 的 sid 一致
 *   - code:       响应码（1 字节，应用自定义）
 *   - data:       响应数据（可选，最多 P2P_MSG_DATA_MAX 字节）
 */
ret_t p2p_signal_compact_response(struct p2p_session *s,
                                  uint8_t code, const void *data, int len) {
    const char* PROTO = "RSP";

    P_check(len >= 0 && len <= P2P_MSG_DATA_MAX, return E_INVALID;)
    P_check(len == 0 || data, return E_INVALID;)
    p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;
    if (!sess_ctx->resp_sid) {
        print("E:", LA_F("%s: no rpc request\n", LA_F167, 167), PROTO);
        return E_INVALID;
    }

    // 缓存响应数据用于重发
    sess_ctx->resp_state    = 1/* waiting RSP_ACK */;
    sess_ctx->resp_code     = code;
    sess_ctx->resp_data_len = len;
    if (len > 0) memcpy(sess_ctx->resp_data, data, (size_t)len);

    send_rpc_resp(s, P_tick_ms());
    sess_ctx->resp_retries = 0;
    return E_NONE;
}

///////////////////////////////////////////////////////////////////////////////

/*
 * 信令服务周期维护（拉取阶段）- on action
 * > 接收/拼接完整的外部请求，并触发对应的响应
 * > 自身请求超时的处理，重试或超时
 */

void p2p_signal_compact_tick_recv(struct p2p_instance *inst, uint64_t now) {

    // 如果还未登录，或已经出错
    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    if (sig_ctx->state < SIG_COMPACT_WAIT_REG_ACK) return;

    // REGISTERING 状态：定期重发 REG
    if (sig_ctx->state == SIG_COMPACT_WAIT_REG_ACK) {

        if (tick_diff(now, sig_ctx->last_send_time) >= REG_INTERVAL_MS) {

            // 超时检查
            if (sig_ctx->sig_attempts++ < MAX_SIG_ATTEMPTS) {

                print("I:", LA_F("%s: retry, (attempt %d/%d)\n", LA_F231, 231),
                      TASK_REG, sig_ctx->sig_attempts, MAX_SIG_ATTEMPTS);

                send_online(inst, sig_ctx, now);
            }
            else {

                print("W:", LA_F("%s: timeout, max(%d) attempts reached, reset to INIT\n", LA_F259, 259),
                      TASK_REG, MAX_SIG_ATTEMPTS);

                sig_ctx->state = SIG_COMPACT_INIT;
            }
        }

        return;
    }
    assert(sig_ctx->state == SIG_COMPACT_REG);

    for (struct p2p_session *s = inst->sessions_head; s; s = s->next) {
        p2p_compact_session_t *sess_ctx = &s->sig_sess.compact;

        // 前面已经确保完成上线、且无需等待 stun 完成对公网候选的收集，所以不应再有 WAIT SYNCABLE 的会话存在了
        assert(sess_ctx->state != SIG_COMPACT_SESS_WAIT_REG);
        if (sess_ctx->state == SIG_COMPACT_SESS_SUSPENDED) continue;

        // WAIT_SYN0_ACK 状态：SYN0 重传
        if (sess_ctx->state == SIG_COMPACT_SESS_WAIT_SYN0_ACK) {

            // SYN0 重传（等待服务器缓存首批候选）
            if (tick_diff(now, sess_ctx->sync_send_time) >= REG_INTERVAL_MS) {

                if (sess_ctx->sync_attempts++ < MAX_SIG_ATTEMPTS) {

                    print("I:", LA_F("SYN0: retry, (attempt %d/%d)\n", LA_F403, 403),
                          sess_ctx->sync_attempts, MAX_SIG_ATTEMPTS);

                    send_sync0(inst, s, now);
                }
                else {

                    print("W:", LA_F("SYN0: timeout, max(%d) attempts reached, reset to INIT\n", LA_F404, 404),
                          MAX_SIG_ATTEMPTS);

                    sess_ctx->state = SIG_COMPACT_SESS_SUSPENDED;
                }
            }
            continue;
        }

        // SYNCING 不设超时（统一使用 signaling 的超时，和 p2p 的连接超时）
        if (sess_ctx->state == SIG_COMPACT_SESS_SYNCING) {

            // 如果有待发送的 trickle 候选，且超过了攒批间隔时间窗口
            if (sess_ctx->trickle_last_pack_time && tick_diff(now, sess_ctx->trickle_last_pack_time) >= TRICKLE_BATCH_MS
                && sess_ctx->trickle_seq_next <= 16 && sess_ctx->trickle_queue[sess_ctx->trickle_seq_next] > 0) {

                send_trickle_candidates(s);
            }

            if (tick_diff(now, sess_ctx->sync_send_time) >= SYNC_INTERVAL_MS) {

                // todo 确认 resend_candidates_and_fin 也对 trickle 进行重传？

                print("V:", LA_F("%s, retry remaining candidates and FIN to peer\n", LA_F69, 69), TASK_SYNC);
                resend_candidates_and_fin(s, now);
            }
        }

        // 当前（请求端）处于等待（服务器返回的）REQ_ACK 的阶段
        if (sess_ctx->req_state == 1/* waiting REQ_ACK */) {

            if (tick_diff(now, sess_ctx->req_send_time) >= REQ_INTERVAL_MS) {

                /* 超时失败 */
                if (sess_ctx->req_retries++ < REQ_MAX_RETRIES) {

                    print("I:", LA_F("%s: retry(%d/%d) req (sid=%u)\n", LA_F229, 229),
                          TASK_RPC, sess_ctx->req_retries, REQ_MAX_RETRIES, sess_ctx->req_sid);

                    send_rpc_req(s, now);
                }
                else {

                    uint16_t sid = sess_ctx->req_sid;
                    uint8_t  msg = sess_ctx->req_msg;
                    sess_ctx->req_sid = 0;
                    sess_ctx->req_state = 0;

                    print("W:", LA_F("%s: %s timeout after %d retries (sid=%u)\n", LA_F70, 70),
                          TASK_RPC, "req", REQ_MAX_RETRIES, sid);

                    if (s->inst->cfg.on_response)
                        s->inst->cfg.on_response((p2p_session_t)s, sid, msg, NULL, -1, s->inst->cfg.userdata);
                }
            }
        }

        // 当前（响应端）处于等待（服务器返回的）RSP_ACK 的阶段
        if (sess_ctx->resp_state == 1/* waiting RSP_ACK */) {

            if (tick_diff(now, sess_ctx->resp_send_time) >= REQ_INTERVAL_MS) {

                /* 超时失败（与 A 端对称，使用相同的超时配置） */
                if (sess_ctx->resp_retries++ < REQ_MAX_RETRIES) {

                    print("I:", LA_F("%s: retry(%d/%d) resp (sid=%u)\n", LA_F230, 230),
                          TASK_RPC, sess_ctx->resp_retries, REQ_MAX_RETRIES, sess_ctx->resp_sid);

                    send_rpc_resp(s, now);
                }
                else {

                    uint16_t sid = sess_ctx->resp_sid;
                    sess_ctx->resp_sid = 0;
                    sess_ctx->resp_state = 0;
                    sess_ctx->resp_session_id = 0;

                    print("W:", LA_F("%s: %s timeout after %d retries (sid=%u)\n", LA_F70, 70),
                          TASK_RPC, "resp", REQ_MAX_RETRIES, sid);
                }
            }
        }

    } // for (struct p2p_session *s = inst->sessions_head; s; s = s->next)
}

/*
 * 信令输出（推送阶段）— on submit/on complete
 * > 继续提交发送缓存中的数据
 * > 同步，根据最新状态机进行联动或随动
 *
 * 处理向对端发送的候选包：
 * - SYNCING：重发剩余候选和 FIN
 * - READY：发送新收集到的候选（如果有）
 */
void p2p_signal_compact_tick_send(struct p2p_instance *inst, uint64_t now) {

    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;
    if (sig_ctx->state != SIG_COMPACT_REG || sig_ctx->sig_sessions) return;

    if (tick_diff(now, sig_ctx->last_send_time) >= REG_KEEPALIVE_INTERVAL_MS) {

        /*
         * 发送 ALIVE 保活包
         *
         * 包头: [type=SIG_PKT_ALIVE | flags=0 | seq=0]
         * 负载: [auth_key(SIG_AUTH_KEY_PSZ)]
         *   - auth_key: 客户端-服务器认证令牌（来自 REG_ACK）
         * 说明: 保活包，维持服务器上的注册状态
         */
        {   const char* PROTO = "ALIVE";

            if (sig_ctx->auth_key) {

                uint8_t payload[SIG_PKT_ALV_PSZ];
                nwrite_ll(payload, sig_ctx->auth_key);

                err_t err = udp_send(inst, PROTO, SIG_PKT_ALV, 0, 0, payload, (int) sizeof(payload), now);
                if (err == E_NONE) {

                    print("V:", LA_F("%s sent\n", LA_F65, 65), PROTO);

                    // 通知路径管理器：ALIVE 是唯一需要 per-packet RTT 的信令包（仅当 SIGNALING 作为 relay 路径被启用时才统计 RTT）
                    if (inst->signaling.active) {
                        path_manager_on_sig_alive_send(inst, now);
                    }
                }
            }
            else print("W:", LA_F("%s skipped: auth_key=0\n", LA_F66, 66), PROTO);
        }
    }
}

/*
 * 根据 COMPACT 信令/探测状态推导并写入当前 NAT 检测结果到 s->inst->nat_type。
 * 由 p2p.c 在每次 update tick 中调用。
 */
void p2p_signal_compact_nat_detect_tick(struct p2p_instance *inst, uint64_t now) {

    assert(inst->sig_mode == P2P_SIGNALING_MODE_COMPACT);
    p2p_compact_ctx_t *sig_ctx = &inst->sig_ctx.compact;

    // 探测端口未知
    if (sig_ctx->state == SIG_COMPACT_INIT || sig_ctx->state == SIG_COMPACT_WAIT_REG_ACK) {
        return;
    }
    // 不支持探测
    if (!sig_ctx->probe_port) {
        // 服务器不支持 NAT probe，设置为 UNDETECTABLE
        if (inst->nat_type != P2P_NAT_UNDETECTABLE) {
            inst->nat_type = P2P_NAT_UNDETECTABLE;
        }
        return;
    }
    // 已经探测完成、或超时
    if (sig_ctx->nat_probe_retries < 0) {
        return;
    }

    // 间隔等待
    if (tick_diff(now, sig_ctx->nat_probe_send_time) < NAT_PROBE_INTERVAL_MS) return;

    if (sig_ctx->nat_probe_retries++ < NAT_PROBE_MAX_RETRIES) {

        print("V:", LA_F("%s: retry(%d/%d) probe\n", LA_F228, 228),
              TASK_NAT_PROBE, sig_ctx->nat_probe_retries, REQ_MAX_RETRIES);
        send_nat_probe(inst, now);
    }
    else {

        inst->nat_type = P2P_NAT_TIMEOUT;
        sig_ctx->nat_probe_retries = -2/* 探测超时 */;

        print("W:", LA_F("%s: timeout after %d retries , type unknown\n", LA_F257, 257), 
              REQ_MAX_RETRIES, TASK_NAT_PROBE);
    }
}

#pragma clang diagnostic pop
