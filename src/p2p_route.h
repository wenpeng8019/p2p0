
#ifndef P2P_ROUTE_H
#define P2P_ROUTE_H

#include "../stdc/stdc.h"   /* cross-platform utilities */

typedef struct {
    struct sockaddr_in* local_addrs;
    uint32_t*           local_masks;
    int                 addr_count;
    struct sockaddr_in  lan_peer_addr;   /* confirmed LAN addr */
    int                 lan_confirmed;
    uint64_t            probe_time;
} route_ctx_t;

void route_init(route_ctx_t *rt);
void route_final(route_ctx_t *rt);

/**
 * 检测获取本地所有有效的网络地址
 * 返回:
 * <0: 检测失败
 * >=0: 检测到的地址数量
 */
int  route_detect_local(route_ctx_t *rt);

bool  route_check_same_subnet(route_ctx_t *rt, const struct sockaddr_in *peer_priv);

int  route_send_probe(route_ctx_t *rt, sock_t sock, const struct sockaddr_in *peer_priv, uint16_t local_port);
int  route_on_probe(route_ctx_t *rt, const struct sockaddr_in *from, sock_t sock);
int  route_on_probe_ack(route_ctx_t *rt, const struct sockaddr_in *from);

#endif /* P2P_ROUTE_H */
