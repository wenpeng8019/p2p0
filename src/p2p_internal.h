...
static inline const char* p2p_nat_type_str(int type) {
    switch (type) {
        ...
        case P2P_NAT_TIMEOUT:           return LA_W("Timeout (no response)", LA_W10, 10);
        case P2P_NAT_UNKNOWN:           return LA_W("Unknown", LA_W13, 13);
        case P2P_NAT_OPEN:              return LA_W("Open Internet (No NAT)", LA_W4, 4);
        case P2P_NAT_FULL_CONE:         return LA_W("Full Cone NAT", LA_W3, 3);
        case P2P_NAT_RESTRICTED:        return LA_W("Restricted Cone NAT", LA_W7, 7);
        case P2P_NAT_PORT_RESTRICTED:   return LA_W("Port Restricted Cone NAT", LA_W5, 5);
        case P2P_NAT_SYMMETRIC:         return LA_W("Symmetric NAT (port-random)", LA_W9, 9);
        case P2P_NAT_BLOCKED:           return LA_W("UDP Blocked (STUN unreachable)", LA_W11, 11);
        case P2P_NAT_UNDETECTABLE:       return LA_W("Undetectable (no STUN/probe configured)", LA_W12, 12);
        default:                        return LA_W("Unknown", LA_W13, 13);
    }
}
...
static inline ret_t p2p_remote_cands_reserve(struct p2p_session *s, int need) {
    ...
    p2p_remote_candidate_entry_t *p = (p2p_remote_candidate_entry_t *)realloc(s->remote_cands, nc * sizeof(p2p_remote_candidate_entry_t));
    if (!p) {
        print("E:", LA_F("Failed to realloc memory for remote candidates (capacity: %d)", LA_F309, 309), nc);
