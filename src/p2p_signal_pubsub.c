...
void p2p_signal_pubsub_set_role(p2p_signal_pubsub_ctx_t *ctx, p2p_signal_role_t role) {
    ...
    print("I:", LA_F("%s %s", LA_F7, 263), LA_W("Initialized:", LA_W36, 46),
                 role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W70, 83) : LA_W("SUB", LA_W97, 124));
}
...
static void process_payload(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s, const char *b64_data) {
    ...
    if (!dec_buf) {
        print("E: %s",  LA_S("Out of memory", LA_S49, 192));
        return;
    }
    ...
    if (enc_len >= sizeof(p2p_signaling_payload_hdr_t) && unpack_signaling_payload_hdr(&payload, dec_buf) == 0 &&
        ...
        print("I:", LA_F("Received valid signal from '%s'", LA_F128, 349), payload.sender);
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            if (s->remote_cand_cnt > 0 || s->ice_state != P2P_ICE_STATE_INIT) {
                printf(LA_F("[DEBUG] First offer, resetting ICE and clearing %d stale candidates", LA_F157, 361), s->remote_cand_cnt);
                s->remote_cand_cnt = 0;
        ...
        for (int i = 0; i < payload.candidate_count; i++) {
            ...
            print("I:", LA_F("%s: %s=%d, %s=%s:%d", LA_F52, 312),
                 LA_W("Received remote candidate", LA_W74, 92), LA_S("type", LA_S88, 211), c->cand.type,
            ...
            if (s->ice_state == P2P_ICE_STATE_CHECKING) {
                ...
                printf(LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F161, 363),
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            ...
            if (n > 0) {
                ...
                print("I:", LA_F("Auto-send answer (with %d candidates) total sent %s", LA_F61, 316),
                       s->local_cand_cnt, payload.sender);
            }
        }
    } else {
        print("W: %s", LA_S("Signal payload deserialization failed", LA_S78, 205));
    }
    ...
    *dst = '\0';
     ...
     */
    {
        ...
        *bp = '\0';
        ...
        print("I:", LA_F("Updating Gist field '%s'...", LA_F153, 358), field_name);
...
void p2p_signal_pubsub_tick_recv(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s) {
    ...
    *dst2 = '\0';
    ...
    if (value_end - value_start < 10) {
        printf(LA_F("Field %s is empty or too short", LA_F70, 319), target_field);
        return;
    }
    ...
    *dst = '\0';
    ...
    if (strlen(content) > 10) {
        print("I:", LA_F("Processing (role=%s)", LA_F100, 339),
                     ctx->role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W70, 83) : LA_W("SUB", LA_W97, 124));
    ...
    if (!s->signal_sent ||
        ...
        if (n > 0) {
            ...
            print("I: [SIGNALING] %s %s %d %s %s\n",
                s->last_cand_cnt_sent > 0 ? LA_W("Resent", LA_W81, 103) : LA_W("Published", LA_W71, 84),
                LA_S("offer with", LA_S47, 190),
                s->local_cand_cnt,
                LA_W("to", LA_W104, 135),
                s->remote_peer_id);
        }
