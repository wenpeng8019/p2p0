...
static void derive_key(const char *auth_key, uint8_t key_out[8]) {
    ...
    if (auth_key[0]) {
        ...
        print("W: %s", LA_S("No auth_key provided, using default key (insecure)", LA_S42, 67));
...
ret_t p2p_signal_pubsub_init(p2p_signal_pubsub_ctx_t *ctx, const char *token, const char *channel_id) {
    ...
    if (!is_safe_string(channel_id)) {
        print("E: %s",  LA_S("Invalid channel_id format (security risk)", LA_S37, 62));
        return -1;
    }
    
    print("I:", LA_F("Initialized: %s", LA_F141, 233), channel_id);
    return E_NONE;
}
...
void p2p_signal_pubsub_set_role(p2p_signal_pubsub_ctx_t *ctx, p2p_signal_role_t role) {
    ...
    ctx->role = role;
    print("I:", LA_F("Initialized: %s", LA_F141, 233), 
...
static void process_payload(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s, const char *b64_data) {
    ...
    if (enc_len <= 0) {
        print("W: %s", LA_S("Base64 decode failed", LA_S14, 39));
        return;
    }
    ...
    if (!dec_buf) {
        print("E: %s",  LA_S("Out of memory", LA_S44, 69));
        return;
    }
    ...
    if (enc_len >= sizeof(p2p_signaling_payload_hdr_t) && unpack_signaling_payload_hdr(&payload, dec_buf) == 0 &&
        ...
        print("I:", LA_F("Received valid signal from '%s'", LA_F164, 256), payload.sender);
        ...
        for (int i = 0; i < payload.candidate_count; i++) {
            ...
            print("I:", LA_F("Received remote candidate: type=%d, address=%s:%d", LA_F163, 255),
            ...
            if (s->ice_state == P2P_ICE_STATE_CHECKING) {
                ...
                printf(LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F196, 290),
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            ...
            if (n > 0) {
                ...
                print("I:", LA_F("Auto-send answer (with %d candidates) total sent %s", LA_F119, 211),
                       s->local_cand_cnt, payload.sender);
            }
        }
    } else {
        print("W: %s", LA_S("Signal payload deserialization failed", LA_S61, 86));
    }
...
int p2p_signal_pubsub_send(p2p_signal_pubsub_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    if (!is_safe_string(ctx->channel_id)) {
        print("E: %s",  LA_S("Channel ID validation failed", LA_S16, 41));
        return -1;
    }
    ...
    *dst = '\0';
    ...
    if (ctx->role == P2P_SIGNAL_ROLE_PUB && existing_answer[0] != '\0') {
        print("I: %s", LA_S("Answer already present, skipping offer re-publish", LA_S13, 38));
        free(padded_data);
        free(enc_data);
        return 0;
    }
     ...
     */
    {
        ...
        *bp = '\0';
        ...
        print("I:", LA_F("Updating Gist field '%s'...", LA_F189, 282), field_name);
...
void p2p_signal_pubsub_tick_recv(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s) {
    ...
    *dst = '\0';
    ...
    if (strlen(content) > 10) {
        print("I:", LA_F("Processing (role=%s)", LA_F158, 250),
