...
int p2p_signal_pubsub_init(p2p_signal_pubsub_ctx_t *ctx, const char *token, const char *channel_id) {
    ...
    P2P_LOG_INFO("SIGNAL_PUBSUB", "%s %s", LA_W("Initialized:", LA_W45, 46), channel_id);
    return 0;
}
...
void p2p_signal_pubsub_set_role(p2p_signal_pubsub_ctx_t *ctx, p2p_signal_role_t role) {
    ...
    ctx->role = role;
    P2P_LOG_INFO("SIGNAL_PUBSUB", "%s %s", LA_W("Initialized:", LA_W45, 46), 
                 role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W82, 83) : LA_W("SUB", LA_W123, 124));
}
...
static void process_payload(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s, const char *b64_data) {
    ...
    if (enc_len <= 0) {
        P2P_LOG_WARN("SIGNAL_PUBSUB", "%s", LA_S("Base64 decode failed", LA_S8, 159));
        return;
    }
    ...
    if (enc_len >= sizeof(p2p_signaling_payload_hdr_t) && unpack_signaling_payload_hdr(&payload, dec_buf) == 0 &&
        ...
        P2P_LOG_INFO("SIGNAL_PUBSUB", "%s '%s'", LA_W("Received valid signal from", LA_W95, 96), payload.sender);
...
int p2p_signal_pubsub_send(p2p_signal_pubsub_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    if (!is_safe_string(ctx->channel_id)) {
        P2P_LOG_ERROR("SIGNAL_PUBSUB", "%s", LA_S("Channel ID validation failed", LA_S11, 162));
        return -1;
    }
    ...
    *dst = '\0';
    ...
    if (ctx->role == P2P_SIGNAL_ROLE_PUB && existing_answer[0] != '\0') {
        P2P_LOG_INFO("SIGNAL_PUBSUB", "%s", LA_S("Answer already present, skipping offer re-publish", LA_S7, 158));
        free(padded_data);
        free(enc_data);
        return 0;
    }
     ...
     */
    {
        ...
        *bp = '\0';
        ...
        P2P_LOG_INFO("SIGNAL_PUBSUB", "%s '%s'...", LA_W("Updating Gist field", LA_W141, 142), field_name);
...
void p2p_signal_pubsub_tick(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s) {
    ...
    *dst2 = '\0';
    ...
    if (value_end - value_start < 10) {
        P2P_LOG_DEBUG("SIGNAL_PUBSUB", "%s %s %s", LA_W("Field", LA_W36, 37), target_field, LA_W("is empty or too short", LA_W48, 49));
        return;
    }
    ...
    *dst = '\0';
    ...
    if (strlen(content) > 10) {
        P2P_LOG_INFO("SIGNAL_PUBSUB", "%s %s (%s=%s)",
                     LA_W("Processing", LA_W81, 82), target_field, LA_W("role", LA_W105, 106),
                     ctx->role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W82, 83) : LA_W("SUB", LA_W123, 124));
