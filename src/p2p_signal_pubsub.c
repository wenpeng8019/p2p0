...
static void derive_key(const char *auth_key, uint8_t key_out[8]) {
    ...
    if (auth_key[0]) {
        ...
        print("W: %s", LA_S("No auth_key provided, using default key (insecure)", LA_S44, 147));
...
ret_t p2p_signal_pubsub_init(p2p_signal_pubsub_ctx_t *ctx, const char *token, const char *channel_id) {
    ...
    if (!is_safe_string(channel_id)) {
        print("E: %s",  LA_S("Invalid channel_id format (security risk)", LA_S38, 141));
        return -1;
    }
    
    print("I:", LA_F("%s %s", LA_F7, 211), LA_W("Initialized:", LA_W32, 33), channel_id);
    return E_NONE;
}
...
void p2p_signal_pubsub_set_role(p2p_signal_pubsub_ctx_t *ctx, p2p_signal_role_t role) {
    ...
    ctx->role = role;
    print("I:", LA_F("%s %s", LA_F7, 211), LA_W("Initialized:", LA_W32, 33),
                 role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W58, 59) : LA_W("SUB", LA_W82, 83));
}
...
static void process_payload(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s, const char *b64_data) {
    ...
    if (enc_len <= 0) {
        print("W: %s", LA_S("Base64 decode failed", LA_S7, 110));
        return;
    }
    ...
    if (!dec_buf) {
        print("E: %s",  LA_S("Out of memory", LA_S48, 151));
        return;
    }
    ...
    if (enc_len >= sizeof(p2p_signaling_payload_hdr_t) && unpack_signaling_payload_hdr(&payload, dec_buf) == 0 &&
        ...
        print("I:", LA_F("Received valid signal from '%s'", LA_F171, 375), payload.sender);
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            if (s->remote_cand_cnt > 0 || s->ice_state != P2P_ICE_STATE_INIT) {
                printf(LA_F("[DEBUG] First offer, resetting ICE and clearing %d stale candidates", LA_F194, 398), s->remote_cand_cnt);
                s->remote_cand_cnt = 0;
        ...
        for (int i = 0; i < payload.candidate_count; i++) {
            ...
            print("I:", LA_F("%s: %s=%d, %s=%s:%d", LA_F64, 268),
                 LA_W("Received remote candidate", LA_W63, 64), LA_S("type", LA_S98, 201), c->cand.type,
            ...
            if (s->ice_state == P2P_ICE_STATE_CHECKING) {
                ...
                printf(LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F198, 402),
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            ...
            if (n > 0) {
                ...
                print("I:", LA_F("Auto-send answer (with %d candidates) total sent %s", LA_F134, 338),
                       s->local_cand_cnt, payload.sender);
            }
        }
    } else {
        print("W: %s", LA_S("Signal payload deserialization failed", LA_S90, 193));
    }
...
int p2p_signal_pubsub_send(p2p_signal_pubsub_ctx_t *ctx, const char *target_name, const void *data, int len) {
    ...
    if (!is_safe_string(ctx->channel_id)) {
        print("E: %s",  LA_S("Channel ID validation failed", LA_S10, 113));
        return -1;
    }
    ...
    *dst = '\0';
    ...
    if (ctx->role == P2P_SIGNAL_ROLE_PUB && existing_answer[0] != '\0') {
        print("I: %s", LA_S("Answer already present, skipping offer re-publish", LA_S6, 109));
        free(padded_data);
        free(enc_data);
        return 0;
    }
     ...
     */
    {
        ...
        *bp = '\0';
        ...
        print("I:", LA_F("Updating Gist field '%s'...", LA_F190, 394), field_name);
...
void p2p_signal_pubsub_tick_recv(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s) {
    ...
    if (got <= 0) {
        printf("%s", LA_S("Gist GET failed", LA_S35, 138));
        free(buffer);
        return;
    }
    ...
    *dst2 = '\0';
    ...
    if (value_end - value_start < 10) {
        printf(LA_F("Field %s is empty or too short", LA_F143, 347), target_field);
        return;
    }
    ...
    *dst = '\0';
    ...
    if (strlen(content) > 10) {
        print("I:", LA_F("Processing (role=%s)", LA_F156, 360),
                     ctx->role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W58, 59) : LA_W("SUB", LA_W82, 83));
    ...
    if (!s->signal_sent ||
        ...
        if (n > 0) {
            ...
            print("I: [SIGNALING] %s %s %d %s %s\n",
                s->last_cand_cnt_sent > 0 ? LA_W("Resent", LA_W68, 69) : LA_W("Published", LA_W59, 60),
                LA_S("offer with", LA_S46, 149),
                s->local_cand_cnt,
                LA_W("to", LA_W89, 90),
                s->remote_peer_id);
        }
