...
static void derive_key(const char *auth_key, uint8_t key_out[8]) {
    ...
    if (auth_key[0]) {
        ...
        printf("W: %s", LA_S("No auth_key provided, using default key (insecure)", LA_S50, 188));
...
int p2p_signal_pubsub_init(p2p_signal_pubsub_ctx_t *ctx, const char *token, const char *channel_id) {
    ...
    if (!is_safe_string(channel_id)) {
        printf("E: %s",  LA_S("Invalid channel_id format (security risk)", LA_S44, 184));
        return -1;
    }
    
    printf("I:", LA_F("%s %s", LA_F8, 263), LA_W("Initialized:", LA_W40, 46), channel_id);
    return 0;
}
...
void p2p_signal_pubsub_set_role(p2p_signal_pubsub_ctx_t *ctx, p2p_signal_role_t role) {
    ...
    ctx->role = role;
    printf("I:", LA_F("%s %s", LA_F8, 263), LA_W("Initialized:", LA_W40, 46),
                 role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W74, 83) : LA_W("SUB", LA_W108, 124));
}
...
static void process_payload(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s, const char *b64_data) {
    ...
    if (!dec_buf) {
        printf("E: %s",  LA_S("Out of memory", LA_S55, 192));
        return;
    }
    ...
    if (enc_len >= sizeof(p2p_signaling_payload_hdr_t) && unpack_signaling_payload_hdr(&payload, dec_buf) == 0 &&
        ...
        printf("I:", LA_F("Received valid signal from '%s'", LA_F100, 349), payload.sender);
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            if (s->remote_cand_cnt > 0 || s->ice_state != P2P_ICE_STATE_INIT) {
                printf("D:", LA_F("[DEBUG] First offer, resetting ICE and clearing %d stale candidates", LA_F117, 361), s->remote_cand_cnt);
                s->remote_cand_cnt = 0;
        ...
        for (int i = 0; i < payload.candidate_count; i++) {
            ...
            printf("I:", LA_F("%s: %s=%d, %s=%s:%d", LA_F57, 312),
                 LA_W("Received remote candidate", LA_W81, 92), LA_S("type", LA_S88, 211), c->cand.type,
            ...
            if (s->ice_state == P2P_ICE_STATE_CHECKING) {
                ...
                printf("D:", LA_F("[Trickle] Immediately probing new candidate %s:%d", LA_F121, 363),
        ...
        if (ctx->role == P2P_SIGNAL_ROLE_SUB && !ctx->answered) {
            ...
            if (n > 0) {
                ...
                printf("I:", LA_F("Auto-send answer (with %d candidates) total sent %s", LA_F62, 316),
                       s->local_cand_cnt, payload.sender);
            }
        }
    } else {
        printf("W: %s", LA_S("Signal payload deserialization failed", LA_S80, 205));
    }
    ...
    *dst = '\0';
     ...
     */
    {
        ...
        *bp = '\0';
        ...
        printf("I:", LA_F("Updating Gist field '%s'...", LA_F114, 358), field_name);
...
void p2p_signal_pubsub_tick(p2p_signal_pubsub_ctx_t *ctx, struct p2p_session *s) {
    ...
    if (got <= 0) {
        printf("D: %s", LA_S("Gist GET failed", LA_S41, 182));
        free(buffer);
        return;
    }
    ...
    *dst = '\0';
    ...
    if (strlen(content) > 10) {
        printf("I:", LA_F("Processing (role=%s)", LA_F90, 339),
                     ctx->role == P2P_SIGNAL_ROLE_PUB ? LA_W("PUB", LA_W74, 83) : LA_W("SUB", LA_W108, 124));
