...
p2p_create(const char *local_peer_id, const p2p_config_t *cfg) {
    ...
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_PUBSUB) {
        if (!cfg->gh_token || !cfg->gist_id) {
            print("E: %s", LA_S("PUBSUB mode requires gh_token and gist_id", LA_S80, 184));
            return NULL;
        }
    }
    else if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT || cfg->signaling_mode == P2P_SIGNALING_MODE_RELAY) {
        if (!cfg->server_host) {
            print("E: %s", LA_S("RELAY/COMPACT mode requires server_host", LA_S84, 188));
            return NULL;
        }
    } else {
        print("E: %s", LA_S("Invalid signaling mode in configuration", LA_S40, 144));
        return NULL;
    }
    ...
    if (!s) {
        print("E: %s", LA_S("Failed to allocate memory for session", LA_S26, 130));
        P_sock_close(sock);
        return NULL;
    }

    // 初始化信令服务模式
    print("I:", LA_F("Initialize signaling mode: %d", LA_F148, 354), (int)cfg->signaling_mode);
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT) p2p_signal_compact_init(&s->sig_compact_ctx);
        ...
        print("E:", LA_F("Initialize PUBSUB signaling context failed(%d)", LA_F147, 353), ret);
    ...
    if ((ret = route_detect_local(&s->route)) < 0) {
        print("E:", LA_F("Detect local network interfaces failed(%d)", LA_F140, 346), ret);
        free(s); P_sock_close(sock);
        return NULL;
    }
    ...
    if (!s->local_cands || !s->remote_cands) {
        print("E: %s", LA_S("Failed to allocate memory for candidate lists", LA_S25, 129));
...
#else
        print("W: %s", LA_S("DTLS (MbedTLS) requested but library not linked", LA_S21, 125));
#endif
    } 
...
#else
        print("W: %s", LA_S("OpenSSL requested but library not linked", LA_S48, 152));
#endif
    }
...
#else
        print("W: %s", LA_S("SCTP (usrsctp) requested but library not linked", LA_S87, 191));
#endif
    }
    else if (cfg->use_pseudotcp) {
        print("I: %s", LA_S("PseudoTCP enabled as transport layer", LA_S77, 181));
        s->trans = &p2p_trans_pseudotcp;
    }
    else print("I: %s", LA_S("No advanced transport layer enabled, using simple reliable layer", LA_S44, 148));
...
p2p_destroy(p2p_handle_t hdl) {
...
#ifdef P2P_THREADED
    if (s->cfg.threaded) {
        print("I: %s", LA_S("Stopping internal thread", LA_S95, 199));
        p2p_thread_stop(s);
    }
#endif
    ...
    if (s->signaling_mode == P2P_SIGNALING_MODE_RELAY
        ...
        print("I: %s", LA_S("Closing TCP connection to RELAY signaling server", LA_S13, 117));
...
p2p_connect(p2p_handle_t hdl, const char *remote_peer_id) {
    ...
    if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT && !remote_peer_id) {
        print("E: %s", LA_S("COMPACT mode requires explicit remote_peer_id", LA_S14, 118));
    ...
    switch (s->signaling_mode) {
        ...
        case P2P_SIGNALING_MODE_COMPACT: {
            ...
            if (!s->cfg.skip_host_candidates) {
                for (int i = 0; i < s->route.addr_count; i++) {
                    ...
                    print("I:", LA_F("Append Host candidate: %s:%d", LA_F134, 340),
            ...
            if ((ret = p2p_signal_compact_connect(s, s->local_peer_id, remote_peer_id, &server_addr)) != E_NONE) {
                print("E:", LA_F("Connect to COMPACT signaling server failed(%d)", LA_F137, 343), ret);
                s->state = P2P_STATE_ERROR;
                UNLOCK(s);
                return -1;
            }
        ...
        case P2P_SIGNALING_MODE_RELAY: {
            ...
            if (s->sig_relay_ctx.state != SIGNAL_CONNECTED) {
                ...
                print("I:", LA_F("Connecting to RELAY signaling server at %s:%d", LA_F139, 345),
                ...
                if ((ret = p2p_signal_relay_login(&s->sig_relay_ctx, s->cfg.server_host, s->cfg.server_port, s->local_peer_id)) != E_NONE) {
                    print("E:", LA_F("Connect to RELAY signaling server failed(%d)", LA_F138, 344), ret);
                    s->state = P2P_STATE_ERROR;
                    UNLOCK(s);
                    return -1;
                }
            ...
            // 被动模式：等待任意对等方的 offer
                ...
                print("I: %s", LA_W("Waiting for incoming offer from any peer", LA_W96, 97));
            }
        ...
        case P2P_SIGNALING_MODE_PUBSUB: {
            ...
            if (remote_peer_id) {
                ...
                print("I: %s", LA_S("PUBSUB (PUB): gathering candidates, waiting for STUN before publishing", LA_S78, 182));
            }
            else {
                ...
                print("I: %s", LA_S("PUBSUB (SUB): waiting for offer from any peer", LA_S79, 183));
            }
            break;
        }
...
p2p_update(p2p_handle_t hdl) {
    ...
    if (s->state == P2P_STATE_REGISTERING && s->nat.state == NAT_PUNCHING) {
        print("I: %s", LA_S("P2P punching in progress ...", LA_S53, 157));
    ...
    if ((s->state == P2P_STATE_PUNCHING || s->state == P2P_STATE_REGISTERING) &&
        ...
        print("I: %s", LA_S("P2P connection established", LA_S51, 155));
    ...
    if (s->nat.state == NAT_CONNECTED && s->state == P2P_STATE_RELAY) {
        print("I: %s", LA_S("NAT connection recovered, upgrading from RELAY to CONNECTED", LA_S41, 145));
        ...
        if (best_path >= 0) {
            ...
            if (addr) {
                ...
                print("I:", LA_F("Path recovered: switched to PUNCH", LA_F154, 359));
            }
        }
    }
    ...
    if (s->state == P2P_STATE_PUNCHING && s->nat.state == NAT_RELAY) {
        ...
        // 添加中继路径
        print("I: %s", LA_S("P2P punch failed, adding relay path", LA_S52, 156));
        ...
        if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT) {
            if (s->sig_compact_ctx.relay_support) {
                ...
                print("I: %s", LA_W("NAT punch failed, using COMPACT server relay", LA_W43, 44));
            } else {
                print("W: %s", LA_W("NAT punch failed, server has no relay support", LA_W42, 43));
            }
        } else {
            print("W: %s", LA_W("NAT punch failed, no TURN server configured", LA_W41, 42));
        }
        
        if (relay_available) {
            /* 方案 A：设置 RELAY 路径 */
            path_manager_set_relay(s, &relay_addr);
            print("I:", LA_F("Added RELAY path to path manager", LA_F133, 339));
        }
        ...
        if (best_path >= -1) {  // -1=RELAY, >=0=候选
            ...
            if (addr) {
                ...
                print("I:", LA_F("Using path: RELAY", LA_F197, 400));
            }
        } else {
            // 无可用路径：降级到传统方式
            s->state = P2P_STATE_RELAY;
    ...
    if (s->nat.state == NAT_LOST && s->state == P2P_STATE_CONNECTED) {
        print("W: %s", LA_S("NAT connection timeout, downgrading to relay mode", LA_S42, 146));
        ...
        if (best_path >= -1) {  // -1=RELAY, >=0=候选
            ...
            if (addr) {
                ...
                s->path_mgr.active_path = best_path;
                s->state = P2P_STATE_RELAY;
                print("I:", LA_F("Switched to backup path: RELAY", LA_F191, 395));
            }
        } else {
            // 无备用路径：NAT 层会继续尝试恢复
            s->state = P2P_STATE_RELAY;
    ...
    if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
        ...
        if (s->path_mgr.active_path >= -1) {  // -1=RELAY, >=0=候选
            ...
            if (active_addr && !sockaddr_equal(&s->active_addr, active_addr)) {
                ...
                print("I:", LA_F("Synced path after failover", LA_F192, 360));
                ...
                if (should_switch) {
                    ...
                    if (ret == 0) {
                        ...
                        if (new_addr) {
                            ...
                            print("I:", LA_F("Path switched to better route (idx=%d)", LA_F156, 360), best_path);
                        }
                    } else if (ret == 1) {
                        print("V: %s", LA_S("Path switch debounced, waiting for stability", 0, 0));
                    }
                }
            }
        }
