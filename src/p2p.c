...
p2p_create(const char *local_peer_id, const p2p_config_t *cfg) {
    ...
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_PUBSUB) {
        if (!cfg->gh_token || !cfg->gist_id) {
            print("E: %s", LA_S("PUBSUB mode requires gh_token and gist_id", LA_S58, 195));
            return NULL;
        }
    }
    else if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT || cfg->signaling_mode == P2P_SIGNALING_MODE_RELAY) {
        if (!cfg->server_host) {
            print("E: %s", LA_S("RELAY/COMPACT mode requires server_host", LA_S66, 199));
            return NULL;
        }
    } else {
        print("E: %s", LA_S("Invalid signaling mode in configuration", LA_S39, 185));
        return NULL;
    }

    // 创建 UDP 套接字，port 0 为有效值，表示由操作系统分配随机端口
    print("I:", LA_F("Open P2P UDP socket on port %d", LA_F89, 232), cfg->bind_port);
    sock_t sock = udp_open_socket(cfg->bind_port);
    if (sock == P_INVALID_SOCKET) {
        print("E:", LA_F("Open P2P UDP socket on port %d failed(%d)", LA_F90, 33), cfg->bind_port, P_sock_errno());
        return NULL;
    }

    p2p_session_t *s = (p2p_session_t*)calloc(1, sizeof(*s));
    if (!s) {
        print("E: %s", LA_S("Failed to allocate memory for session", LA_S27, 225));
        P_sock_close(sock);
        return NULL;
    }

    // 初始化信令服务模式
    print("I:", LA_F("Initialize signaling mode: %d", LA_F75, 243), (int)cfg->signaling_mode);
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT) p2p_signal_compact_init(&s->sig_compact_ctx);
        ...
        print("E:", LA_F("Initialize PUBSUB signaling context failed(%d)", LA_F74, 229), ret);
    ...
    if ((ret = route_detect_local(&s->route)) < 0) {
        print("E:", LA_F("Detect local network interfaces failed(%d)", LA_F63, 227), ret);
        free(s); P_sock_close(sock);
        return NULL;
    }
    ...
    if (!s->local_cands || !s->remote_cands) {
        print("E: %s", LA_S("Failed to allocate memory for candidate lists", LA_S26, 224));
    ...
    print("I:", LA_F("Path manager initialized with strategy: %d (0=conn,1=perf,2=hybrid)", LA_F89, 232), strategy);
...
#else
        print("W: %s", LA_S("OpenSSL requested but library not linked", LA_S47, 191));
#endif
    }
...
#else
        print("W: %s", LA_S("SCTP (usrsctp) requested but library not linked", LA_S71, 202));
#endif
    }
    else if (cfg->use_pseudotcp) {
        print("I: %s", LA_S("PseudoTCP enabled as transport layer", LA_S55, 233));
        s->trans = &p2p_trans_pseudotcp;
    }
    else print("I: %s", LA_S("No advanced transport layer enabled, using simple reliable layer", LA_S44, 231));
...
#ifdef P2P_THREADED
    if (cfg->threaded) {
        print("I: %s", LA_S("Starting internal thread", LA_S81, 237));
        if ((ret = p2p_thread_start(s)) != E_NONE) {
            print("E:", LA_F("Start internal thread failed(%d)", LA_F116, 570), ret);
            P_sock_close(s->sock);
...
p2p_destroy(p2p_handle_t hdl) {
...
#ifdef P2P_THREADED
    if (s->cfg.threaded) {
        print("I: %s", LA_S("Stopping internal thread", LA_S82, 238));
        p2p_thread_stop(s);
    }
#endif
    ...
    if (s->sock != P_INVALID_SOCKET) {
        ...
        if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
            ...
            print("I: %s", LA_S("Sending FIN packet to peer", LA_S73, 234));
        ...
        if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT &&
            ...
            print("I: %s", LA_S("Sending UNREGISTER packet to COMPACT signaling server", LA_S75, 236));
            ...
            printf(LA_F("Sent UNREGISTER Pkt for local_peer_id=%s, remote_peer_id=%s", LA_F113, 240),
    ...
    switch (s->signaling_mode) {
        ...
        case P2P_SIGNALING_MODE_COMPACT: {
            ...
            if ((ret = resolve_host(s->cfg.server_host, s->cfg.server_port, &server_addr)) != E_NONE) {
                print("E:", LA_F("Resolve COMPACT signaling server address: %s:%d failed(%d)", LA_F109, 230),
            ...
            if (!s->cfg.skip_host_candidates) { p2p_candidate_entry_t *c;
                for (int i = 0; i < s->route.addr_count; i++) {
                    ...
                    print("I:", LA_F("Append Host candidate: %s:%d", LA_F57, 4),
                                 inet_ntoa(c->addr.sin_addr), ntohs(c->addr.sin_port));
                }
            }
            else print("I: %s", LA_S("Skipping local Host candidates on --public-only", LA_S79, 121));

            // 注册（连接）到 COMPACT 信令服务器
            print("I:", LA_F("Register to COMPACT signaling server at %s:%d", LA_F105, 244),
                         inet_ntoa(server_addr.sin_addr), ntohs(server_addr.sin_port));

            if ((ret = p2p_signal_compact_connect(s, s->local_peer_id, remote_peer_id, &server_addr)) != E_NONE) {
                print("E:", LA_F("Connect to COMPACT signaling server failed(%d)", LA_F60, 226), ret);
                s->state = P2P_STATE_ERROR;
                UNLOCK(s);
                return -1;
            }
        ...
        case P2P_SIGNALING_MODE_RELAY: {
            ...
            if (s->sig_relay_ctx.state != SIGNAL_CONNECTED) {
                ...
                print("I:", LA_F("Connecting to RELAY signaling server at %s:%d", LA_F62, 239),
                ...
                if ((ret = p2p_signal_relay_login(&s->sig_relay_ctx, s->cfg.server_host, s->cfg.server_port, s->local_peer_id)) != E_NONE) {
                    print("E:", LA_F("Connect to RELAY signaling server failed(%d)", LA_F61, 176), ret);
                    s->state = P2P_STATE_ERROR;
                    UNLOCK(s);
                    return -1;
                }
            ...
            if (remote_peer_id && s->local_cand_cnt > 0) {
                ...
                print("I:", LA_F("Sent initial offer(%d) to %s)", LA_F114, 245), n, remote_peer_id);
                if ((ret = p2p_signal_relay_send_connect(&s->sig_relay_ctx, remote_peer_id, buf, n)) != E_NONE) {
                    print("E:", LA_F("Send offer to RELAY signaling server failed(%d)", LA_F112, 568), ret);
                    // todo
                }
                s->signal_sent = true;
            }
            // 被动模式：等待任意对等方的 offer
                ...
                print("I: %s", LA_W("Waiting for incoming offer from any peer", LA_W115, 100));
            }
        ...
        case P2P_SIGNALING_MODE_PUBSUB: {
            ...
            if (remote_peer_id) {
                ...
                print("I: %s", LA_S("PUBSUB (PUB): gathering candidates, waiting for STUN before publishing", LA_S56, 85));
            }
            else {
                ...
                print("I: %s", LA_S("PUBSUB (SUB): waiting for offer from any peer", LA_S57, 86));
            }
            break;
        }

        default:
            print("E:", LA_F("Unknown signaling mode: %d", LA_F120, 140), s->signaling_mode);
            s->state = P2P_STATE_ERROR;
            UNLOCK(s);
            return -1;
    }
...
p2p_close(p2p_handle_t hdl) {
    ...
    if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
        ...
        print("V: %s", LA_S("Sending FIN packet to peer before closing", LA_S74, 235));
        udp_send_packet(s->sock, &s->active_addr, P2P_PKT_FIN, 0, 0, NULL, 0);
    }
...
p2p_update(p2p_handle_t hdl) {
    ...
    if (s->route.lan_confirmed && s->path != P2P_PATH_LAN) {
        print("I: %s", LA_S("Same subnet confirmed, switching to LAN path", LA_S70, 624));
        ...
        if (lan_idx >= 0) {
            ...
            print("I:", LA_F("Added LAN path to path manager, idx=%d", LA_F89, 232), lan_idx);
            ...
            if (best_path >= 0 && best_path == lan_idx) {
                ...
                print("I: %s", LA_S("Switched to LAN path", LA_S70, 624));
            }
        } else {
            // 降级：路径管理器失败，使用传统方式
            s->path = P2P_PATH_LAN;
    ...
    if (s->state == P2P_STATE_REGISTERING && s->nat.state == NAT_PUNCHING) {
        print("I: %s", LA_S("P2P punching in progress ...", LA_S52, 623));
    ...
    if ((s->state == P2P_STATE_PUNCHING || s->state == P2P_STATE_REGISTERING) &&
        ...
        print("I: %s", LA_S("P2P connection established", LA_S50, 621));
        ...
        if (path_idx >= 0) {
            ...
            print("I:", LA_F("Added PUNCH path to path manager, idx=%d", LA_F89, 232), path_idx);
        }
        ...
        if (best_path >= 0) {
            ...
            if (pi) {
                ...
                s->path_mgr.active_path = best_path;
                print("I:", LA_F("Selected path: %s (idx=%d)", LA_F89, 232),
        ...
        if (peer_priv && route_check_same_subnet(&s->route, peer_priv)) {
            ...
            if (!s->cfg.disable_lan_shortcut) {
                ...
                print("I:", LA_F("Same subnet detecting, sent ROUTE_PROBE to %s:%d", LA_F111, 109),
                       inet_ntoa(priv.sin_addr), ntohs(priv.sin_port));
            }
            // 同子网但 disable_lan_shortcut=true：跳过升级，仅打印日志
            else print("V: %s", LA_S("Skipping LAN shortcut on --disable-lan-shortcut", LA_S78, 122));
        }
    }

    // NAT 重新连接后恢复路径（NAT_RELAY → NAT_CONNECTED）
    if (s->nat.state == NAT_CONNECTED && s->state == P2P_STATE_RELAY) {
        print("I: %s", LA_S("NAT connection recovered, upgrading from RELAY to CONNECTED", LA_S42, 620));
        ...
        if (best_path >= 0) {
            ...
            print("I:", LA_F("Path recovered: switched to %s", LA_F89, 232), path_type_str(pi->type));
        }
    }
    ...
    if (s->state == P2P_STATE_PUNCHING && s->nat.state == NAT_RELAY) {
        ...
        // 添加中继路径
        print("I: %s", LA_S("P2P punch failed, adding relay path", LA_S51, 622));
        ...
        if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT) {
            if (s->sig_compact_ctx.relay_support) {
                ...
                print("I: %s", LA_W("NAT punch failed, using COMPACT server relay", LA_W53, 62));
            } else {
                print("W: %s", LA_W("NAT punch failed, server has no relay support", LA_W52, 61));
            }
        } else {
            print("W: %s", LA_W("NAT punch failed, no TURN server configured", LA_W51, 60));
        }
        ...
        if (relay_available) {
            ...
            if (relay_idx >= 0) {
                ...
                print("I:", LA_F("Added RELAY path to path manager, idx=%d", LA_F89, 232), relay_idx);
            }
        }
        ...
        if (best_path >= 0) {
            ...
            print("I:", LA_F("Using path: %s", LA_F89, 232), path_type_str(pi->type));
        } else {
            // 无可用路径：降级到传统方式
            s->state = P2P_STATE_RELAY;
    ...
    if (s->nat.state == NAT_DISCONNECTED && s->state == P2P_STATE_CONNECTED) {
        print("W: %s", LA_S("NAT connection timeout, downgrading to relay mode", LA_S42, 620));
        ...
        if (best_path >= 0) {
            ...
            s->path_mgr.active_path = best_path;
            s->state = P2P_STATE_RELAY;
            print("I:", LA_F("Switched to backup path: %s", LA_F89, 232), path_type_str(pi->type));
        } else {
            // 无备用路径：NAT 层会继续尝试恢复
            s->state = P2P_STATE_RELAY;
        ...
        if (now_ms - last_path_reselect > 5000) {
            ...
            if (best_path >= 0 && best_path != current_path &&
                ...
                if (should_switch) {
                    ...
                    print("I:", LA_F("Path switched: %s -> %s (RTT: %u -> %u ms)", LA_F89, 232),
