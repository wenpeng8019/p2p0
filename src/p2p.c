...
p2p_create(const char *local_peer_id, const p2p_config_t *cfg) {
    ...
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_PUBSUB) {
        if (!cfg->gh_token || !cfg->gist_id) {
            print("E: %s", LA_S("PUBSUB mode requires gh_token and gist_id", LA_S61, 82));
            return NULL;
        }
    }
    else if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT || cfg->signaling_mode == P2P_SIGNALING_MODE_RELAY) {
        if (!cfg->server_host) {
            print("E: %s", LA_S("RELAY/COMPACT mode requires server_host", LA_S63, 84));
            return NULL;
        }
    } else {
        print("E: %s", LA_S("Invalid signaling mode in configuration", LA_S45, 66));
        return NULL;
    }

    // 创建 UDP 套接字，port 0 为有效值，表示由操作系统分配随机端口
    print("I:", LA_F("Open P2P UDP socket on port %d", LA_F154, 250), cfg->bind_port);
    sock_t sock = udp_open_socket(cfg->bind_port);
    if (sock == P_INVALID_SOCKET) {
        print("E:", LA_F("Open P2P UDP socket on port %d failed(%d)", LA_F155, 251), cfg->bind_port, P_sock_errno());
        return NULL;
    }

    p2p_session_t *s = (p2p_session_t*)calloc(1, sizeof(*s));
    if (!s) {
        print("E: %s", LA_S("Failed to allocate memory for session", LA_S33, 55));
        P_sock_close(sock);
        return NULL;
    }

    // 初始化信令服务模式
    print("I:", LA_F("Initialize signaling mode: %d", LA_F147, 243), (int)cfg->signaling_mode);
    if (cfg->signaling_mode == P2P_SIGNALING_MODE_COMPACT) p2p_signal_compact_init(&s->sig_compact_ctx);
        ...
        print("E:", LA_F("Initialize PUBSUB signaling context failed(%d)", LA_F146, 242), ret);
    ...
    if ((ret = route_detect_local(&s->route)) < 0) {
        print("E:", LA_F("Detect local network interfaces failed(%d)", LA_F130, 226), ret);
        free(s); P_sock_close(sock);
        return NULL;
    }
    ...
    if (!s->local_cands || !s->remote_cands) {
        print("E: %s", LA_S("Failed to allocate memory for candidate lists", LA_S32, 54));
    ...
    if (cfg->use_sctp) {
        ...
        print("I: %s", LA_S("PseudoTCP enabled as transport layer", LA_S58, 79));
        s->trans = &p2p_trans_pseudotcp;
    }
    else print("I: %s", LA_S("No advanced transport layer enabled, using simple reliable layer", LA_S48, 69));
...
#ifdef P2P_THREADED
    if (cfg->threaded) {
        print("I: %s", LA_S("Starting internal thread", LA_S72, 91));
        if ((ret = p2p_thread_start(s)) != E_NONE) {
            print("E:", LA_F("Start internal thread failed(%d)", LA_F205, 301), ret);
            P_sock_close(s->sock);
...
p2p_destroy(p2p_handle_t hdl) {
...
#ifdef P2P_THREADED
    if (s->cfg.threaded) {
        print("I: %s", LA_S("Stopping internal thread", LA_S73, 92));
        p2p_thread_stop(s);
    }
#endif
    ...
    if (s->sock != P_INVALID_SOCKET) {
        ...
        if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
            ...
            print("I: %s", LA_S("Sending FIN packet to peer on destroy", LA_S66, 87));
            nat_send_fin(s);
        }
        ...
        if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT &&
            ...
            print("I: %s", LA_S("Sending UNREGISTER packet to COMPACT signaling server", LA_S67, 88));
        ...
        print("I: %s", LA_S("Close P2P UDP socket", LA_S20, 43));
        P_sock_close(s->sock);
    }
    ...
    if (s->signaling_mode == P2P_SIGNALING_MODE_RELAY
        ...
        print("I: %s", LA_S("Closing TCP connection to RELAY signaling server", LA_S21, 44));
...
p2p_connect(p2p_handle_t hdl, const char *remote_peer_id) {
    ...
    if (s->signaling_mode == P2P_SIGNALING_MODE_COMPACT && !remote_peer_id) {
        print("E: %s", LA_S("COMPACT mode requires explicit remote_peer_id", LA_S22, 45));
    ...
    switch (s->signaling_mode) {
        ...
        case P2P_SIGNALING_MODE_COMPACT: {
            ...
            if ((ret = resolve_host(s->cfg.server_host, s->cfg.server_port, &server_addr)) != E_NONE) {
                print("E:", LA_F("Resolve COMPACT signaling server address: %s:%d failed(%d)", LA_F190, 286),
            ...
            if (!s->cfg.skip_host_candidates) {
                for (int i = 0; i < s->route.addr_count; i++) {
                    ...
                    print("I:", LA_F("Append Host candidate: %s:%d", LA_F119, 215),
                                 inet_ntoa(c->addr.sin_addr), ntohs(c->addr.sin_port));
                }
            }
            else print("I: %s", LA_S("Skipping local Host candidates on --public-only", LA_S69, 90));

            // 注册（连接）到 COMPACT 信令服务器
            print("I:", LA_F("Register to COMPACT signaling server at %s:%d", LA_F185, 281),
                         inet_ntoa(server_addr.sin_addr), ntohs(server_addr.sin_port));

            if ((ret = p2p_signal_compact_connect(s, s->local_peer_id, remote_peer_id, &server_addr)) != E_NONE) {
                print("E:", LA_F("Connect to COMPACT signaling server failed(%d)", LA_F124, 220), ret);
                s->state = P2P_STATE_ERROR;
                UNLOCK(s);
                return -1;
            }
        ...
        case P2P_SIGNALING_MODE_RELAY: {
            ...
            if (s->sig_relay_ctx.state != SIGNAL_CONNECTED) {
                ...
                print("I:", LA_F("Connecting to RELAY signaling server at %s:%d", LA_F127, 223),
                ...
                if ((ret = p2p_signal_relay_login(&s->sig_relay_ctx, s->cfg.server_host, s->cfg.server_port, s->local_peer_id)) != E_NONE) {
                    print("E:", LA_F("Connect to RELAY signaling server failed(%d)", LA_F125, 221), ret);
                    s->state = P2P_STATE_ERROR;
                    UNLOCK(s);
                    return -1;
                }
            ...
            if (remote_peer_id && s->local_cand_cnt > 0) {
                ...
                print("I:", LA_F("Sent initial offer(%d) to %s)", LA_F204, 300), n, remote_peer_id);
                if ((ret = p2p_signal_relay_send_connect(&s->sig_relay_ctx, remote_peer_id, buf, n)) != E_NONE) {
                    print("E:", LA_F("Send offer to RELAY signaling server failed(%d)", LA_F197, 293), ret);
                    // todo
                }
                s->signal_sent = true;
            }
        ...
        case P2P_SIGNALING_MODE_PUBSUB: {
            ...
            if (remote_peer_id) {
                ...
                print("I: %s", LA_S("PUBSUB (PUB): gathering candidates, waiting for STUN before publishing", LA_S59, 80));
            }
            else {
                ...
                print("I: %s", LA_S("PUBSUB (SUB): waiting for offer from any peer", LA_S60, 81));
            }
            break;
        }

        default:
            print("E:", LA_F("Unknown signaling mode: %d", LA_F217, 312), s->signaling_mode);
            s->state = P2P_STATE_ERROR;
            UNLOCK(s);
            return -1;
    }
...
p2p_close(p2p_handle_t hdl) {
    ...
    if (s->state == P2P_STATE_CONNECTED || s->state == P2P_STATE_RELAY) {
        ...
        print("V: %s", LA_S("Sending FIN packet to peer before closing", LA_S65, 86));
        nat_send_fin(s);
    }
...
p2p_update(p2p_handle_t hdl) {
    ...
    while ((n = udp_recv_from(s->sock, &from, buf, sizeof(buf))) > 0) {
        ...
        if (n >= 20 && buf[0] < 2) { // STUN type 0x00xx or 0x01xx
            ...
            if (magic == STUN_MAGIC) {
                ...
                printf(LA_F("Received STUN/TURN pkt from %s:%d, type=0x%04x, len=%d", LA_F173, 269),
        ...
        switch (hdr.type) {
             ...
             */
            case P2P_PKT_DATA:
                printf(LA_F("Received DATA pkt from %s:%d, seq=%u, len=%d", LA_F171, 267),
            ...
            handle_ack: {
                ...
                if (hdr.type == P2P_PKT_ACK) {
                    printf(LA_F("Received ACK pkt from %s:%d, ack_seq=%u, sack=0x%08x", LA_F170, 266),
            ...
            case SIG_PKT_MSG_RESP_ACK:
                ...
                break;
            default:
                print("V:", LA_F("Received UNKNOWN pkt type: 0x%02X", LA_F175, 271), hdr.type);
                printf(LA_F("Received UNKNOWN pkt from %s:%d, type=0x%02X, seq=%u, len=%d", LA_F174, 270),
    ...
    if (s->state == P2P_STATE_REGISTERING && s->nat.state == NAT_PUNCHING) {
        print("I: %s", LA_S("P2P punching in progress ...", LA_S55, 76));
    ...
    if ((s->state == P2P_STATE_PUNCHING || s->state == P2P_STATE_REGISTERING) &&
        ...
        print("I: %s", LA_S("P2P connection established", LA_S53, 74));
    ...
    if (s->nat.state == NAT_CONNECTED && s->state == P2P_STATE_RELAY) {
        print("I: %s", LA_S("NAT connection recovered, upgrading from RELAY to CONNECTED", LA_S46, 67));
    ...
    if (s->state == P2P_STATE_PUNCHING && s->nat.state == NAT_RELAY) {
        ...
        // 添加中继路径
        print("I: %s", LA_S("P2P punch failed, adding relay path", LA_S54, 75));
        ...
        if (relay_available) {
            ...
            print("I: %s", LA_S("Added SIGNALING path to path manager", LA_S13, 37));
        }
        ...
        if (best_path >= -1) {  // -1=SIGNALING, >=0=候选
            ...
            if (addr) {
                ...
                print("I: %s", LA_S("Using path: RELAY", LA_S76, 95));
            }
        } else {
            // 无可用路径：降级到传统方式
            s->state = P2P_STATE_RELAY;
    ...
    if (s->nat.state == NAT_CLOSED && 
        ...
        print("I: %s", LA_S("Received FIN packet, connection closed", LA_S62, 83));
    ...
    if (s->nat.state == NAT_LOST && s->state == P2P_STATE_CONNECTED) {
        print("W: %s", LA_S("NAT connection timeout, downgrading to relay mode", LA_S47, 68));
        ...
        if (best_path >= -1) {  // -1=SIGNALING, >=0=候选
            ...
            if (addr) {
                ...
                s->state = P2P_STATE_RELAY;
                print("I: %s", LA_S("Switched to backup path: RELAY", LA_S75, 94));
            }
        } else {
            // 无备用路径：NAT 层会继续尝试恢复
            s->state = P2P_STATE_RELAY;
            ...
            if (ready) {
                ...
                if (n > 0) {
                    ...
                    if (sent > 0) {
                        ...
                        print("W:", LA_F("transport send_data failed, %d bytes dropped", LA_F243, 590), n);
                    }
                }
            }
        }
            ...
            if (best_path >= -1 && best_path != current_path) {
                ...
                if (!old_stats ||                                                                                   // 当前无路径，立即切换
                    ...
                    if (ret == 0) {
                        ...
                        print("V: %s", LA_S("Path switch debounced, waiting for stability", LA_S57, 78));
                    }
                }
            }
        }
