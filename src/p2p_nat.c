...
static void nat_send_punch(p2p_session_t *s, const char *reason,
    ...
    print("V:", LA_F("%s sent to %s:%d for %s, seq=%d, path=%d", LA_F115, 115),
...
ret_t nat_punch(p2p_session_t *s, int idx) {
    ...
    if (idx == -1) {
        ...
        if (s->remote_cand_cnt == 0) {
            print("E:", LA_F("%s: no remote candidates to punch", LA_F173, 173), TASK_NAT);
            return E_NONE_EXISTS;
        }

        // 已连接时忽略（避免破坏已建立的连接）
        if (n->state == NAT_CONNECTED) {
            print("V:", LA_F("%s: already connected, ignoring batch punch request", LA_F144, 144), TASK_NAT);
            return E_NONE;
        }
        ...
        print("I:", LA_F("%s: start punching all(%d) remote candidates", LA_F189, 189), TASK_NAT, s->remote_cand_cnt);
        ...
        if (p2p_get_log_level() == P2P_LOG_LEVEL_VERBOSE) {
            ...
            for (int i = 0; i < s->remote_cand_cnt; i++) {
                ...
                print("V:", LA_F("  [%d]<%s:%d> (type: %s)", LA_F106, 106), i,
    ...
    if (idx < 0 || idx >= s->remote_cand_cnt) {
        print("E:", LA_F("%s: invalid cand idx: %d (count: %d)", LA_F167, 167), TASK_NAT, idx, s->remote_cand_cnt);
    ...
    if (n->state == NAT_CONNECTED) {
        ...
        print("I:", LA_F("%s: punching additional cand<%s:%d>[%d] while connected", LA_F175, 175), TASK_NAT,
              inet_ntoa(entry->cand.addr.sin_addr), ntohs(entry->cand.addr.sin_port), idx);
    }
    else {
        print("I:", LA_F("%s: punching remote cand<%s:%d>[%d]", LA_F176, 176), TASK_NAT,
              inet_ntoa(entry->cand.addr.sin_addr), ntohs(entry->cand.addr.sin_port), idx);
...
void nat_send_fin(p2p_session_t *s) {
    ...
    print("V:", LA_F("%s sent to %s:%d", LA_F114, 114),
...
void nat_on_punch(p2p_session_t *s, const p2p_packet_hdr_t *hdr,
    ...
    printf(LA_F("Recv %s pkt from %s:%d seq=%u", LA_F269, 269),
    ...
    */
    {
        ...
        print("V:", LA_F("%s sent to %s:%d, echo_seq=%u", LA_F116, 116),
    ...
    print("V:", LA_F("%s: accepted from cand[%d]", LA_F136, 136), PROTO, cand_idx);

    // 更新最后接收时间（保活/心跳超时检测）
    n->last_recv_time = now;

    if (cand_idx >= remote_cnt_before) {
        print("I:", LA_F("%s: discovered unsynced prflx cand<%s:%d>[%d]", LA_F149, 149),
    ...
    if (n->state != NAT_PUNCHING && n->state != NAT_RELAY && n->state != NAT_LOST) {
        print("W:", LA_F("%s: ignored in state(%d)", LA_F164, 164), TASK_NAT, n->state);
        return;
    }
    ...
    if (!n->rx_confirmed) {
        n->rx_confirmed = true;
        print("I:", LA_F("%s: rx confirmed: peer->me path is UP (%s:%d)", LA_F184, 184),
                TASK_NAT, inet_ntoa(from->sin_addr), ntohs(from->sin_port));
    }
    ...
    if (n->rx_confirmed && n->tx_confirmed) {
        ...
        print("I:", LA_F("%s: bidirectional confirmed: NAT_CONNECTED (%s:%d)", LA_F147, 147),
                TASK_NAT, inet_ntoa(from->sin_addr), ntohs(from->sin_port));
    }
}
...
void nat_on_punch_ack(p2p_session_t *s, const p2p_packet_hdr_t *hdr,
    ...
    printf(LA_F("Recv %s pkt from %s:%d echo_seq=%u", LA_F268, 268),
          PROTO, inet_ntoa(from->sin_addr), ntohs(from->sin_port), hdr->seq);
    ...
    if (!n->tx_confirmed) {
        n->tx_confirmed = true;
        print("I:", LA_F("%s: tx confirmed: me->peer path is UP (echoed seq=%u)", LA_F199, 199),
                TASK_NAT, hdr->seq);
    }
...
void nat_on_fin(p2p_session_t *s, const struct sockaddr_in *from) {
    ...
    if (s->nat.state != NAT_CLOSED) {
        ...
        print("I:", LA_F("%s: received FIN from peer, marking NAT as CLOSED", LA_F177, 177), TASK_NAT);
    }
}
        ...
        case NAT_CONNECTED:
            ...
            if (now_ms - n->last_send_time >= PING_INTERVAL_MS) {
                ...
                if (alive_cnt) {
                    ...
                    print("V:", LA_F("%s: keep alive to %d reachable cand(s)", LA_F171, 171), TASK_NAT, alive_cnt);
                }
            }
            break;
