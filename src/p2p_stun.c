...
void p2p_stun_handle_packet(struct p2p_session *s, const uint8_t *buf, int len,
    ...
    case NAT_TEST_I_SENT: {
        ...
        if (get_local_address(s->sock, &local) == 0) {
            if (addr_equal(&mapped, &local)) {
                ...
                print("I:", LA_F("%s %s", LA_F7, 213), LA_W("Detection completed", LA_W18, 19), p2p_nat_type_str(ctx->detected_type));
                return;
            }
        }
        ...
        print("I:", LA_F("%s %s %s: %s:%d", LA_F15, 221), LA_W("Test", LA_W86, 87), "I:", LA_W("Mapped address", LA_W39, 40),
    ...
    case NAT_TEST_II_SENT: {
        ...
        ctx->state = NAT_TEST_COMPLETED;
        s->nat_type = (int)ctx->detected_type;
        print("I:", LA_F("%s %s %s! %s %s", LA_F14, 220), LA_W("Test", LA_W86, 87), "II:", LA_W("Success", LA_W84, 85),
                     LA_S("Detection completed", LA_S20, 124), p2p_nat_type_str(ctx->detected_type));
        break;
    }
    ...
    case NAT_TEST_III_SENT: {
        ...
        ctx->state = NAT_TEST_COMPLETED;
        s->nat_type = (int)ctx->detected_type;
        print("I:", LA_F("%s %s %s! %s %s", LA_F14, 220), LA_W("Test", LA_W86, 87), "III:", LA_W("Success", LA_W84, 85),
                     LA_S("Detection completed", LA_S20, 124), p2p_nat_type_str(ctx->detected_type));
        break;
    }
...
void p2p_stun_nat_detect_tick(struct p2p_session *s) {
    ...
    if (ctx->state != NAT_TEST_IDLE && 
        ...
        if (ctx->retry_count >= STUN_MAX_RETRIES) {
            ...
            switch (ctx->state) {
            case NAT_TEST_I_SENT:
                print("W:", LA_F("%s %s %s", LA_F9, 215), LA_W("Test", LA_W86, 87), "I:", LA_W("Timeout", LA_W88, 89));
            ...
            case NAT_TEST_II_SENT:
                print("W:", LA_F("%s %s %s (%s %s %s)", LA_F12, 218), LA_W("Test", LA_W86, 87), "II:", LA_W("Timeout", LA_W88, 89),
                             LA_S("need", LA_S43, 147), LA_S("Test", LA_S97, 201), "III");
                ctx->test_ii_success = 0;
                ctx->state = NAT_TEST_II_DONE;
                break;
            
            case NAT_TEST_III_SENT:
                print("W:", LA_F("%s %s %s", LA_F9, 215), LA_W("Test", LA_W86, 87), "III:", LA_W("Timeout", LA_W88, 89));
    ...
    case NAT_TEST_IDLE: {
        ...
        if (resolve_host(s->cfg.stun_server, s->cfg.stun_port, &stun_addr) < 0) {
            print("E:", LA_F("%s %s %s", LA_F9, 215), LA_W("Failed to resolve", LA_W22, 23), LA_S("STUN server", LA_S96, 200), s->cfg.stun_server);
            ctx->state = NAT_TEST_COMPLETED;
            return;
        }
        ...
        */
        ...
        if (len > 0) {
            ...
            */
            ...
            print("I:", LA_F("%s %s %s %s %s:%d (%s=%d)", LA_F11, 217), LA_W("Sending", LA_W75, 76), LA_S("Test", LA_S97, 201), "I",
                         LA_W("to", LA_W90, 91), s->cfg.stun_server, s->cfg.stun_port, LA_W("len", LA_W37, 38), len);
        } else {
            print("E: %s", LA_S("Failed to build STUN request", LA_S27, 131));
        }
        break;
    }
    ...
    case NAT_TEST_III_DONE: {
        ...
        ctx->state = NAT_TEST_COMPLETED;
        s->nat_type = (int)ctx->detected_type;
        print("I:", LA_F("%s %s", LA_F7, 213), LA_W("Detection completed", LA_W18, 19),
