...
ret_t route_detect_local(route_ctx_t *rt) {
    ...
    print("I:", LA_F("%s: %d %s", LA_F48, 309), LA_W("Local address detection done", LA_W41, 53), rt->addr_count, LA_W("address(es)", LA_W5, 7));
...
bool route_check_same_subnet(route_ctx_t *rt, const struct sockaddr_in *peer_priv) {
    ...
    for (int i = 0; i < rt->addr_count; i++) {
        ...
        if ((local_ip & mask) == (peer_ip & mask)) {
            print("I:", LA_F("Peer IP %s matches local network segment %s (connectivity pending probe)", LA_F104, 267),
                  inet_ntoa(peer_priv->sin_addr), inet_ntoa(rt->local_addrs[i].sin_addr));
            return true;
        }
    }

    print("I:", LA_F("Peer IP %s does not match any local network segment", LA_F103, 311), inet_ntoa(peer_priv->sin_addr));
    return false;
}
...
ret_t route_send_probe(route_ctx_t *rt, sock_t sock, const struct sockaddr_in *peer_priv, uint16_t local_port) {
    ...
    uint8_t payload[2];
    nwrite_s(payload, local_port);

    print("V:", LA_F("Sent route probe to %s:%d", LA_F151, 276),
                 inet_ntoa(peer_priv->sin_addr), ntohs(peer_priv->sin_port));

    printf(LA_F("Send ROUTE_PROBE pkt to %s:%d, seq=0, flags=0, len=2", LA_F142, 904),
           inet_ntoa(peer_priv->sin_addr), ntohs(peer_priv->sin_port));
 ...
 */
ret_t route_on_probe(route_ctx_t *rt, const struct sockaddr_in *from, sock_t sock) { (void)rt;

    printf(LA_F("Received ROUTE_PROBE pkt from %s:%d, seq=0, flags=0, len=2", LA_F128, 146),
           inet_ntoa(from->sin_addr), ntohs(from->sin_port));

    printf(LA_F("Send ROUTE_PROBE_ACK pkt to %s:%d, seq=0, flags=0, len=0", LA_F143, 905),
           inet_ntoa(from->sin_addr), ntohs(from->sin_port));
...
ret_t route_on_probe_ack(route_ctx_t *rt, const struct sockaddr_in *from) {
    ...
    printf(LA_F("Received ROUTE_PROBE_ACK pkt from %s:%d, seq=0, flags=0, len=0", LA_F129, 147),
           inet_ntoa(from->sin_addr), ntohs(from->sin_port));

    rt->lan_peer_addr = *from;
    rt->lan_confirmed = 1;
    print("I:", LA_F("LAN peer confirmed %s:%d", LA_F89, 276),
                 inet_ntoa(from->sin_addr), ntohs(from->sin_port));
    return 0;
}
