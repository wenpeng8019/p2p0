...
static int openssl_init(struct p2p_session *s) {
    ...
    if (!os) {
        print("E:", LA_F("Failed to allocate OpenSSL context", LA_F303, 303));
        return -1;
    }
    s->dtls_data = os;
    ...
    if (!os->ctx) {
        print("E:", LA_F("SSL_CTX_new failed", LA_F399, 399));
    ...
    if (!os->ssl) {
        print("E:", LA_F("SSL_new failed", LA_F400, 400));
    ...
    if (!os->read_bio || !os->write_bio) {
        print("E:", LA_F("BIO_new failed", LA_F281, 281));
    ...
    print("I:", LA_F("[OpenSSL] DTLS role: %s (mode=%s)", LA_F465, 465),
...
static void openssl_tick(struct p2p_session *s) {
    ...
    if (!os->handshake_done) {
        ...
        if (ret == 1) {
            os->handshake_done = 1;
            print("I:", LA_F("[OpenSSL] DTLS handshake completed", LA_F464, 464));
        }
    }
