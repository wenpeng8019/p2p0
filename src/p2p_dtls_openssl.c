...
static int openssl_init(p2p_session_t *s) {
    ...
    if (!os) {
        print("E: %s", LA_S("Failed to allocate OpenSSL context", LA_S60, 60));
        return -1;
    }
    s->dtls_data = os;
    ...
    if (!os->ctx) {
        print("E: %s", LA_S("SSL_CTX_new failed", LA_S95, 95));
    ...
    if (!os->ssl) {
        print("E: %s", LA_S("SSL_new failed", LA_S96, 96));
    ...
    if (!os->read_bio || !os->write_bio) {
        print("E: %s", LA_S("BIO_new failed", LA_S43, 43));
    ...
    print("I:", LA_F("[OpenSSL] DTLS role: %s (mode=%s)", LA_F319, 319),
