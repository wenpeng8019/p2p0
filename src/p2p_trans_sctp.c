                ...
                if ((size_t)n >= sizeof(notif->sn_header)) {
                    ...
                    case SCTP_ASSOC_CHANGE: {
                        ...
                        if (sac->sac_state == SCTP_COMM_UP) {
                            ctx->state = 2;
                            print("I:", LA_S("[SCTP] association established", LA_S557, 557));
                        } else if (sac->sac_state == SCTP_COMM_LOST ||
                                   sac->sac_state == SCTP_SHUTDOWN_COMP ||
                                   sac->sac_state == SCTP_CANT_STR_ASSOC) {
                            ctx->state = 0;
                            print("W:", LA_F("[SCTP] association lost/shutdown (state=%u)", LA_F584, 584),
                                  sac->sac_state);
                        }
                        break;
                    }
...
static int sctp_init(p2p_session_t *s) {
    ...
    if (!sock) {
        print("E:", LA_S("[SCTP] usrsctp_socket failed", LA_S559, 559));
        goto fail;
    }
    ctx->sock = sock;
    ...
    if (usrsctp_bind(sock, (struct sockaddr *)&sconn, sizeof(sconn)) < 0) {
        print("E:", LA_F("[SCTP] bind failed: %s", LA_F585, 585), strerror(errno));
        goto fail;
    }
    ...
    if (ret < 0 && errno != EINPROGRESS) {
        print("E:", LA_F("[SCTP] connect failed: %s", LA_F586, 586), strerror(errno));
        goto fail;
    }

    ctx->last_tick = P_tick_ms();
    print("I:", LA_S("[SCTP] usrsctp initialized, connecting...", LA_S558, 558));
    return 0;
...
static int sctp_send(p2p_session_t *s, const void *buf, int len) {
    ...
    if (sent < 0) {
        ...
        print("W:", LA_F("[SCTP] sendv failed: %s", LA_F587, 587), strerror(errno));
        return -1;
    }
    return (int)sent;
}
