...
int p2p_send_packet(struct p2p_session *s, const sockAddr_t *addr,
    ...
    if (s->path_type == P2P_PATH_RELAY) {
        if (s->inst->turn.state != TURN_ALLOCATED) {
            print("E:", LA_F("RELAY path but TURN not allocated", LA_F371, 371));
            return -1;
        }
    ...
    if (s->path_type == P2P_PATH_SIGNALING) {
        if (!s->inst->signaling_relay_fn) {
            print("E:", LA_F("SIGNALING path but signaling relay not available", LA_F397, 397));
            return -1;
        }
...
void p2p_send_dtls_record(struct p2p_session *s, const sockAddr_t *addr,
    ...
    if (s->path_type == P2P_PATH_RELAY) {
        if (s->inst->turn.state != TURN_ALLOCATED) {
            print("W:", LA_F("RELAY path but TURN not allocated (dtls)", LA_F372, 372));
            return;
        }
