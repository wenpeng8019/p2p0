...
void probe_trigger(struct p2p_session *s) {
    ...
    switch (s->signaling_mode) {
        ...
        case P2P_SIGNALING_MODE_COMPACT:
            ...
            ctx->retries = 0;
            print("I:", LA_F("%s: triggered via COMPACT msg echo", LA_F144, 167), TASK_RELAY_PROBE);
            break;
        ...
        case P2P_SIGNALING_MODE_RELAY:
            ...
            ctx->retries = 0;
            print("I:", LA_F("%s: triggered via RELAY TUNE echo", LA_F145, 171), TASK_RELAY_PROBE);
            break;
...
static void probe_compact_tick(struct p2p_session *s, uint64_t now_ms) {
    ...
    if (ctx->state != P2P_PROBE_STATE_RUNNING) {
        ...
        if (ctx->state == P2P_PROBE_STATE_SUCCESS ||
            ...
            if (now_ms - ctx->complete_ms >= PROBE_COMPACT_REPEAT_INTERVAL) {
                print("V:", LA_F("%s: restarting periodic check", LA_F120, 163), TASK_RELAY_PROBE);
    ...
    switch (ctx->mode.compact.phase) {
        ...
        case PROBE_COMPACT_PHASE_SENDING: {
            ...
            print("I:", LA_F("%s: sent MSG(msg=0, sid=%u)", LA_F127, 413), TASK_RELAY_PROBE, ctx->mode.compact.sid);
            ...
            if (ret != E_NONE) {
                ...
                ctx->start_ms           = now_ms;
            } else { print("W:", LA_F("%s: send failed(%d)", LA_F126, 412), TASK_RELAY_PROBE, ret);
                ctx->state = P2P_PROBE_STATE_READY;
                ctx->mode.compact.phase = PROBE_COMPACT_PHASE_INIT;
            }
        } break;
        ...
        case PROBE_COMPACT_PHASE_WAIT_ECHO:
            ...
            if (now_ms - ctx->start_ms >= PROBE_COMPACT_TIMEOUT_MS) {
                if (ctx->retries < PROBE_COMPACT_MAX_RETRIES) {
                    ctx->retries++;
                    ctx->mode.compact.phase = PROBE_COMPACT_PHASE_SENDING;
                    print("W:", LA_F("%s: timeout, retry %d/%d", LA_F143, 414), TASK_RELAY_PROBE,
                          ctx->retries, PROBE_COMPACT_MAX_RETRIES);
                } else {
                    ctx->state = P2P_PROBE_STATE_PEER_TIMEOUT;
                    ctx->complete_ms = now_ms;
                    print("W:", LA_F("%s: timeout, peer did not respond", LA_F142, 166), TASK_RELAY_PROBE);
                }
            }
            break;
    ...
    switch (ctx->mode.relay.step) {
        ...
        case PROBE_RELAY_STEP_TURN_ALLOC:
            // 步骤1：重新分配 TURN 地址
            {
                ...
                if (ret == E_NONE) {
                    ctx->start_ms = now_ms;
                    print("I:", LA_F("%s: TURN allocation request sent", LA_F65, 177), TASK_RELAY_PROBE);
                    // 保持 TURN_ALLOC 状态，等待回调
                } else {
                    print("W:", LA_F("%s: TURN allocation failed: ret=%d", LA_F64, 416), TASK_RELAY_PROBE, ret);
                    ctx->state = P2P_PROBE_STATE_TIMEOUT;
                    ctx->complete_ms = now_ms;
                }
            }
        ...
        case PROBE_RELAY_STEP_ADDR_EXCHANGE:
            ...
            if (now_ms - ctx->start_ms >= PROBE_RELAY_EXCHANGE_TIMEOUT_MS) {
                if (ctx->retries < PROBE_RELAY_MAX_RETRIES) {
                    ctx->retries++;
                    print("W:", LA_F("%s: exchange timeout, retry %d/%d", LA_F90, 419), TASK_RELAY_PROBE,
                          ctx->retries, PROBE_RELAY_MAX_RETRIES);
                    // TODO: 重新发送地址交换请求
                } else {
                    ctx->state = P2P_PROBE_STATE_PEER_TIMEOUT;
                    ctx->complete_ms = now_ms;
                    print("W:", LA_F("%s: exchange timeout: peer not responding", LA_F91, 172), TASK_RELAY_PROBE);
                }
            }
            break;
        ...
        case PROBE_RELAY_STEP_UDP_PROBE:
            ...
            if (now_ms - ctx->start_ms >= PROBE_RELAY_UDP_TIMEOUT_MS) {
                if (ctx->retries < PROBE_RELAY_MAX_RETRIES) {
                    ctx->retries++;
                    print("W:", LA_F("%s: UDP timeout, retry %d/%d", LA_F66, 418), TASK_RELAY_PROBE,
                          ctx->retries, PROBE_RELAY_MAX_RETRIES);
                    // TODO: 重新发送 UDP 探测包
                } else {
                    ctx->state = P2P_PROBE_STATE_PEER_TIMEOUT;
                    ctx->complete_ms = now_ms;
                    print("W:", LA_F("%s: UDP timeout: peer not responding", LA_F67, 180), TASK_RELAY_PROBE);
                }
            }
            break;
