...
static int p2p_ice_form_check_list(
    ...
    if (pair_cnt > 5) {
        print("I:", LA_F("  ... %s %d %s", LA_F0, 256), LA_W("and", LA_W6, 8), pair_cnt - 5, LA_W("more pairs", LA_W44, 56));
    }
...
int p2p_ice_send_local_candidate(p2p_session_t *s, p2p_candidate_entry_t *c) {
    ...
    if (ret < 0) {
        print("W:", LA_F("%s (ret=%d), %s", LA_F31, 291),
               LA_W("[Trickle] TCP send failed", LA_W1, 2), ret,
               LA_W("will be retried by p2p_update()", LA_W115, 147));
        return -1;
    }

    /* 发送成功（无论对端在线与否）*/
    print("I:", LA_F("%s %s %s (%s=%s)", LA_F13, 270), LA_W("[Trickle] Sent", LA_W0, 1), LA_S("1 candidate to", LA_S0, 151), s->remote_peer_id,
           LA_W("online", LA_W53, 65), ret > 0 ? LA_W("yes", LA_W117, 150) : LA_W("no (cached)", LA_W51, 63));
    return ret > 0 ? ret : 0;
}
...
void p2p_ice_on_remote_candidates(p2p_session_t *s, const uint8_t *payload, int len) {
    ...
    while (offset + 7 <= len) {
        ...
        if (!exists) {
            ...
            print("I:", LA_F("%s: %d -> %s:%d", LA_F50, 310), LA_W("Received New Remote Candidate", LA_W73, 91),
...
void p2p_ice_on_check_success(p2p_session_t *s, const struct sockaddr_in *from) {
    ...
    if (s->sig_relay_ctx.incoming_peer_name[0] != '\0') {
        ...
        if (answer_len > 0) {
            ...
            print("I:", LA_F("%s '%s'", LA_F24, 282), LA_W("Sent answer to", LA_W89, 114), s->sig_relay_ctx.incoming_peer_name);
        }
    }
}
...
void p2p_ice_tick(p2p_session_t *s, uint64_t now_ms) {
    ...
    if (s->ice_check_count == 1) {
        print("I:", LA_F("%s (%d %s)", LA_F30, 288),
               LA_W("UDP hole-punch probing remote candidates", LA_W106, 137),
               s->remote_cand_cnt, "candidates");
    } else if (s->ice_check_count % 2 == 0) {
        printf(LA_F("%s round %d/%d", LA_F39, 299),
               LA_W("UDP hole-punch probing remote candidates", LA_W106, 137),
