...
static int p2p_ice_form_check_list(
    ...
    print("I:", LA_F("Formed check list with %d candidate pairs", LA_F142, 238), pair_cnt);
    for (int i = 0; i < pair_cnt && i < 5; i++) {  /* 只打印前 5 个 */
        print("I:", LA_F("  [%d] L=%s:%d -> R=%s:%d, pri=0x%016llx", LA_F2, 98),
               i,
    ...
    if (pair_cnt > 5) {
        print("I:", LA_F("  ... and %d more pairs", LA_F0, 96), pair_cnt - 5);
    }
...
int p2p_ice_gather_candidates(p2p_session_t *s) {
        ...
        if (pAddrs && ret == NO_ERROR) {
            for (PIP_ADAPTER_ADDRESSES a = pAddrs; a != NULL; a = a->Next) {
                ...
                for (PIP_ADAPTER_UNICAST_ADDRESS ua = a->FirstUnicastAddress;
                    ...
                    print("I:", LA_F("Gathered Host Candidate: %s:%d (priority=0x%08x)",LA_F143, 239
    ...
    if (!s->cfg.lan_punch && s->cfg.stun_server) {
        ...
        if (slen > 0) {
            ...
            if (he) {
                ...
                print("I:", LA_F("Requested Srflx Candidate from %s", LA_F188, 284), s->cfg.stun_server);
            }
        }
    }
    ...
    if (!s->cfg.lan_punch && s->cfg.turn_server) {
        if (p2p_turn_allocate(s) == 0) {
            print("I:", LA_F("Requested Relay Candidate from %s", LA_F187, 283), s->cfg.turn_server);
        }
    }
...
void p2p_ice_on_remote_candidates(p2p_session_t *s, const uint8_t *payload, int len) {
    ...
    while (offset + 7 <= len) {
        ...
        if (idx >= 0) {
            ...
            print("I:", LA_F("Received New Remote Candidate: %d -> %s:%d", LA_F172, 268),
...
void p2p_ice_on_check_success(p2p_session_t *s, const struct sockaddr_in *from) {
    ...
    print("I:", LA_F("Nomination successful! Using! Using %s path %s:%d%s", LA_F153, 249),
    ...
    if (s->sig_relay_ctx.incoming_peer_name[0] != '\0') {
        ...
        if (answer_len > 0) {
            ...
            print("I:", LA_F("Sent answer to '%s'", LA_F201, 297), s->sig_relay_ctx.incoming_peer_name);
        }
    }
}
...
void p2p_ice_tick(p2p_session_t *s, uint64_t now_ms) {
    ...
    if (s->ice_check_count >= P2P_ICE_MAX_CHECKS) {
        print("W:", LA_F("Connectivity checks timed out (sent %d rounds), giving up", LA_F128, 224), s->ice_check_count);
