...
static int p2p_ice_form_check_list(
    ...
    print("I:", LA_F("%s %d %s", LA_F5, 261), LA_W("Formed check list with", LA_W28, 38), pair_cnt, LA_W("candidate pairs", LA_W13, 20));
    for (int i = 0; i < pair_cnt && i < 5; i++) {  /* 只打印前 5 个 */
        print("I:", LA_F("  [%d] L=%s:%d -> R=%s:%d, pri=0x%016llx", LA_F2, 259),
               i,
    ...
    if (pair_cnt > 5) {
        print("I:", LA_F("  ... %s %d %s", LA_F0, 256), LA_W("and", LA_W6, 8), pair_cnt - 5, LA_W("more pairs", LA_W43, 56));
    }
...
int p2p_ice_send_local_candidate(p2p_session_t *s, p2p_candidate_entry_t *c) {
    ...
    if (s->signaling_mode != P2P_SIGNALING_MODE_RELAY) {
        ...
        print("E:", LA_S("Error: p2p_ice_send_local_candidate called in non-RELAY mode", LA_S22, 172));
        return -1;
    }
    ...
    if (s->sig_relay_ctx.state != SIGNAL_CONNECTED) {
        print("W: %s", LA_S("[Trickle] TCP not connected, skipping single candidate send", LA_S4, 155));
        return -1;
    }
    ...
    if (ret < 0) {
        print("W:", LA_F("%s (ret=%d), %s", LA_F30, 291),
               LA_W("[Trickle] TCP send failed", LA_W1, 2), ret,
               LA_W("will be retried by p2p_update()", LA_W110, 147));
        return -1;
    }

    /* 发送成功（无论对端在线与否）*/
    print("I:", LA_F("%s %s %s (%s=%s)", LA_F13, 270), LA_W("[Trickle] Sent", LA_W0, 1), LA_S("1 candidate to", LA_S0, 151), s->remote_peer_id,
           LA_W("online", LA_W52, 65), ret > 0 ? LA_W("yes", LA_W112, 150) : LA_W("no (cached)", LA_W50, 63));
    return ret > 0 ? ret : 0;
}
...
int p2p_ice_gather_candidates(p2p_session_t *s) {
        ...
        if (pAddrs && ret == NO_ERROR) {
            for (PIP_ADAPTER_ADDRESSES a = pAddrs; a != NULL; a = a->Next) {
                ...
                for (PIP_ADAPTER_UNICAST_ADDRESS ua = a->FirstUnicastAddress;
                    ...
                    print("I:", LA_F("%s %s: %s:%d (priority=0x%08x)",LA_F18, 275
                           LA_W("Gathered", LA_W32, 42), LA_S("Host Candidate", LA_S36, 183),
    ...
    if (!s->cfg.lan_punch && s->cfg.stun_server) {
        ...
        if (slen > 0) {
            ...
            if (he) {
                ...
                print("I:", LA_F("%s %s %s %s", LA_F10, 267), LA_W("Requested", LA_W76, 102), LA_S("Srflx Candidate", LA_S85, 206), LA_S("from", LA_S33, 181), s->cfg.stun_server);
            }
        }
    }
    ...
    if (!s->cfg.lan_punch && s->cfg.turn_server) {
        if (p2p_turn_allocate(s) == 0) {
            print("I:", LA_F("%s %s %s %s", LA_F10, 267), LA_W("Requested", LA_W76, 102), LA_S("Relay Candidate", LA_S71, 198), LA_S("from", LA_S33, 181), s->cfg.turn_server);
        }
    }
...
void p2p_ice_on_remote_candidates(p2p_session_t *s, const uint8_t *payload, int len) {
    ...
    while (offset + 7 <= len) {
        ...
        if (!exists) {
            ...
            print("I:", LA_F("%s: %d -> %s:%d", LA_F49, 310), LA_W("Received New Remote Candidate", LA_W70, 91),
...
void p2p_ice_on_check_success(p2p_session_t *s, const struct sockaddr_in *from) {
    ...
    if (matched_idx < 0) {
        ...
        if (idx >= 0) {
            ...
            print("I:", LA_F("[prflx] %s %s:%d (Peer Reflexive - symmetric NAT)", LA_F172, 366),
                         LA_W("Received New Remote Candidate", LA_W70, 91),
    ...
    print("I:", LA_F("%s! %s %s %s %s:%d%s", LA_F45, 306),
            LA_W("Nomination successful! Using", LA_W51, 64), LA_S("Using", LA_S93, 212), cand_type_str, LA_W("path", LA_W58, 71),
            inet_ntoa(from->sin_addr), ntohs(from->sin_port),
            connection_desc);
    ...
    if (s->sig_relay_ctx.incoming_peer_name[0] != '\0') {
        ...
        if (answer_len > 0) {
            ...
            print("I:", LA_F("%s '%s'", LA_F23, 282), LA_W("Sent answer to", LA_W85, 114), s->sig_relay_ctx.incoming_peer_name);
        }
    }
}
...
void p2p_ice_tick(p2p_session_t *s, uint64_t now_ms) {
    ...
    if (s->cfg.lan_punch) {
        if (s->nat.state == NAT_INIT) {
            print("I:", LA_F("[lan_punch] 启动 PUNCH 流程 (Host 候选 %d 个)", LA_F171, 365),
                         s->remote_cand_cnt);
            nat_punch(s, -1);
        }
    ...
    if (s->ice_check_count >= P2P_ICE_MAX_CHECKS) {
        print("W:", LA_F("连通性检查超时（已发送 %d 轮），放弃", LA_F178, 371), s->ice_check_count);
    ...
    if (s->ice_check_count == 1) {
        print("I:", LA_F("%s (%d %s)", LA_F29, 288),
               LA_W("UDP hole-punch probing remote candidates", LA_W101, 137),
               s->remote_cand_cnt, "candidates");
    } else if (s->ice_check_count % 2 == 0) {
        printf(LA_F("%s round %d/%d", LA_F38, 299),
               LA_W("UDP hole-punch probing remote candidates", LA_W101, 137),
