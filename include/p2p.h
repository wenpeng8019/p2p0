 ...
 *              msg=P2P_RPC_ERR_PEER_OFF (0xFF): B 在等待响应期间离线
 *              msg=P2P_RPC_ERR_TIMEOUT (0xFE): 服务器向 B 转发请求超时
 */
typedef void (*p2p_on_response_fn)(p2p_session_t session, uint16_t sid,
                                   uint8_t msg, const void *data, int len,
                                   void *userdata);
#define P2P_RPC_ERR_PEER_OFF        0xFF    // B端在等待响应期间离线（区别于 REQ_ACK 时已知离线）
#define P2P_RPC_ERR_TIMEOUT         0xFE    // 服务器转发请求超时
#define P2P_RPC_ERR_BREAK           0xFD    // 中断执行
