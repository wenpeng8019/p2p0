...
enum {
    P2P_CONNECT_MODE_SIMPLE = 0,                /* 简单无状态信令（UDP，无登录） */
    P2P_CONNECT_MODE_ICE,                       /* ICE 有状态信令（TCP，需登录） */
    P2P_CONNECT_MODE_PUBSUB                     /* 发布/订阅模式（Gist/KV 存储，角色由 remote_peer_id 决定） */
};
...
typedef struct {
    ...
    /* 信令配置 */
    int         signaling_mode;                 /* P2P_CONNECT_MODE_* (连接时使用的信令模式) */
    const char* gh_token;                       /* GitHub Token (用于 Gist API) */
    const char* gist_id;                        /* Gist ID (用于 PUB/SUB 模式) */
    
    /* 协议选择 */
    int         use_ice;                        /* 0 = SIMPLE (私有协议), 1 = ICE (RFC 5245) */
    
    /* STUN/TURN 配置 (仅当 use_ice=1 或需要高级诊断时使用) */
    ...
    /* TCP 选项 */
    int         enable_tcp;                     /* 是否尝试 TCP 打洞/回退 */
    uint16_t    tcp_port;                       /* TCP 监听端口 (0 = any) */

    /* 传输质量 */
    int         use_pseudotcp;                  /* 1 = 启用拥塞控制 (AIMD) */
    int         use_dtls;                       /* 1 = 启用 MbedTLS DTLS */
    int         use_openssl;                    /* 1 = 启用 OpenSSL DTLS */
    int         use_sctp;                       /* 1 = 启用 usrsctp (SCTP) */
    int         dtls_server;                    /* 1 = 作为 DTLS 服务端 (握手被动方) */

    /* 其他选项 */
    int         threaded;                       /* 0 = 手动更新, 1 = 内部线程 */
    int         update_interval_ms;             /* 内部线程更新间隔 (默认 10) */
    int         nagle;                          /* 1 = 启用 Nagle 批处理 (默认 0) */
 ...
 *    - 原理：双方各自向服务器注册 pair<local_id, remote_id> 映射，
 *            服务器匹配成功后返回对方地址，开始 NAT 打洞
 ...
 *      * NULL: 被动等待，接受来自任意对等方的连接（等待 offer）
 *    - 原理：通过 TCP 长连接登录信令服务器，服务器转发 ICE 候选者，
 *            支持一对多场景（服务器可转发多个对等方的消息）
 *    - 配置要求：server_host, server_port
 *    - 示例：
 ...
 *    - 原理：使用 GitHub Gist 或类似的 KV 存储作为信令中介，
 *            发布者写入 offer，订阅者读取并回复 answer
