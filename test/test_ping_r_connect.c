/*
 * test_ping_r_connect.c - P2P Ping 双客户端连接测试 (RELAY 模式)
 *
 * ============================================================================
 * 测试架构
 * ============================================================================
 ...
 *        +-----------+               +-----------+
 *
 * 1. 启动 signaling server（同时支持 COMPACT/RELAY）
 * 2. 启动 Alice (RELAY 模式, --debugger alice, -t bob)
 * 3. 启动 Bob   (RELAY 模式, --debugger bob, -t alice)
 * 4. 通过 instrument 日志观察连接流程
 * 5. 验证连接成功（双方都收到 "P2P connection established"）
 *
 * RELAY 模式：不带 --compact，使用 TCP 长连接信令
 *
 * ============================================================================
 * 同步协议
 * ============================================================================
...
// 启动 ping 客户端（RELAY 模式：不带 --compact）
...
// 测试 1: 基本 P2P 连接（Alice <-> Bob，RELAY 模式）
static void test_basic_connection(void) {
    const char *TEST_NAME = "basic_connection_relay";
    printf("\n--- Test: %s ---\n", TEST_NAME);
    clear_logs();
    
    // 1. 启动 Alice (target=bob)
    printf("[1] Starting Alice (target=bob, relay)...\n");
    ...
    printf("[3] Starting Bob (target=alice, relay)...\n");
...
int main(int argc, char *argv[]) {
    ...
    printf("=== P2P Ping Pair Tests (RELAY Mode) ===\n");
    printf("Ping path:   %s\n", g_ping_path);
