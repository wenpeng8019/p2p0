/*
 * test_ping_w_reconnect.c - P2P WSS 模式重连测试集合
 *
 * ============================================================================
 * 测试场景 (WSS 模式)
 * ============================================================================
...
#define MOD_TAG "TEST_RECONNECT_W"
...
static void on_instrument(uint16_t rid, uint8_t chn, const char* tag, char *txt, int len) {
    ...
    // 连接成功检测 (WSS 模式 - 包括 CONNECTED 和 RELAY 状态)
...
static int start_server(void) {
    ...
    if (g_server_pid < 0) {
        ...
        // WSS 模式: 使用 --ws, -r 和 -m 标志
        execl(g_server_path, g_server_path, "--ws", "-p", port_str, "-r", "-m", NULL);
        perror("exec server");
        _exit(127);
    }

    printf("    Server PID: %d (port=%d, WSS mode)\n", g_server_pid, g_server_port);
    P_usleep(500 * 1000);
    return 0;
}
...
static void test_wss_both_crash(void) {
    const char *TEST_NAME = "wss_both_crash";
    printf("\n--- Test: %s ---\n", TEST_NAME);
    clear_logs();
...
///////////////////////////////////////////////////////////////////////////////
// 测试 2: 单端崩溃重连 + 双向消息验证 (WSS 模式)
//   Alice 停止 → Bob 在线收 FIN → Alice 重启 → 双向消息
///////////////////////////////////////////////////////////////////////////////

static void test_wss_single_crash(void) {
    const char *TEST_NAME = "wss_single_crash";
    printf("\n--- Test: %s ---\n", TEST_NAME);
    clear_logs();
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <ping_path> <server_path> [port] [test_filter]\n", argv[0]);
        fprintf(stderr, "\nExamples:\n");
        fprintf(stderr, "  %s ./p2p_ping ./p2p_server\n", argv[0]);
        fprintf(stderr, "  %s ./p2p_ping ./p2p_server 9434 crash\n", argv[0]);
        return 1;
    }
    ...
    printf("=== P2P Ping Reconnect Tests (WSS Mode) ===\n");
    printf("Ping path:   %s\n", g_ping_path);
...
#define RUN_IF(name, fn) do { \
    ...
    RUN_IF(wss_both_crash,   test_wss_both_crash);
    RUN_IF(wss_single_crash, test_wss_single_crash);
