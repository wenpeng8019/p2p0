/*
 * test_ping_r_template.c - P2P Ping 客户端单元测试模版 (RELAY 模式)
 *
 * ============================================================================
 * 测试架构
 * ============================================================================
 ...
 * 3. 使用 RELAY 模式（TCP 长连接）进行信令和数据传输
 *
 * ============================================================================
 * 同步协议
 * ============================================================================
 *
 *   [Test Client]                    [p2p_ping --relay --debugger test_dbg]
 *        |                                    |
...
#define DEFAULT_SERVER_PORT     9433        // RELAY 模式使用不同端口
...
static int start_server(const char *server_path) {
    ...
    if (g_server_pid < 0) {
        ...
        // 子进程：执行 server（带 -r 参数启用 RELAY 支持）
        execl(server_path, server_path, "-p", port_str, "-r", NULL);
...
// 启动 ping 子进程（RELAY 模式，带 debugger 参数）
static int start_ping(const char *name, const char *target, const char *debugger_name) {
    ...
    if (g_ping_pid < 0) {
        ...
        // 子进程：执行 ping（RELAY 模式：默认，不带 --compact）
        if (target) {
    ...
    printf("    Ping PID: %d (name=%s, debugger=%s, relay mode)\n", g_ping_pid, name, debugger_name);
    return 0;
}
...
// 测试 1: 验证 debugger 同步机制（RELAY 模式）
static void test_debugger_sync(void) {
    const char *TEST_NAME = "debugger_sync_relay";
    printf("\n--- Test: %s ---\n", TEST_NAME);
    clear_logs();
...
int main(int argc, char *argv[]) {
    ...
    printf("=== P2P Ping Client Tests (RELAY Mode) ===\n");
    printf("Ping path:   %s\n", g_ping_path);
    ...
    if (server_path) {
        ...
        printf("[*] Using external server at %s:%d (ensure relay mode enabled)\n", g_server_host, g_server_port);
    }
