...
typedef struct compact_pair {
    ...
    time_t                  info0_sent_time;                    // 当前待确认 seq=0 最近发送时间
...
static void enqueue_info0_pending(compact_pair_t *pair, uint8_t base_index, time_t now) {
...
static bool check_addr_change(sock_t udp_fd, compact_pair_t *pair, const struct sockaddr_in *from) {
    ...
    // 条件：已配对 + 对端已确认首包 → 发送地址变更通知
    if (pair->peer && pair->peer != (compact_pair_t*)(void*)-1 && pair->peer->info0_acked) {
        ...
        enqueue_info0_pending(pair->peer, pair->peer->addr_notify_seq, now);
...
// 发送 REGISTER_ACK: [hdr(4)][status(1)][max_candidates(1)][public_ip(4)][public_port(2)][probe_port(2)] = 14字节
// status: SIG_REGACK_PEER_OFFLINE(0) / SIG_REGACK_PEER_ONLINE(1) / 2(error)
static void send_register_ack(sock_t udp_fd, const struct sockaddr_in *to, const char *to_str, uint8_t status) {
    uint8_t ack[14];
    ...
    if (status <= 1) {
        ...
        ack[4] = status;
        ack[5] = MAX_CANDIDATES;
        memcpy(ack + 6,  &to->sin_addr.s_addr, 4);
        memcpy(ack + 10, &to->sin_port, 2);
        uint16_t probe = htons(g_probe_port);
        memcpy(ack + 12, &probe, 2);
    } else {
        ack[4] = status;
        memset(ack + 5, 0, 9);
    }

    printf("Send REGISTER_ACK pkt to %s, seq=0, flags=0x%02x, status=%u, len=14\n",
           to_str, hdr->flags, ack[4]);
    sendto(udp_fd, (const char *)ack, 14, 0, (const struct sockaddr *)to, sizeof(*to));

    if (status <= 1) {
        printf(LA_F("[UDP] V: REGISTER_ACK sent, status=%s, max_cands=%d, relay=%s, public=%s:%d, probe=%d\n", LA_F75, 75),
               status ? "peer_online" : "peer_offline",
               MAX_CANDIDATES,
               g_relay_enabled ? "yes" : "no",
               inet_ntoa(to->sin_addr), ntohs(to->sin_port), g_probe_port);
    } else {
...
static void retry_info0_pending(sock_t udp_fd, time_t now) {
    ...
    for(;;) {
        ...
        if (now - g_info0_pending_head->info0_sent_time < PEER_INFO0_RETRY_INTERVAL) {
            return;
        }
...
// 将配对加入 REQ 待确认链表
static void enqueue_req_pending(compact_pair_t *pair, time_t now) {
...
// 将配对加入 RESP 待确认链表
static void enqueue_resp_pending(compact_pair_t *pair, time_t now) {
...
static void send_msg_resp_to_requester(sock_t udp_fd, compact_pair_t *pair) {
    ...
    if (!(pair->resp_flags & (SIG_MSG_FLAG_PEER_OFFLINE | SIG_MSG_FLAG_TIMEOUT))) {
        pkt[n++] = pair->resp_msg;
        if (pair->resp_data_len > 0) {
            memcpy(pkt + n, pair->resp_data, pair->resp_data_len);
            n += pair->resp_data_len;
        }
    }
...
static void transition_to_resp_pending(sock_t udp_fd, compact_pair_t *pair, time_t now,
    ...
    pair->resp_msg = msg;
    pair->resp_data_len = 0;
    if (len > 0 && data) {
        memcpy(pair->resp_data, data, len);
        pair->resp_data_len = len;
    }
...
static void retry_req_pending(sock_t udp_fd, time_t now) {
    ...
    while (curr != (compact_pair_t*)(void*)-1) {
        ...
        if (now - curr->req_sent_time >= MSG_REQ_RETRY_INTERVAL) {
...
static void retry_resp_pending(sock_t udp_fd, time_t now) {
    ...
    while (curr != (compact_pair_t*)(void*)-1) {
        ...
        if (now - curr->resp_sent_time >= MSG_RESP_RETRY_INTERVAL) {
...
static void handle_compact_signaling(sock_t udp_fd, uint8_t *buf, size_t len, struct sockaddr_in *from) {
    ...
    if (hdr->type == SIG_PKT_REGISTER) { const char* PROTO = "REGISTER";
        ...
        // 直接 HASH_FIND payload 前 64 字节，即 peer_key [local_peer_id(32)][remote_peer_id(32)]
        compact_pair_t *existing = NULL;
        ...
        if (existing && existing->instance_id == instance_id) {
            ...
            send_register_ack(udp_fd, from, from_str, status);
            return;
        }
        ...
        if (existing) i = (int)(existing - g_compact_pairs);
            ...
            for (i = 0; i < MAX_PEERS; i++) {
                if (!g_compact_pairs[i].valid) { 
                    ...
                    g_compact_pairs[i].resp_data_len = 0;
            ...
            // 无法分配槽位，发送错误 ACK
            if (i == MAX_PEERS) {
                send_register_ack(udp_fd, from, from_str, 2);
                return;
            }
        }
    ...
    else if (hdr->type == SIG_PKT_UNREGISTER) { const char* PROTO = "UNREGISTER";
    ...
    else if (hdr->type == SIG_PKT_ALIVE) { const char* PROTO = "ALIVE";
    ...
    // 格式: [hdr(4)][session_id(8)]，确认序号使用 hdr->seq
        ...
        if (ack_seq == 0) {
            ...
            if (pair && pair->valid) {
                ...
                // 标记为已确认，从待确认链表移除
                if (!pair->info0_acked && pair->info0_base_index == 0) {
    ...
    else if (hdr->type == SIG_PKT_MSG_REQ) { const char* PROTO = "MSG_REQ";
    ...
    else if (hdr->type == SIG_PKT_MSG_RESP) { const char* PROTO = "MSG_RESP";
    ...
    // 格式: [sid(2)]
    else if (hdr->type == SIG_PKT_MSG_RESP_ACK) { const char* PROTO = "MSG_RESP_ACK";
...
static void print_usage(const char *prog) {
    printf(LA_F("Usage: %s [port] [probe_port] [relay]", LA_F28, 28), prog);
    printf("\n\n");
    printf("%s\n", LA_S("Parameters:", LA_S18, 18));
    printf("%s\n", LA_S("  port         Signaling server listen port (default: 8888)", LA_S6, 6));
    printf("%s\n", LA_S("               - TCP: RELAY mode signaling (stateful/long connection)", LA_S3, 3));
    printf("%s\n", LA_S("               - UDP: COMPACT mode signaling (stateless)", LA_S4, 4));
    printf("%s\n", LA_S("  probe_port   NAT type detection port (default: 0=disabled)", LA_S7, 7));
    printf("%s\n", LA_S("               Used to detect symmetric NAT (port consistency)", LA_S5, 5));
    printf("%s\n", LA_S("  relay        Enable data relay support (COMPACT mode fallback)", LA_S8, 8));
    printf("\n%s\n", LA_S("Examples:", LA_S17, 17));
    printf(LA_F("  %s                    # Default config (port 8888, no probe, no relay)", LA_F20, 20), prog);
    printf("\n");
    printf(LA_F("  %s 9000               # Listen on port 9000", LA_F21, 21), prog);
    printf("\n");
    printf(LA_F("  %s 9000 9001          # Listen 9000, probe port 9001", LA_F22, 22), prog);
    printf("\n");
    printf(LA_F("  %s 9000 9001 relay    # Listen 9000, probe 9001, enable relay", LA_F23, 23), prog);
    printf("\n\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (opt) {
        fprintf(stderr, LA_F("Error: Unknown option '%s' (expected: 'relay')", LA_F26, 26), argv[1]);
        fprintf(stderr, "\n\n");
        print_usage(argv[0]);
        return -1;
    }
    ...
    if (argc > 1) {
        ...
        if (p == argv[1] || *p != '\0' || val <= 0 || val > 65535) {
            fprintf(stderr, LA_F("Error: Invalid port number '%s' (range: 1-65535)", LA_F24, 24), argv[1]);
            fprintf(stderr, "\n\n");
            print_usage(argv[0]);
            return 1;
        }
    ...
    if (argc > 2) {
        ...
        if (p == argv[2] || *p != '\0' || val < 0 || val > 65535) {
            fprintf(stderr, LA_F("Error: Invalid probe port '%s' (range: 0-65535)", LA_F25, 25), argv[2]);
            fprintf(stderr, "\n\n");
            print_usage(argv[0]);
            return 1;
        }
    ...
    if (argc > 3) {
        if (strcmp(argv[3], "relay") != 0) {
            fprintf(stderr, LA_F("Error: Unknown option '%s' (expected: 'relay')", LA_F26, 26), argv[3]);
            fprintf(stderr, "\n\n");
            print_usage(argv[0]);
            return 1;
        }
    ...
    if (argc > 4) {
        fprintf(stderr, "%s\n\n", LA_S("Error: Too many arguments", LA_S16, 16));
        print_usage(argv[0]);
        return 1;
    }
    ...
    while (g_running) {
        ...
        if (g_info0_pending_head && (now - last_compact_retry_check) >= COMPACT_RETRY_INTERVAL) {
            retry_info0_pending(udp_fd, now);
            last_compact_retry_check = now;
        }

        // 检查并重传 MSG RPC 包（每秒检查一次）
        if ((g_req_pending_head || g_resp_pending_head) && (now - last_compact_retry_check) >= MSG_REQ_RETRY_INTERVAL) {
