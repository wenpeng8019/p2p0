...
typedef int (*cw_handle_handshake_cb)(cw_client_t *client, uint8_t* hdr_buf, uint16_t hdr_len, uint8_t *payload, uint16_t payload_len);
...
typedef void (*ct_client_unreachable_cb)(cw_client_t *client, bool readOrWrite);
typedef void (*ct_error_item_cb)(cw_client_t *client, buf16_item_t* buffer_item);

//-----------------------------------------------------------------------------

struct custom_ws_ctx {
    const char*                     sub_protocol;           // WebSocket 子协议名，NULL 表示不协商
    cw_handle_handshake_cb          handle_handshake;       // 收到握手请求
    cw_handle_text_cb               handle_text;            // 收到 text 帧
    cw_handle_data_cb               handle_data;            // 收到 binary 帧
