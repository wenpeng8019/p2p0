...
enum {
    ...
    LA_S7,  /* "Shutting down...\n"  [server.c] */
    LA_S8,  /* "Signaling server listen port (TCP+UDP)"  [server.c] */
    LA_S9,  /* "Use Chinese language"  [server.c] */

    /* Formats (LA_F) */
    LA_F10,  /* "% Client closed connection (EOF on recv)\n"  [server.c] */
    LA_F11,  /* "% Client sent data before ONLINE_ACK completed\n"  [server.c] */
    ...
    LA_F87,  /* "MSG_RESP resent back to '%s', sid=%u, attempt %d/%d (ses_id=%u)\n" (%s,%u,%d,%d,%u)  [server.c] */
    LA_F88,  /* "% NAT probe disabled (bind failed)\n"  [server.c] */
    LA_F89,  /* "NAT probe socket listening on port %d\n" (%d)  [server.c] */
    LA_F90,  /* "NAT probe: %s (port %d)\n" (%s,%d)  [server.c] */
    LA_F91,  /* "ONLINE: '%s' came online (inst=%u)\n" (%s,%u)  [server.c] */
    LA_F92,  /* "ONLINE: '%s' new instance (old=%u, new=%u), destroying old\n" (%s,%u,%u)  [server.c] */
    LA_F93,  /* "ONLINE: '%s' reconnected (inst=%u), migrating fd\n" (%s,%u)  [server.c] */
    LA_F94,  /* "ONLINE: bad payload(len=%u, expected=%u)\n" (%u,%u)  [server.c] */
    LA_F95,  /* "ONLINE: duplicate from '%s'\n" (%s)  [server.c] */
    LA_F96,  /* "ONLINE_ACK sent to '%s'\n" (%s)  [server.c] */
    LA_F97,  /* "P2P Signaling Server listening on port %d (TCP + UDP)...\n" (%d)  [server.c] */
    LA_F98,  /* "% RPC_ERR: OOM\n"  [server.c] */
    LA_F99,  /* "Relay %s seq=%u: '%s' -> '%s' (ses_id=%u)\n" (%s,%u,%s,%s,%u)  [server.c] */
    LA_F100,  /* "Relay support: %s\n" (%s)  [server.c] */
    LA_F101,  /* "SYNC resent, %s <-> %s, attempt %d/%d (ses_id=%u)\n" (%s,%s,%d,%d,%u)  [server.c] */
    LA_F102,  /* "SYNC retransmit failed: %s <-> %s (gave up after %d tries)\n" (%s,%s,%d)  [server.c] */
    LA_F103,  /* "SYNC0: candidates exchanged '%.*s'(%d) <-> '%.*s'(%d)\n" (%d,%d)  [server.c] */
    LA_F104,  /* "SYNC0_ACK queue busy for '%s', drop\n" (%s)  [server.c] */
    LA_F105,  /* "SYNC_ACK queue busy for '%s', drop\n" (%s)  [server.c] */
    LA_F106,  /* "Send %s: auth_key=%llu, peer='%s'\n" (%s,%l,%s)  [server.c] */
    LA_F107,  /* "Send %s: base_index=%u, cands=%d, ses_id=%u, peer='%s'\n" (%s,%u,%d,%u,%s)  [server.c] */
    LA_F108,  /* "Send %s: cands=%d, ses_id=%u, peer='%s'\n" (%s,%d,%u,%s)  [server.c] */
    LA_F109,  /* "Send %s: mapped=%s:%d\n" (%s,%s,%d)  [server.c] */
    LA_F110,  /* "Send %s: max_cands=%d, relay=%s, msg=%s, public=%s:%d, probe=%d, auth_key=%llu, inst_id=%u\n" (%s,%d,%s,%s,%s,%d,%d,%l,%u)  [server.c] */
    LA_F111,  /* "Send %s: peer='%s', reason=%s, ses_id=%u\n" (%s,%s,%s,%u)  [server.c] */
    LA_F112,  /* "Send %s: rejected (no slot available)\n" (%s)  [server.c] */
    LA_F113,  /* "Send %s: ses_id=%u, peer=%s\n" (%s,%u,%s)  [server.c] */
    LA_F114,  /* "Send %s: ses_id=%u, sid=%u, msg=%u, data_len=%d, peer='%s'\n" (%s,%u,%u,%u,%d,%s)  [server.c] */
    LA_F115,  /* "Send %s: ses_id=%u, sid=%u, peer='%s', flags=0x%02x, code=%u, data_len=%d\n" (%s,%u,%u,%s,%u,%d)  [server.c] */
    LA_F116,  /* "Send %s: ses_id=%u, sid=%u, peer='%s'\n" (%s,%u,%u,%s)  [server.c] */
    LA_F117,  /* "Send %s: ses_id=%u, sid=%u, status=%u\n" (%s,%u,%u,%u)  [server.c] */
    LA_F118,  /* "Starting P2P signal server on port %d\n" (%d)  [server.c] */
    LA_F119,  /* "Timeout & cleanup for client '%s' (inactive for %.1f seconds)\n" (%s)  [server.c] */
    LA_F120,  /* "Unknown packet type 0x%02x from %s\n" (%s)  [server.c] */
    LA_F121,  /* "[Relay] %s for ses_id=%u: peer unavailable (dropped)\n" (%s,%u)  [server.c] */
    LA_F122,  /* "[Relay] %s for unknown ses_id=%u (dropped)\n" (%s,%u)  [server.c] */
    ...
    LA_F127,  /* "[Relay] %s: missing SESSION flag, dropped\n" (%s)  [server.c] */
    LA_F128,  /* "% [TCP] Failed to set client socket to non-blocking mode\n"  [server.c] */
    LA_F129,  /* "% [TCP] Max peers reached, rejecting connection\n"  [server.c] */
    LA_F130,  /* "[TCP] New connection from %s:%d\n" (%s,%d)  [server.c] */
    LA_F131,  /* "% [TCP] OOM: cannot allocate recv buffer for new client\n"  [server.c] */
    LA_F132,  /* "[UDP] %s recv from %s, len=%zu\n" (%s,%s)  [server.c] */
    LA_F133,  /* "[UDP] %s recv from %s, seq=%u, flags=0x%02x, len=%zu\n" (%s,%s,%u)  [server.c] */
    LA_F134,  /* "[UDP] %s send to %s failed(%d)\n" (%s,%s,%d)  [server.c] */
    LA_F135,  /* "[UDP] %s send to %s, seq=%u, flags=0x00, len=%d\n" (%s,%s,%u,%d)  [server.c] */
    LA_F136,  /* "[UDP] %s send to %s, seq=0, flags=0, len=%d\n" (%s,%s,%d)  [server.c] */
    LA_F137,  /* "[UDP] %s send to %s, seq=0, flags=0x%02x, len=%d\n" (%s,%s,%d)  [server.c] */
    LA_F138,  /* "[UDP] %s send to %s:%d failed(%d)\n" (%s,%s,%d,%d)  [server.c] */
    LA_F139,  /* "[UDP] %s send to %s:%d, seq=0, flags=0, len=%d\n" (%s,%s,%d,%d)  [server.c] */
    LA_F140,  /* "[UDP] %s send to %s:%d, seq=0, flags=0x%02x, len=%d, retries=%d\n" (%s,%s,%d,%d,%d)  [server.c] */
    LA_F141,  /* "bad payload len %u (type=%u)\n" (%u,%u)  [server.c] */
    LA_F142,  /* "bad payload len %u\n" (%u)  [server.c] */
    LA_F143,  /* "% net init failed\n"  [server.c] */
    LA_F144,  /* "probe UDP bind failed(%d)\n" (%d)  [server.c] */
    LA_F145,  /* "recv() failed: errno=%d\n" (%d)  [server.c] */
    LA_F146,  /* "select failed(%d)\n" (%d)  [server.c] */
    LA_F147,  /* "send(%s) failed: errno=%d\n" (%s,%d)  [server.c] */
    LA_F148,  /* "ses_id=%u busy (pending relay)\n" (%u)  [server.c] */
    LA_F149,  /* "ses_id=%u peer not connected (type=%u)\n" (%u,%u)  [server.c] */
    LA_F150,  /* "type=%u rejected: client not logged in\n" (%u)  [server.c] */
    LA_F151,  /* "unknown ses_id=%u (type=%u)\n" (%u,%u)  [server.c] */
    LA_F152,  /* "unsupported type=%u (ses_id=%u)\n" (%u,%u)  [server.c] */
